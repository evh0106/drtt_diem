 ...
 *
 * @ejb.bean name="ACoilRcvL2BakSeEJB" jndi-name="ACoilRcvL2BakSeEJB" type="Stateless"
...
public class ACoilRcvL2SeEJBSBean extends BaseSessionBean implements YfQueryIFOld, YfQueryIFOld2 {
...
//			String ydCrnSchId  	= commUtils.trim(rcvMsg.getFieldString("YD_CRN_SCH_ID")); //SCHEDULEID
	...
	*/
	public JDTORecord rcvF1YFL007(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "크레인작업지시요구[ACoilRcvL2SeEJBSBean.rcvF1YML007] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		...
		try 
		{
			...
			**********************************************************/
			if (jsSch.size() > 0) 
			{
				...
				**********************************************************/
				...
				{
					commUtils.printLog(logId, "크레인이 작업중 : 크레인작업진행상태 [" + ydWrkProgStat + "]", "SL");
				}
				
				/**********************************************************
				* 3.3 크레인작업지시(YFF1L004) 전문 생성
				**********************************************************/
				if ("1EYD99MM".equals(sYD_SCH_CD))  //분동코일
				{ 
					/**********************************************************
					* 3.3.1  분동코일 크레인작업지시(YFF1L004) 전문 생성
					**********************************************************/
				...
				{
					/**********************************************************
					* 3.3.2  설비추출일 경우, 권상위치 변경 유무 확인
					**********************************************************/
					if(("KD".equals(sYD_SCH_CD.substring(2, 4)) || "FD".equals(sYD_SCH_CD.substring(2, 4))
							||"QD".equals(sYD_SCH_CD.substring(2, 4))) 
							&& "L".equals(sYD_SCH_CD.substring(6,7)) )
					{
						JDTORecordSet chgSet = JDTORecordFactory.getInstance().createRecordSet("Temp");
						jrParam.setField("STL_NO", stlNo);
						jrParam.setField("YD_UP_WO_LOC", ydUpWoLoc);
						chgSet = commDao.select(jrParam, getEQPChangeYN, logId, methodNm, "크레인스케줄 조회");
						
						if(chgSet==null || chgSet.size()<1)
						{
							throw new Exception("설비트레킹 이상 [" + stlNo +", "+sYD_SCH_CD + "]");
						}
						
						//권상위치가 변경되었다면,지시 취소
						if("Y".equals(chgSet.getRecord(0).getFieldString("CHANGE_YN")))
						{
							String WorkId = "";
							String ProcessId = "";
							if("K".equals(sYD_SCH_CD.substring(2, 3)))
							{
								WorkId = "S";
							}
							else if("F".equals(sYD_SCH_CD.substring(2, 3)))
							{
								WorkId = "H";
							}
							else if("Q".equals(sYD_SCH_CD.substring(2, 3)))
							{
								WorkId = "E";
							}
							
							if("1".equals(sYD_SCH_CD.substring(5, 6)))
							{
								ProcessId = "3"; //추출
							}
							else if("2".equals(sYD_SCH_CD.substring(5, 6)))
							{
								ProcessId = "4"; //take out
							}
							
							
							JDTORecord cnclParam = JDTORecordFactory.getInstance().create();
							cnclParam.setField("YD_WBOOK_ID"		, ydWbookId );
							cnclParam.setField("YD_CRN_SCH_ID"		, ydCrnSchId);
							cnclParam.setField("YD_EQP_ID"			, ydEqpId   );
							cnclParam.setField("YD_SCH_CD"			, sYD_SCH_CD   );
							cnclParam.setField("YD_L2_RETURN_FLAG"	, "Y"       );
							cnclParam.setField("IS_LAST_SELECTED"	, "1");
							cnclParam.setField("IS_SCH_MTL"			, "Y"); // 스케줄 단위 취소
							
							//크레인스케줄 취소
							EJBConnector ejbConn = new EJBConnector("default", "ACoilJspBakSeEJB", this);
							JDTORecord jrRst = (JDTORecord)ejbConn.trx("trtCrnSchCncl", new Class[] { JDTORecord.class }, new Object[] { cnclParam });
							jrRtn = commUtils.addSndData(jrRtn, jrRst);

							//작업예약 취소
							ejbConn = new EJBConnector("default", "ACoilJspBakSeEJB", this);
							jrRst = (JDTORecord)ejbConn.trx("trtWrkBookCncl", new Class[] { JDTORecord.class }, new Object[] { cnclParam });
							jrRtn = commUtils.addSndData(jrRtn, jrRst);
							
							JDTORecord etrtParam = JDTORecordFactory.getInstance().create();
							etrtParam.setResultCode(logId);	//Log ID
							etrtParam.setResultMsg(methodNm);	//Log Method Name
							
							etrtParam.setField("JMS_TC_CD"         , "POYMJ004"               ); //JMSTC코드
							etrtParam.setField("JMS_TC_CREATE_DDTT", commUtils.getDateTime14()); //JMSTC생성일시
							etrtParam.setField("YardId"				, "1"   );
							etrtParam.setField("WorkId"				, WorkId   ); // 공정코드 S:SPM,H:HFL,D:결속대, 2SPM  처리
							etrtParam.setField("ProcessId"			, ProcessId   ); // 1 보급, 2 보급취소, 3 추출, 4 Take-Out, 5 Take-In
							etrtParam.setField("CoilNo"				, stlNo   );
							//ejbConn = new EJBConnector("default", "ACoilRcvL3BakSeEJB", this);
							//jrRst = (JDTORecord)ejbConn.trx("rcvPOYMJ004", new Class[] { JDTORecord.class }, new Object[] { etrtParam });
							jrRtn = commUtils.addSndData(jrRtn, etrtParam);
			...
			**********************************************************/
			...
			try {
				...
				//크레인작업실적응답 전문 전송
				EJBConnector resConn = new EJBConnector("default", "YfCommBakSeEJB", this);
	...
	*/
	public JDTORecord rcvF1YFL004(JDTORecord rcvMsg) throws DAOException {
		...
		try {
			...
			* 7. CTS 지시재편성
			**********************************************************/
			commUtils.printLog(logId,"modifier:"+modifier+", msgId:"+msgId+", ydEqpId:"+ydEqpId, "SL");
			if (modifier.equals(msgId)) 
			{
				//Backup 이 아니고 L2에서 인터페이스 수신된 경우만 응답 전문 전송여부 Check 
				if ("1XTC01".equals(ydEqpId) || "1XTC02".equals(ydEqpId)) 
				{
					commUtils.printLog(logId,"cts통과", "SL");
					
					//중계동 기준 SET
					JDTORecord param = JDTORecordFactory.getInstance().create();
					param.setResultCode(logId);	//Log ID
					param.setResultMsg(methodNm);	//Log Method Name
					param.setField("DTL_ITEM1"	, ("B".equals(ydEqpStat)?"X":"E")); //야드설비ID
					param.setField("MODIFIER"	, modifier       ); //수정자
					commDao.update(param, updPRI007, logId, methodNm, "설비상태  수정");

					//CTS스케줄 대사이 있다면, CTS스케줄 재기동
					JDTORecordSet schSet = commDao.select(jrParam, selectCTSLyr, logId, methodNm, "CTS스케줄 조회");
					if(schSet.size()>0)
					{
						jrRtn = commUtils.addSndData(jrRtn, yfComm.reCTSSchLevWo(jrParam));
					}
				}
			}
	...
	*/
	public JDTORecord rcvF1YFL006(JDTORecord rcvMsg) throws DAOException {
		String methodNm 	= "CTS작업지시요구[ACoilRcvL2SeEJB.F1YFL006] < " + rcvMsg.getResultMsg();
		String logId 		= rcvMsg.getResultCode();
		
		try {
			commUtils.printLog(logId, methodNm, "S+");
			commUtils.printParam(logId + "CTS작업지시요구(F1YFL006) 수신 ", rcvMsg);
			...
			**********************************************************/
			...
			JDTORecord jrRtn = commUtils.addSndData(commDao.getMsgL2("YFF1L017", jrYdMsg));	// CTS 대차 출발 지시
		
			
			//JDTORecord jrRtn = yfComm.getCTSToLoc(resMsg);
	...
	public JDTORecord rcvF1YFL008(JDTORecord rcvMsg) throws DAOException {
		...
		try {
			...
			**********************************************************/
			...
			commUtils.printLog(logId, " ★★★★   권상처리시작:" + "권상코일"+StockId+" 크레인 스케쥴 ID"+ ydCrnSchId, "S+");
			...
			* 3. 권상실적위치가 대차(하차)
			**********************************************************/
			if ("TC03".equals(ydUpWrLoc.substring(2, 6))) 
			{

				/**********************************************************
				* 3.1 대차하차스케쥴 조회
				**********************************************************/
				JDTORecordSet jsTC = commDao.select(jrParam, getAxYML008TcarSchUd, logId, methodNm, "대차하차스케쥴 조회");
				
				if (jsTC.size() < 1) 
				{
					commUtils.printLog(logId, "대차하차스케줄이 존재하지 않습니다.", "SL");
				}
				else
				{
					...
					**********************************************************/
					if ("Y".equals(tcarUdCmplYn)) 
					{
						...
						jrRtn = commUtils.addSndData(jrRtn, yfComm.trtTcarSchUdCmpl(jrYdMsg));
					...
					**********************************************************/
					else 
					{
						...
						//대차스케줄(하차) 수정
						commDao.update(jrParam, updAxYML005TcarSchUd, logId, methodNm, "대차스케줄(하차) 수정");
	
						//대차이송재료 삭제
						commDao.update(jrParam, updAxYML005TcarMtlDel, logId, methodNm, "대차이송재료 삭제");
				...
				else 
				{
					...
					**********************************************************/
					...
					else 
					{
						...
						commUtils.printLog(logId, methodNm+  "★★★★★  권상차량스케쥴:" + ydCarSchId +  " 차량하차완료여부:" + CarHaChaEnd +" ★★★★", "SL");
			...
			* 5.1 구내운송 소재차량상차개시(YMTSJ007) 전송
			*   - 야드차량사용구분이 구내운송(L)
			* 5.2 출하관리출하상차개시(YMDSJ006) 전송
			*   - 야드차량사용구분이 출하차량(G)
			* 5.3 차량스케줄 수정
			...
			**********************************************************/
			...
			/**********************************************************
			* 6. 권상실적위치가 DC(DC Line Off)
			**********************************************************/
			if ("DC".equals(ydUpWrLoc.substring(2, 4))) 
			{
				JDTORecord param = JDTORecordFactory.getInstance().create();
				param.setField("STL_NO" , StockId); 
				param.setField("MODIFIER" 	, modifier     ); 
				commDao.update(param, updLineOffTarget, logId, methodNm, "DC LINE OFF 여부 초기화"); 
			}
			...
			**********************************************************/
			...
			/**********************************************************
			* 9. 적재위치를 크레인으로 수정(분동코일)
			**********************************************************/
			if ("41".equals(ydSchCd.substring(4, 6))) 
			{
				jrParam.setField("YD_CRN_SCH_ID"   , ydCrnSchId);
				...
				commDao.update(jrParam, updAxYML008UpStkLyrToCRForScr, logId, methodNm, "크레인 LAYER 수정");
			} 
			/**********************************************************
			* 10. 적재위치를 크레인으로 수정(분동코일 이외)
			**********************************************************/
			else 
			{
				/**********************************************************
				...
				**********************************************************/
				...
				if (ydUpWrLoc.substring(2, 4).equals("KD")||ydUpWrLoc.substring(2, 4).equals("FD")
						||ydUpWrLoc.substring(2, 4).equals("QD")||ydUpWrLoc.substring(2, 4).equals("DC")) 
				{
					jsStklyrChk = commDao.select(jrParam, getAxYML008UpStkLyrCR2, logId, methodNm, "권상정보 조회");
				}
				else
				{
			...
			**********************************************************/
			...
			/**********************************************************
			* 13. 조업L2 송신있는지 확인 필요. kbs
			*   - 분기 LINE OFF 스케줄이면  분기 CF1BP04 (Conveyor COIL Line Off 완료) 송신 
			*   - 분기 Take Out 스케줄이면  분기 CF1BP05 (Conveyor COIL Take Out 완료) 송신 
			// 분기 LINE OFF 
			if (ydSchCd.substring(2, 8).equals("DC01LM")) {
				jrRtn = commUtils.addSndData(jrRtn, commDao.getMsgL2("CF1BP04", jrParam));
			}
			// 분기 Take Out
			if (ydSchCd.substring(2, 8).equals("DC03LM")) {
				jrRtn = commUtils.addSndData(jrRtn, commDao.getMsgL2("CF1BP05", jrParam));
			}
			**********************************************************/
			...
			**********************************************************/
			...
			sndL2Msg.setField("MSG_GP"			, "I"); //전문구분
			sndL2Msg.setField("YD_STK_COL_GP"  	, ydUpWrLoc.substring(0, 6)); //야드적치열구분
			sndL2Msg.setField("YD_STK_BED_NO"  	, ydUpWrLoc.substring(6, 8)); //야드적치Bed번호
			sndL2Msg.setField("YD_GP"          	, ydUpWrLoc.substring(0, 1)                                 ); //야드구분
			sndL2Msg.setField("STL_NO"       	, StockId); //재료번호

			//전송Data 생성
			if (StockId.startsWith("S")) {
				jrRtn = commUtils.addSndData(jrRtn,commDao.getMsgL2("YFF1L002_SCRAP", sndL2Msg));
			} else {
			...
			*************************************************************/
			...
			/**********************************************************
			* 17. 권상실적위치가 SADDLE 이라면, 대차 이동 지시 호출
			*    - SADDLE이 가득차 CTS대차가 휴식중 이라면, 
			*      이 시점에서 다시 CTS대차를 기동시킨다. 
			**********************************************************/
			if ("SL".equals(ydUpWrLoc.substring(2, 4))
					||"SR".equals(ydUpWrLoc.substring(2, 4))) 
			{
				JDTORecord ctsParam = JDTORecordFactory.getInstance().create();
				ctsParam.setResultCode(logId);	//Log ID
				ctsParam.setResultMsg(methodNm);	//Log Method Name
				ctsParam.setField("MODIFIER" , modifier     ); 
				ctsParam.setField("BAY_GP" 	 , ydUpWrLoc.substring(1, 2)); 
				JDTORecordSet ctsSet = commDao.select(ctsParam, getCTSEqpId, logId, methodNm, "cts대차 설비 조회"); 
		    	if(ctsSet.size()>0)
		    	{
		    		ctsParam.setField("YD_EQP_ID" 	, ctsSet.getRecord(0).getFieldString("YD_EQP_ID")); 
					jrRtn = commUtils.addSndData(jrRtn, yfComm.getCTSToLoc(ctsParam));
		    	}
			}		
			/*****************************************
			 * 확인 필요. kbs
			 *****************************************/
			/*// 신고도화 내용 추가
			String sAPP101_YN = yfComm.ACoilApplyYn("APP101","1","CAR");
			if (sAPP101_YN.equals("Y")) {
				...
				jrYdMsg1.setField("YD_EVENT_GP"		, ydEqpId);
				EJBConnector ejbConn = new EJBConnector("default", "BCoilL2RcvBakSeEJB", this);
				String RtnCd = (String)ejbConn.trx("procEqpIdEventCar", new Class[] { JDTORecord.class }, new Object[] { jrYdMsg1 });
				
			}*/
			
			
			/******************************************
			 * HFL2 보급/추출 시 다음 작업예약 스케쥴 생성
			 * 박판도 권상실적시, 작업예약 필요하면 이 시점에서.. kbs
			 ******************************************/
			/*if ("HS".equals(ydSchCd.substring(2 , 4)) ){
				//명령선택기동
				JDTORecord jrYdMsg2 = JDTORecordFactory.getInstance().create();
				jrYdMsg2.setResultCode(logId);	//Log ID
				jrYdMsg2.setResultMsg(methodNm);	//Log Method Name
				jrYdMsg2.setField("JMS_TC_CD"			, "YMYMJ302"); 
				jrYdMsg2.setField("JMS_TC_CREATE_DDTT"	, commUtils.getDateTime14()); //JMSTC생성일시				
				jrYdMsg2.setField("YD_WBOOK_ID"			, ""); //야드작업예약ID
				jrYdMsg2.setField("YD_SCH_CD"  			, ydSchCd); //야드스케쥴코드
				jrYdMsg2.setField("YD_EQP_ID"  			, ""); //야드설비ID		
				jrRtn = commUtils.addSndData(jrRtn, jrYdMsg2);
...
//				if ("Y".equals(sAPPLY038) && "0000".equals(resMsg.getFieldString("YD_L3_HD_RS_CD"))) {
			...
			**********************************************************/
			...
			else 
			{
				...
				commUtils.printLog(logId, " ★★★★   권하처리시작:" + "권하코일:"+StockId+" 작업모드2:"+ ydEqpWrkMode2, "S+");
				// 실적에서 MODE2 가 안올라 오는 경우 DB에 있는 DATA SETUP
				if ("".equals(ydEqpWrkMode2)) 
				{
					ydEqpWrkMode2 = tmpMode2;
			...
			commUtils.printLog(logId, " ★★★★   권하처리시작:" +"스케줄코드:" +ydSchCd +"권상위치:" +ydUpWrLoc +"권하위치:" +ydDnWrLoc + " 권하코일:"+StockId+" 크레인 스케쥴 ID: "+ ydCrnSchId, "S+");
			...
			* 4. 권하실적위치가 대차(상차)
			**********************************************************/
			if ("TC03".equals(ydDnWrLoc.substring(2, 6))) 
			{
				...
				**********************************************************/
				...
				JDTORecordSet jsTc = commDao.select(jrParam, getA7YML009TCarSchLd, logId, methodNm, "대차상차스케쥴 조회 조회!!!");  
				
				
				if (jsTc.size() <1) 
				{
					commUtils.printLog(logId, methodNm + "오류:크레인권하실적 상차 대차스케줄 DB정보 없음", "SL");
				}
				else
				{
					JDTORecord jrTc = jsTc.getRecord(0);
					
					if("B".equals(jrTc.getFieldString("YD_EQP_PROG_STAT")))
					{
						resMsg.setField("YD_L3_HD_RS_CD", "DN02"); //야드L3처리결과코드
						resMsg.setField("YD_L3_MSG"     , "확장대차 고장"   ); //야드L3MESSAGE
						throw new Exception("확장대차 고장");
					}
					
					if(!ydDnWrLoc.substring(1, 2).equals(jrTc.getFieldString("YD_CURR_BAY_GP")))
					{
						resMsg.setField("YD_L3_HD_RS_CD", "DN02"); //야드L3처리결과코드
						resMsg.setField("YD_L3_MSG"     , "권하위치에 확장대차가 존재 안함"   ); //야드L3MESSAGE
						throw new Exception("권하위치에 확장대차가 존재 안함");
					}
					
					/**********************************************************
					* 4.2 작업예약 야드작업계획대차 수정
					*    -계획대차는 작업예약시 만들어지는것 아닌지?? kbs
					**********************************************************/
					commDao.update(jrParam, updA7YML009WbTCar, logId, methodNm, "작업예약 야드작업계획대차 수정");
					...
					**********************************************************/
					...
					commDao.update(jrParam, updA7YML009TCarMtlIns, logId, methodNm, "대차이송재료 등록");

					
					
					String tcarLdCmplYn  = commUtils.trim(jrTc.getFieldString("TCAR_LD_CMPL_YN"));    	//대차상차완료여부
					...
					jrParam.setField("YD_CARUD_STOP_LOC", tcarUdStopLoc);
						...
						if (jsDnTc.size() > 0) 
						{
							...
							for (int i = 0; i < jsDnTc.size(); i++) 
							{
								...
								**********************************************************/
								...
								jrParamTc.setField("YD_DN_WR_LOC"        , ydDnWrLoc       ); //야드권하실적위치
								commDao.update(jrParamTc, updA7YSL009WbTCarIns, logId, methodNm, "작업예약 등록");
								...
								**********************************************************/
								...
								commDao.update(jrParamTc, insA7YML009WbMtlTCarIns, logId, methodNm, "작업예약재료 등록");							        
							}
						}
					}
					
					/**********************************************************
					* 4.5 대차스케줄 야드설비작업매수, 중량, 야드차량진행상태, 야드상차개시일시, 야드상차완료일시 등 수정   
					**********************************************************/
					jrParam.setField("YD_CARUD_WRK_BOOK_ID", ydTcWrkBookId); //야드하차작업예약ID
					commDao.update(jrParam, updA7YML009TcarSchLd, logId, methodNm, "대차스케줄 수정");
			...
			if ("PT".equals(ydDnWrLoc.substring(2, 4)) && !"PT3".equals(ydSchCd.substring(2, 5))) //차량동간이적은 제외
			{
				...
				**********************************************************/
				...
				else
				{
					...
					/**********************************************************
					* 5.2 상차 차량이송재료(TB_YD_CARFTMVMTL) 등록
					**********************************************************/
					recPara = JDTORecordFactory.getInstance().create();
					recPara.setField("YD_CAR_SCH_ID" , ydCarSchId ); 
					recPara.setField("YD_CRN_SCH_ID" , ydCrnSchId ); 
					...
					**********************************************************/
					if ("G".equals(ydCarUseGp)) {
						...
						//검수작업 확인						
						recPara.setField("STL_NO" , StockId );   	
						commDao.update(recPara, insCarExaminationjlNEW, logId, methodNm, "검수등록 ");
					}
						...
						**********************************************************/
						if ("L".equals(ydCarUseGp)) 
						{
							...
							* 2020 03 18 삭제 
		    				...
		    				sndL2Msg.setField("MSG_GP"			, "I"                         ); //전문구분
		    				sndL2Msg.setField("YD_STK_COL_GP"    , ydDnWrLoc.substring(0, 6));
		    				sndL2Msg.setField("YD_STK_BED_NO"    , "");
		    				commUtils.printParam(logId, sndL2Msg);
		    				jrRtn = commUtils.addSndData(jrRtn,commDao.getMsgL2("YFF1L001", sndL2Msg)); 

		    				commUtils.printLog(logId, "[" + methodNm + "] 저장위치제원 : 코일야드L2 로 송신 열구분[" + ydDnWrLoc.substring(0, 6) + "] - 저장위치제원 : 코일야드L2 로 송신 호출 성공"+jrRtn.size(), "SL");
		    				**********************************************************/
							...
							**********************************************************/
							if (jsPofrto.size() > 0 ) 
							{
								...
								**********************************************************/
								...
								{
									EJBConnector ejbConn1 = new EJBConnector("default","YfCommCarMvBakSeEJB",this); //오타 있음..kbs
			...
			**********************************************************/
				...
				**********************************************************/
				...
				else
				{
					...
					**********************************************************/
					...
					/**********************************************************
					* 6.3 구내이송 소재이송이면 이송완료처리
					**********************************************************/
					if (ydCarUseGp.equals("L") && !stlAppearGp.equals("Y")) 
					{
						jrYdMsg			= JDTORecordFactory.getInstance().create();
    					jrYdMsg = JDTORecordFactory.getInstance().create();
						jrYdMsg.setResultCode(logId);	//Log ID
			...
			/**********************************************************
			* 7. DC Take in 경우 stock 갱신
			**********************************************************/
			else if ("DC".equals(ydDnWrLoc.substring(2, 4)))
			{
				recPara = JDTORecordFactory.getInstance().create();
				recPara.setResultCode(logId);	//Log ID
				recPara.setResultMsg(methodNm);	//Log Method Name
				recPara.setField("MODIFIER" 		, modifier     ); 
				recPara.setField("STL_NO" 			, StockId ); 
				commDao.update(recPara, updStockDCTakeIn, logId, methodNm, "하차 차량스케줄 수정  ");
			}
			
			
			if ("TC21LM".equals(ydSchCd.substring(2))) // CTS난방코일 하차라면	
			{
				JDTORecord heatParam = JDTORecordFactory.getInstance().create();
				heatParam.setResultCode(logId);	//Log ID
				heatParam.setResultMsg(methodNm);	//Log Method Name
				heatParam.setField("HEATING_COIL_YN", "N"); 
				heatParam.setField("MODIFIER"     	, modifier); 
				heatParam.setField("STL_NO"    		, StockId); 
				
				commDao.update(heatParam, updHeatCoilYn, logId, methodNm, "난방코일 초기화");
			}
			
			
			/**********************************************************
			* 8. 설비(야드설비상태, 권하완료(D)) 수정
			**********************************************************/
			commDao.update(jrParam, updStatEqp2, logId, methodNm, "설비상태 수정");
			...
			**********************************************************/
			if ("N".equals(CmbnCarldSndYn) 
					&&!"D".equals(ydDnWrLoc.substring(2, 3))
					&&!"K".equals(ydDnWrLoc.substring(2, 3))
					&&!"F".equals(ydDnWrLoc.substring(2, 3))
					&&!"Q".equals(ydDnWrLoc.substring(2, 3))
			...
			**********************************************************/
			...
			/**********************************************************
			* 15. SPM/HFL 설비위 권하처리
			*   - 조업송신(YMPOJ161),EQL조업송신(YDHRJ005),품질 열연정정입측보급실적(YDQMJ002)
			**********************************************************/
			if ("KE".equals(ydDnWrLoc.substring(2, 4))||"FE".equals(ydDnWrLoc.substring(2, 4))||"QE".equals(ydDnWrLoc.substring(2, 4)))
			{
				
				//품질 열연정정입측보급실적(YDQMJ002)
				jrRtn = commUtils.addSndData(jrRtn, commDao.getMsgL3("YDQMJ002", jrParam));
				
				if("QE".equals(ydDnWrLoc.substring(2, 4)))
    			{
    				JDTORecord jrYDHRJ005 = JDTORecordFactory.getInstance().create();
					
					jrYDHRJ005.setField("JMS_TC_CD",        "YDHRJ005"); 							//열연조업 L3 정정보급완료 실적  전문코드
					jrYDHRJ005.setField("STL_NO",     	StockId);								//재료번호
					jrYDHRJ005.setField("YD_EQP_ID",     ydDnWrLoc.substring(0, 6));			//야드설비id :"1EQE01"
	    			jrYDHRJ005.setField("YD_STK_BED_NO", "01");			//야드적치베드번호
					jrYDHRJ005.setField("YD_DN_CMPL_DT", YmCommonUtil.getTcDate("yyyyMMddHHmmss"));	//야드권하완료일시
					jrYDHRJ005.setField("TREAT_GP", "1");	
					jrRtn = commUtils.addSndData(jrRtn, jrYDHRJ005);		
    			}
				else
				{
					...
					{
						commUtils.printLog(logId, methodNm+  "보급실적 송신:" + StockId, "SL");
					}
	    			else 
					{
			...
			/**********************************************************
			* 16. 출하관리 코일제품이적작업실적 전송  - YDDMR004
			*     - 스케줄 코드 점검 필요 KBS
			**********************************************************/
			if ("Y".equals(stlAppearGp)) 
			{
				// 동내이적, 차량동간이적, 대차동간이적
	        	if ( ("YD".equals(ydSchCd.substring(2, 4)) && ydSchCd.substring(6).equals("MM") )				/* 일반야드 이적 */
	        		|| ( "PT3".equals(ydSchCd.substring(2, 5)) && ydSchCd.substring(6).equals("LM") ) 
	        		|| ( "TC".equals(ydSchCd.substring(2, 4)) && ydSchCd.substring(6).equals("LM") ) ) 
	        	{	
			...
			**********************************************************/
			...
			/**********************************************************
			* 18. 코일공통 수정 : 별도 트렌젝션 처리
			*    -공통 Table, 저장품, 작업이력 등록 (순서 변경 안됨)
			**********************************************************/
			if (!"44".equals(ydSchCd.substring(4, 6))) //분동코일이면 작업이력 등록 할 필요 없음
			{
				jrParam	= JDTORecordFactory.getInstance().create();
				jrParam.setResultCode(logId);		//Log ID
				jrParam.setResultMsg(methodNm);	//Log Method Name
				...
				//저장품 수정** 삭제 KBS
				//commDao.update(jrParam, updAxYDL009Stock, logId, methodNm, "저장품 수정");
				
				
				/**********************************************************
				* 19. 작업실적 Table에 권하실적을 Update
				*    -작업이력 등록
				**********************************************************/
				jrParam.setField("YD_CRN_SCH_ID"   , ydCrnSchId); 
			...
			if (rst.size() > 0) 
			{
				...
				commUtils.printLog(logId, "○○○ YMYMJ662 전문 세팅 END", "[INFO]");
			}
			
			
			/**********************************************************
			* 21. 강제권하시 저장품제원 전문 전송(YFF1L002)
			**********************************************************/
			if ("5".equals(ydWrkProgStat)) 
			{
			...
			sndL2Msg.setField("YD_INFO_SYNC_CD"	, "5"); //야드정보동기화코드
			sndL2Msg.setField("MSG_GP"			, "I"); //전문구분
			sndL2Msg.setField("YD_STK_COL_GP"  	, ydDnWrLoc.substring(0, 6)); //야드적치열구분
			sndL2Msg.setField("YD_STK_BED_NO"  	, ydDnWrLoc.substring(6, 8)); //야드적치Bed번호
			sndL2Msg.setField("YD_GP"          	, ydDnWrLoc.substring(0, 1)                                 ); //야드구분
			sndL2Msg.setField("STL_NO"       	, StockId); //재료번호
			...
			*************************************************************/
			...
			/*******************************************************
			 * SPM2보급스케줄 이면서 권하위치가 밴딩커팅장일 경우
			 * - LYR테이블 수정
			if ("3EKE02UM".equals(ydSchCd) && "3EBD0101".equals(ydDnWrLoc)) 
			{
				jrParam.setField("YD_CRN_SCH_ID", ydCrnSchId);
				JDTORecordSet jsSchInfo = commDao.select(jrParam, getSPM2SchInfo, logId, methodNm, "3EKE02MM 해당 스케줄 검색");
				if (jsSchInfo.size() == 0) 
				{
					commUtils.printLog(logId, "3EKE02MM 스케줄 존재하지 않음", "[Exception]");
					return jrRtn;
				}

				String sSTL_NO = jsSchInfo.getRecord(0).getFieldString("STL_NO");
				
				jrParam.setField("STL_NO", sSTL_NO);
				commDao.update(jrParam, "com.inisteel.cim.ym.bcoil.dao.BCoilDAO.updStackLyrForSPM2", logId, methodNm, "권상,권하위치 Layer 수정");
			}
			 *****************************************************/
			...
			**********************************************************/
			...
			if ("Y".equals(sAPP200_YN)) 
			{
				...
				if ("PT3".equals(ydSchCd.substring(2, 5))) 
				{
					...
					if ("Y".equals(SndYn)) 
					{
						...
						EJBConnector ejbConn1 = new EJBConnector("default", "YfCommCarMvBakSeEJB", this);
						JDTORecord jrRtn1 = (JDTORecord)ejbConn1.trx("procTraillerMoveSch", new Class[] { JDTORecord.class }, new Object[] { recInTemp1 });
						jrRtn = commUtils.addSndData(jrRtn, jrRtn1);
					}	
				}
			}	
			...
			/**********************************************************
			* 24. 권하실적위치가 SADDLE 이라면, CST 대차 스케줄 호출
			**********************************************************/
			if ("SL".equals(ydDnWrLoc.substring(2, 4)) 
					|| "SR".equals(ydDnWrLoc.substring(2, 4))) 
			{
				jrYdMsg = JDTORecordFactory.getInstance().create();
				jrYdMsg.setResultCode(logId);							//Log ID
				jrYdMsg.setResultMsg(methodNm);							//Log Method Name
				...
				jrYdMsg.setField("YD_CARLD_STOP_LOC", ydDnWrLoc); 		//코일이 권하된 SADDLE위치
				jrYdMsg.setField("MODIFIER"    		, modifier   ); 	//수정자
				jrYdMsg.setField("YD_WBOOK_ID"    	, ydWbookId   ); 	//작업예약
				
				jrRtn = commUtils.addSndData(jrRtn, yfComm.trtCTSSchLevWo(jrYdMsg));
			}
			
			
			
			/**********************************************************
			* 25. 크레인작업지시요구 전문 호출(F1YFL007)
	...
	*/
	public JDTORecord rcvF1YFL010(JDTORecord rcvMsg) throws DAOException {
		...
		try {
			...
			**********************************************************/
			...
			/**********************************************************
			* 4. 저장품 CARUNLOAD_PUT_LOC 수정 - 박판은 필요 없을 듯..KBS
	 ...
	 *		@ejb.transaction type="RequiresNew"
	*/
	public JDTORecord rcvF1YFL011_old(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "대차이동실적[ACoilRcvL2SeEJB.rcvF1YFL011] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		...
		try {
			...
			String msgId        = commUtils.getMsgId(rcvMsg); //EAI, JMS, HTTP(출하관리 등) 수신 전문 I/F ID
			String ydEqpId      = commUtils.trim(rcvMsg.getFieldString("YD_EQP_ID"        )); //야드설비ID
			String ydTcMoveGp   = commUtils.trim(rcvMsg.getFieldString("YD_MOVE_GP"       )); //야드대차이동구분(S:출발, M:이동, E:도착)
			...
			**********************************************************/
			...
			/**********************************************************
			* 2. 영대차일때, 출발/도착 정보 이상 Check
			**********************************************************/
			String sAPP049_YN = yfComm.ACoilApplyYn("APP049","1","1");
			if("Y".equals(sAPP049_YN))
			{
				...
				jrParam1.setField("YD_EQP_ID"      		, ydEqpId     ); //야드설비ID
				jrParam1.setField("YD_MOVE_GP"     		, ydTcMoveGp  ); //야드대차이동구분
				jrParam1.setField("YD_TCAR_CURR_BAY"    , TcCurrBay   ); //대차현재동
				JDTORecordSet jsTcChk = commDao.select(jrParam1, getTcarSchByEqpidCheck, logId, methodNm, "대차스케줄CHECK 조회");
				
				
				if (jsTcChk.size() == 0)
				{
//					throw new Exception("영대차 출발/도착 정보 이상");
					commUtils.printLog(logId, "영대차 출발/도착 정보 이상", "SL");
					commUtils.printLog(logId, methodNm, "S-");
					return null;
				}
			}
			...
			* 3. 설비 야드현재동 수정
			**********************************************************/
			String ydStkColGp = ydEqpId.substring(0, 1) + TcCurrBay + ydEqpId.substring(2); //야드적치열구분(현재동)
			...
			commDao.update(jrParam, updYdEqpCurrBay, logId, methodNm, "야드현재위치수정");
			...
			**********************************************************/
			...
			JDTORecordSet jsChk = commDao.select(jrParam, getTcarSchByEqpid, logId, methodNm, "대차스케줄상태 조회");
			if (jsChk.size() < 1) {
				throw new Exception("대차 스케줄이 존재하지 않습니다.[" + ydEqpId + "]");
			}
			
			/**********************************************************
			* 5. 대차스케쥴 정보 수정
			*    - 야드상차/하차 출발/도착 위치/시간, 설비상태 
			**********************************************************/
			...
			commDao.update(jrParam, updTcarProgStat, logId, methodNm, "대차스케줄상태 수정");
			...
			* 6. 대차스케줄 조회
			**********************************************************/     
			JDTORecordSet jsTcar = commDao.select(jrParam, getTcarSch, logId, methodNm, "대차스케줄조회");
			...
			String bRunF1YFL007		 = ""; //명령선택기동여부
			String crnId		 	 = ""; //작업크레인ID
...
/*	고도화 제외 KBS			
//신고도화 내용 추가: 도착시 적용
				String sAPP101_YN = yfComm.ACoilApplyYn("APP101","1","TCXX");
				if ("Y".equals(sAPP101_YN)) {
	
					if ("E".equals(ydTcMoveGp)) {
						String sAPP101_E_YN = "";
						String sAdvGp     = "";
						
						if (ydEqpId.substring(2, 6).equals("TC01")) {
							sAPP101_E_YN = yfComm.ACoilApplyYn("APP101","1","TC01");
							sAdvGp     = "3XTC01";
						} else if (ydEqpId.substring(2, 6).equals("TC02")) {
							sAPP101_E_YN = yfComm.ACoilApplyYn("APP101","1","TC02");
							sAdvGp     = "3XTC02";
						} else if (ydEqpId.substring(2, 6).equals("TC03")) {
							sAPP101_E_YN = yfComm.ACoilApplyYn("APP101","1","TC03");
							sAdvGp     = "3XTC03";
						} else if (ydEqpId.substring(2, 6).equals("TC04")) {
							sAPP101_E_YN = yfComm.ACoilApplyYn("APP101","1","TC04");
							sAdvGp     = "3XTC04";
						} else if (ydEqpId.substring(2, 6).equals("TC05")) {
							sAPP101_E_YN = yfComm.ACoilApplyYn("APP101","1","TC05");
							sAdvGp     = "3XTC05";
						}
							
						if ("Y".equals(sAPP101_E_YN) && "E".equals(ydTcMoveGp)) {

							JDTORecord jrYdMsg1	= JDTORecordFactory.getInstance().create();
							jrYdMsg1.setResultCode(logId);	//Log ID
							jrYdMsg1.setResultMsg(methodNm);	//Log Method Name
							jrYdMsg1.setField("YD_EVENT_GP"		, sAdvGp);
							EJBConnector ejbConn = new EJBConnector("default", "BCoilL2RcvBakSeEJB", this);
							String RtnCd = (String)ejbConn.trx("procEqpIdEvent", new Class[] { JDTORecord.class }, new Object[] { jrYdMsg1 });
						}	
					}	
				}
*/				
			...
			**********************************************************/
			if ("A".equals(ydCarProgStat)) 
			{
				...
				*    -하차작업 예약이 없는 건수만 조회
				**********************************************************/
				JDTORecordSet jsDnTc = commDao.select(jrParam, getA7YML011WbTCar, logId, methodNm, "권하대상재!!!");
				String ydNewWbookId = ""; 
				
				
				/**********************************************************
				* 7.2 크레인 하차 예약 없으면 생성
				**********************************************************/
				if (jsDnTc.size() > 0 ) 
				{
					...
					for(int nIdx=0; nIdx < jsDnTc.size(); nIdx++) 
					{
						...
						**********************************************************/
						...
						jrParamTc.setField("STL_NO" 			 , commUtils.trim(jrDnTc.getFieldString("STL_NO"))); 
						commDao.update(jrParamTc, insA7YML011WbTCarIns, logId, methodNm, "작업예약 등록");

						/**********************************************************
						* 7.2.2 작업예약재료 등록
						**********************************************************/
						jrParamTc.setField("YD_STK_COL_GP" 	, ydEqpId); 
						jrParamTc.setField("YD_STK_BED_NO" 	, "01"); 
						jrParamTc.setField("YD_STK_LYR_NO" , "01"); 
					...
					/**********************************************************
					* 7.2.3 대차스케줄에 하차작업예약id 등록
					**********************************************************/
					jrParam.setField("YD_CARUD_WRK_BOOK_ID", ydNewWbookId); //신야드대차스케쥴ID
					jrParam.setField("YD_TCAR_SCH_ID", ydTcarSchId);
					commDao.update(jrParam, updTcarProgStatCarudWrkbook, logId, methodNm, "대차스케줄에 하차작업예약id 등록");
				
				}					
									
				
				/**********************************************************
				* 7.3 영대차출발이면 출발위치 적치Bed 비활성화 처리
				**********************************************************/
				jrParam.setField("YD_STK_COL_GP"         , ydCarldStopLoc); //야드적치열구분
				jrParam.setField("YD_STK_BED_ACTIVE_STAT", "C"       ); //야드적치Bed활성상태(비활성화)
				commDao.update(jrParam, updStatStkBedActByCol, logId, methodNm, "출발위치 Bed 비활성화");
				
				
				/**********************************************************
				* 7.4 영대차출발이면 출발위치 적치단 재료 삭제, 적치단 비활성화 처리
				*     - 이 시점에서 재료가 적치단에서 사라짐.. 대차이송재료로만 남음. KBS
				**********************************************************/
				commDao.update(jrParam, updYdStkLyrClr1, logId, methodNm, "출발위치 단 비활성화");
			...
			**********************************************************/
			else if ("B".equals(ydCarProgStat)) 
			{
				...
				**********************************************************/
				...
				/**********************************************************
				* 8.2 크레인 하차 예약 없으면 생성
				*    -백업 또는 사용자가 작업예약까지 삭제 했을 경우 
				**********************************************************/
				if (jsDnTc.size() > 0 ) 
				{
					...
					/**********************************************************
					* 8.2.3 대차스케줄에 하차작업예약id 등록
					**********************************************************/
					jrParam.setField("YD_CARUD_WRK_BOOK_ID", ydNewWbookId); //신야드대차스케쥴ID
					jrParam.setField("YD_TCAR_SCH_ID", ydTcarSchId);
					commDao.update(jrParam, updTcarProgStatCarudWrkbook, logId, methodNm, "대차스케줄상태 수정");
				
					ydCarudWrkbookId = ydNewWbookId;// 스케줄 기동용 작업예약ID
				}
				
				
				
				/**********************************************************
				* 8.3 크레인스케줄기동여부 'Y'이면, 야드하차작업예약ID
				**********************************************************/
				if ("Y".equals(crnSchYn)) {
					ydWbookId = ydCarudWrkbookId;
				}

				
				/**********************************************************
				* 8.4 영대차도착이고 적치Bed 비활성화 이면 활성화
				**********************************************************/
				jrParam.setField("YD_STK_COL_GP", ydStkColGp); //적치Bed 야드적치열구분(현위치)
				if (!"L".equals(ydStkBedActStatUd)) 
				{
					//현위치 적치Bed 활성화 처리
					jrParam.setField("YD_STK_BED_ACTIVE_STAT", "L"); //적치Bed 야드적치Bed활성상태(활성)
					commDao.update(jrParam, updStatStkBedActByCol, logId, methodNm, "도착위치 Bed 활성화");
				}

				
				/**********************************************************
				* 8.5 하차위치 적치단 재료번호 등록 
				*    -혹시 정보가 맞지 않을 수도 있으므로 무조건 Update
				*    -적치단active는 적치가능(왜 E인지? L이아니라..), stat는 적치중 kbs
				**********************************************************/
				commDao.update(jrParam, updStkLyrTCarStl, logId, methodNm, "도착위치 단 활성화");
				...
				**********************************************************/
				...
				jrParam1.setField("YD_STK_BED_ACTIVE_STAT", "C"       ); //야드적치Bed활성상태(비활성화)
				JDTORecordSet jsTcar1 = commDao.select(jrParam1, getTcarSchStaErrChk, logId, methodNm, "대차스케줄조회");
				...
				**********************************************************/
				...
				{	
					...
					//상차지 정보 Clear 추가
					if ("Y".equals(szCLEAR_YN)) 
					{
						commDao.update(jrParam1, updStatStkBedActByCol, logId, methodNm, "출발위치 BED 비활성화");
						   
						commDao.update(jrParam1, updYdStkLyrInActiveTC, logId, methodNm, "출발위치 단 비활성화");
					}	
				}
			} 
			...
			**********************************************************/
			else if ("1".equals(ydCarProgStat)) 
			{
				...
				/**********************************************************
				* 9.1 공대차출발이면 출발위치 적치Bed 비활성화 처리
				**********************************************************/
				jrParam.setField("YD_STK_COL_GP"          , ydStkColGp); //야드적치열구분
				jrParam.setField("YD_STK_BED_ACTIVE_STAT" , "C"       ); //야드적치Bed활성상태(비활성화)
				commDao.update(jrParam, updStatStkBedActByCol, logId, methodNm, "출발위치 BED 비활성화");

				commDao.update(jrParam, updYdStkLyrInActiveTC, logId, methodNm, "출발위치 단 비활성화");

			} 
			
			
			/**********************************************************
			* 10. 공대차도착 : 상차도착(2) or 상차대기(0)
			**********************************************************/
			else 
			{
				ydSchReqGp = "6"; 
				...
				/**********************************************************
				* 10.1 크레인스케줄기동여부 'Y'이면, 야드상차작업예약ID set
				**********************************************************/
				if ("Y".equals(crnSchYn)) 
				{
					ydWbookId = ydCarldWrkbookId;
				} 
				
				/**********************************************************
				* 10.2 크레인스케줄기동여부 'N'이면,
				**********************************************************/
				else 
				{
					/***************************
					...
					/**********************************************************
					* 10.2.1 대차에 다음작업예약 조회
					**********************************************************/
					JDTORecordSet jsTcarNext = commDao.select(jrParam, getTcarSchLevWo, logId, methodNm, "대차우선순위조회");
					...
					/**********************************************************
					* 10.2.2 대차에 현재작업예약이 없고, 다음상차작업예약이 있다면, 다음 작업상차예약을 상차작업예약으로 update
					**********************************************************/
		            if ((ydTcarNextWbookId.length() > 0 ) && ("".equals(ydTcarCurrWbookId))) {

		            	ydWbookId = ydTcarNextWbookId;
		            	//crnSchYn = "Y";   // 스케쥴 기동 처리 . 2020 04 14 
		    			
		            	JDTORecord jrParam1 = JDTORecordFactory.getInstance().create();
		    			jrParam1.setResultCode(logId);	
		    			jrParam1.setResultMsg(methodNm);	
		    			jrParam1.setField("MODIFIER"       		, modifier    ); 	//수정자
		    			jrParam1.setField("YD_CAR_PROG_STAT"	, YfConstant.YD_CAR_PROG_STAT_2); //야드차량진행상태
		    			jrParam1.setField("YD_CARLD_WRK_BOOK_ID", ydWbookId); 		//상차 작업 예약
						jrParam1.setField("YD_TCAR_SCH_ID"   	, ydTcarSchId);   	//대차SCH
						commDao.update(jrParam1, updTcarProgStatCarldWrkbook, logId, methodNm, "대차스케줄상태 수정");
		            }
		            
		            
		            /**********************************************************
					* 10.2.3 명령선택 기동 여부 판단
					* 1) 스케줄에서, to위치 확장대차일 경우, 대차가 현재동에 존재하지 않는다면, 명령선택기동을 하지 않고, 
					*   공대차 호출만 한다.
					* 2) 공대차 도착시, 스케줄 W인것을 찾아 명령선택을 기동한다.    
					**********************************************************/
		            JDTORecordSet jsRunF1YFL007 = commDao.select(jrParam, getRunF1YFL007, logId, methodNm, "대차우선순위조회");
					bRunF1YFL007 = jsRunF1YFL007.getRecord(0).getFieldString("RUN_F1YFL007");
					crnId = jsRunF1YFL007.getRecord(0).getFieldString("CRN_ID");
					commUtils.printLog(logId, "명령선택 기동 여부: " +bRunF1YFL007 +", "+crnId, "SL");
				...
				/**********************************************************
				* 10.3 공대차도착이고 적치Bed 비활성화 이면 활성화
				*     -스케줄 기동해야하는 상황인데, 적치단이 비활성이면 안되니까 활성화 시킴
				**********************************************************/
				if (!"L".equals(ydStkBedActStatLd) && "Y".equals(crnSchYn)) {
					//현위치 적치Bed 활성화 처리
					jrParam.setField("YD_STK_COL_GP"         , ydStkColGp); //야드적치열구분(현위치)
					jrParam.setField("YD_STK_BED_ACTIVE_STAT", "L"       ); //야드적치Bed활성상태(활성)
					
					commDao.update(jrParam, updStatStkBedActByCol, logId, methodNm, "도착위치 Bed 활성화");
					
					//적치단 수정
					commDao.update(jrParam, updYdStkLyrActiveTC, logId, methodNm, "도착위치 단 활성화");					

				}
			}

			/**********************************************************
			* 11. 야드저장위치제원(YFF1L001) 전문 전송
			**********************************************************/
			jrParam.setField("YD_INFO_SYNC_CD", "4"       ); //야드정보동기화코드(Bed)
			jrParam.setField("YD_STK_COL_GP"   , ydStkColGp); //야드적치열구분(현재동)
			jrParam.setField("YD_STK_BED_NO"   , "01"       ); //야드적치Bed
			jrRtn = commUtils.addSndData(commDao.getMsgL2("YFF1L001", jrParam));

			
			/**********************************************************
			* 12. 크레인스케줄기동여부 'Y'이고 작업예약ID가 있으면 크레인스케줄 전송
			**********************************************************/
			commUtils.printLog(logId, " 크레인스케줄기동여부:" + crnSchYn + ", 크레인스케줄기동 wb:" + ydWbookId , "SL");
			if ("Y".equals(crnSchYn) && !"".equals(ydWbookId)) 
			{
				/**********************************************************
				* 12.1 공대차도착 : 상차도착(2) or 상차대기(0)
				**********************************************************/
				if ("1XTC03".equals(ydEqpId) && "6".equals(ydSchReqGp)) 
				{
					...
					JDTORecordSet jsTcsch = commDao.select(jrYdMsg, getYdTcarWbookid, logId, methodNm, "크레인스케줄 조회");
				...
				* 12.2 영대차도착 : 하차도착(B)
				**********************************************************/
				else if ("1XTC03".equals(ydEqpId) && "3".equals(ydSchReqGp)) 
				{
					...
					jrYdMsg.setField("YD_STK_COL_GP"  , ydStkColGp ); // 작업위치
					JDTORecordSet jsTcsch = commDao.select(jrYdMsg, getYdTcarWbookidLD, logId, methodNm, "크레인스케줄 조회"); 
			...
			/**********************************************************
			* 13. W상태에 스케줄이 있다면 명령선택 기동한다.
			**********************************************************/
			else if("Y".equals(bRunF1YFL007))
			{
				JDTORecord jrYdMsg = JDTORecordFactory.getInstance().create();
				jrYdMsg.setResultCode(logId);	//Log ID
				jrYdMsg.setResultMsg(methodNm);	//Log Method Name
				...
				jrYdMsg.setField("JMS_TC_CREATE_DDTT", commUtils.getDateTime14()); //JMSTC생성일시
				jrYdMsg.setField("YD_EQP_ID"         , crnId                  ); //야드설비ID
				jrYdMsg.setField("YD_WRK_PROG_STAT"  , "W"                      ); //야드작업진행상태
...
//					throw new Exception("영대차 출발/도착 정보 이상");
			...
			**********************************************************/
			if ("A".equals(ydCarProgStat)) 
			{
				...
				*     - 이 시점에서 재료가 적치단에서 사라짐.. 대차이송재료로만 남음. KBS
				**********************************************************/
				commDao.update(jrParam, updYdStkLyrClr2, logId, methodNm, "출발위치 단 비활성화");
				...
				**********************************************************/
				else 
				{
					...
					**********************************************************/
		            ...
		            /**********************************************************
					* 10.2.3 명령선택 기동 여부 판단
					*    공대차 도착시, 스케줄 W인것을 찾아 명령선택을 기동한다.    
					**********************************************************/
					JDTORecordSet jsRunF1YFL007 = commDao.select(jrParam, getRunF1YFL007, logId, methodNm, "대차우선순위조회");
					bRunF1YFL007 = jsRunF1YFL007.getRecord(0).getFieldString("RUN_F1YFL007");
					crnId = jsRunF1YFL007.getRecord(0).getFieldString("CRN_ID");
	...
	public JDTORecord rcvF1YFL015(JDTORecord rcvMsg) throws DAOException {
		...
		try {
			...
			**********************************************************/
			...
			//LINE OFF 취소 인경우 ( 작업예약 까지 삭제 하기 위해서....)
			if ("D".equals(headMsgGp) && ("1BDC01LM".equals(ydSchCd)||"1CDC01LM".equals(ydSchCd))){
				ydL2RequestStat ="X";
			}
				...
				**********************************************************/
	    		...
	    		if ("TC".equals(szBefEqpGp)||"PT".equals(szBefEqpGp)||"TR".equals(szBefEqpGp)) 
	    		{
	    			if (!szChgEqpGp.equals(szBefEqpGp)) 
	    			{
	    				...
	    				commDao.update(jrParam, updCommTcarSchWbDel, logId, methodNm, "TB_YD_TCARSCH");				
	    			}
	    		}
	    		...
	    		if ("TC".equals(szBefEqpGp)|| "PT".equals(szBefEqpGp)) {
	    			if (!szChgEqpGp.equals(szBefEqpGp)) {
	    				...
	    				if ("TC".equals(szCarGp)) {
	    	 				if ("U".equals(szULGp)) {  
	    	 					//상차인경우 작업예약 정보 삭제
	    	    				commDao.update(jrParam, updCommTcarSchWbDelLd, logId, methodNm, "TB_YD_TCARSCH");				

	    					}else if ("L".equals(szULGp)) {
	    						//하차인경우 작업예약 정보 삭제
	    	    				commDao.update(jrParam, updCommTcarSchWbDelUd, logId, methodNm, "TB_YD_TCARSCH");				
					...
					if(jsCarFtmvMtl.size() > 0)
					{
						...
						for(int Loop_i = 1; Loop_i <= jsCarFtmvMtl.size() ; Loop_i++)
						{
							...
							if(!"".equals(ydWbookIdNew))
							{
								...
								int ins_cnt = commDao.insert(jrWbook, insWrkBook2, logId, methodNm, "TB_YF_WRKBOOK 등록");
...
//				jrInParam.setField("YD_CARLD_WRK_BOOK_ID",	ydWbookId);     			 	//상차 작업예약ID
	...
	*/
	...
	/**
	 *      [A] 오퍼레이션명 : 분기컨베이어트레킹(rcvF1YFL020)
	 *
	 * 		@ejb.interface-method EJBDoclet을 생성하는 태그입니다.
	 *      @param JDTORecord rcvMsg
	 *      @return JDTORecord
	...
	*/
	public JDTORecord rcvF1YFL020(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "분기컨베이어트레킹[ACoilRcvL2SeEJB.rcvF1YFL020] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		...
		try {
			...
			String [] arryLoc = {
					 "1CDC0101","1CDC0102","1CDC0103","1CDC0104","1CDC0105","1CDC0106","1CDC0107","1CDC0108","1CDC0109","1CDC0110"
					,"1CDC0111","1CDC0112","1CDC0113","1CDC0114","1CDC0115","1CDC0116","1CDC0117","1CDC0118","1CDC0119","1CDC0120"
					,"1BDC0101","1BDC0102","1BDC0103","1BDC0104","1BDC0105"};
			...
			**********************************************************/
			...
			param.setField("YD_GP"		, "1");	
			param.setField("PROC_GP"	, "B");	
			param.setField("YD_EQP_ID"	, "");	
			param.setField("MODIFIER"	, modifier);
			commDao.delete(param, deleteEqpTracking, logId, methodNm, "분기컨베이어트레킹 삭제");
			
			for(int i=0; i<arryEqp.length; i++)
			{
				param.setField("YD_EQP_ID"	, arryEqp[i]);	
				param.setField("STL_NO"		, commUtils.trim(rcvMsg.getFieldString(arryEqp[i]+"_STL_NO")));
				param.setField("SORT_SEQ"	, String.valueOf(i));	
				param.setField("YD_EQP_NM"	, arryLoc[i].substring(1, 2)+"동분기컨베이어");
				param.setField("YD_STK_COL_GP"	, arryLoc[i].substring(0, 6));
				param.setField("YD_STK_BED_NO"	, arryLoc[i].substring(6, 8));
				param.setField("YD_BAY_GP"		, arryLoc[i].substring(1, 2));
				commDao.update(param, insEqpTracking, logId, methodNm, "분기컨베이어트레킹 등록");
			}
			
			
			
			// 신고도화 내용 추가
			String sAPP101_YN = yfComm.ACoilApplyYn("APP101","1","SPM1");
			if ("Y".equals(sAPP101_YN)) {
				JDTORecord jrYdMsg1	= JDTORecordFactory.getInstance().create();
				jrYdMsg1.setResultCode(logId);	//Log ID
				jrYdMsg1.setResultMsg(methodNm);	//Log Method Name
				jrYdMsg1.setField("YD_EVENT_GP"		, "1XSPM1");
	...
	*/
	public JDTORecord rcvF1YFL021(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "CTS트레킹[ACoilRcvL2SeEJB.rcvF1YFL021] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		...
		try {
			...
			String [] arryEqp = {"1ASL01","1ASL02","1ASL03","1ASR01","1ASR02"
					,"1ASR03","1BSL01","1BSL02","1BSL03","1BSR01","1BSR02"
					,"1BSR03","1CSL01","1CSL02","1DSL01","1DSL02","1DSL03"
					,"1DSL04","1ESL01","1ESL02","1ESL03","1ESL04","1ESR01"
					,"1ESR02","1ESR03","1ESR04","1FSL01","1FSL02","1FSR01"
					,"1FSR02","1GSL01","1GSL02","1GSR01","1GSR02","1HSL01"
					,"1HSL02","1HSR01","1HSR02"};
			//수신 항목 값
			String msgId    = commUtils.getMsgId(rcvMsg); //EAI, JMS, HTTP(출하관리 등) 수신 전문 I/F ID
			String modifier = commUtils.trim(rcvMsg.getFieldString("MODIFIER"     )); //수정자(Backup Only)
			...
			**********************************************************/
			...
			param.setField("YD_GP"		, "1");	
			param.setField("PROC_GP"	, "C");	
			param.setField("YD_EQP_ID"	, "");	
			param.setField("MODIFIER"	, modifier);
			commDao.delete(param, deleteEqpTracking, logId, methodNm, "CTS트레킹 삭제");
			
			for(int i=0; i<arryEqp.length; i++)
			{
				param.setField("YD_EQP_ID"	, arryEqp[i]);	
				param.setField("STL_NO"		, commUtils.trim(rcvMsg.getFieldString("STL_NO_"+arryEqp[i])));
				param.setField("SORT_SEQ"	, String.valueOf(i));	
				param.setField("YD_EQP_NM"	, arryEqp[i].substring(1, 2)+"동CTS");
				
				//param.setField("YD_STK_COL_GP"	, arryLoc[i].substring(0, 6));
				//param.setField("YD_STK_BED_NO"	, arryLoc[i].substring(6, 8));
				param.setField("YD_BAY_GP"		, arryEqp[i].substring(1, 2));
				commDao.update(param, insEqpTracking, logId, methodNm, "CTS트레킹 등록");
			}
	...
	*/
	public JDTORecord rcvF1YFL022(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "SPM트레킹[ACoilRcvL2SeEJB.rcvF1YFL022] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		...
		try {
			...
			String [] arryEqp = {"D1","D2","D3","D4"
					,"E1","E2","E3","E4","E5","E6","E7","E8","E9","E10","E11"
					,"F1","F2","F3","F4","F5"};
			
			
			String [] arryLoc = {"1DKE0101","1DKE0102","1DKE0103","1DKE0104"	
					,"1EKE0101","1EKD0101","1EKD0102","1EKD0103","1EKD0104","1EKD0105","1EKD0106","1EKD0107"	
					,"1EKD0108","1EKD0109","1EKD0110","1FKD0101","1FKD0102","1FKD0103"	
					,"1FKD0104","1FKD0105"};
			...
			**********************************************************/
			...
			param.setField("YD_GP"		, "1");	
			param.setField("PROC_GP"	, "K");	
			param.setField("YD_EQP_ID"	, "");	
			param.setField("MODIFIER"	, modifier);
			commDao.delete(param, deleteEqpTracking, logId, methodNm, "SPM트레킹 삭제");
			...
			for(int i=0; i<arryEqp.length; i++)
			{
				...
				param.setField("YD_STK_BED_NO"	, arryLoc[i].substring(6, 8));
				param.setField("YD_BAY_GP"		, arryLoc[i].substring(1, 2));
				commDao.update(param, insEqpTracking, logId, methodNm, "SPM트레킹 등록");
			}
	...
	*/
	public JDTORecord rcvF1YFL023(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "EQL트레킹[ACoilRcvL2SeEJB.rcvF1YFL023] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		...
		try {
			...
			String [] arryEqp = {"E1","E2","E3","E4","E5","E6","E7"
					,"F1","F2","F3","F4","F5","F6","F7"
					,"G1","G2","G3","G4","G5","G6","G7"};
			
			
			String [] arryLoc = {"1EQE0101","1EQE0102","1EQE0103","1EQE0104","1EQE0105","1EQE0106","1EQE0107"
					,"1FQE0101","1FQE0102","1FQE0103","1FQE0104","1FQE0105","1FQE0106","1FQE0107"
					,"1GQD0101","1GQD0102","1GQD0103","1GQD0104","1GQD0105","1GQD0106","1GQD0107"};
			...
			**********************************************************/
			...
			param.setField("YD_GP"		, "1");	
			param.setField("PROC_GP"	, "Q");	
			param.setField("YD_EQP_ID"	, "");	
			param.setField("MODIFIER"	, modifier);
			commDao.delete(param, deleteEqpTracking, logId, methodNm, "EQL트레킹 삭제");
			...
			for(int i=0; i<arryEqp.length; i++)
			{
				...
				param.setField("YD_STK_BED_NO"	, arryLoc[i].substring(6, 8));
				param.setField("YD_BAY_GP"		, arryLoc[i].substring(1, 2));
				commDao.update(param, insEqpTracking, logId, methodNm, "EQL트레킹 등록");
			}
	...
	*/
	public JDTORecord rcvF1YFL024(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "HFL트레킹[ACoilRcvL2SeEJB.rcvF1YFL024] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		...
		try {
			...
			String [] arryEqp = {"B1","B2","B3","B4","B5","B6"
					,"C1","C2","C3","C4","C5","C6"};
			
			
			String [] arryLoc = {"1BFE0101","1BFE0102","1BFE0103","1BFE0104","1BFE0105","1BFE0106"
					,"1CFD0101","1CFD0102","1CFD0103","1CFD0104","1CFD0105","1CFD0106"};
			...
			**********************************************************/
			...
			param.setField("YD_GP"		, "1");	
			param.setField("PROC_GP"	, "F");	
			param.setField("YD_EQP_ID"	, "");	
			param.setField("MODIFIER"	, modifier);
			commDao.delete(param, deleteEqpTracking, logId, methodNm, "HFL트레킹 삭제");
			...
			for(int i=0; i<arryEqp.length; i++)
			{
				...
				param.setField("YD_STK_BED_NO"	, arryLoc[i].substring(6, 8));
				param.setField("YD_BAY_GP"		, arryLoc[i].substring(1, 2));
				commDao.update(param, insEqpTracking, logId, methodNm, "HFL트레킹 등록");
			}
	...
	*/
	public JDTORecord rcvF1YFL027(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "분동코일작업지시요구[ACoilRcvL2SeEJB.rcvF1YFL027] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		...
		try {
			...
			//수신 항목 값
			String sYD_UP_WO_LOC           = commUtils.trim(rcvMsg.getFieldString("YD_UP_WO_LOC"          )); //야드권상지시위치
			String sYD_DN_WO_LOC           = commUtils.trim(rcvMsg.getFieldString("YD_DN_WO_LOC"          )); //야드권하지시위치
			/*
			if (sYD_UP_WO_LOC.length() != 8) 
			{
				throw new Exception("권상논리좌표 이상");
			}
			if (sYD_DN_WO_LOC.length() != 8) 
			{
				throw new Exception("권하논리좌표 이상");
			}
			*/
			//SPM/EQL추출
			if("".equals(sYD_DN_WO_LOC))
			{
				jrRtn = procF1YFL027_OUT(rcvMsg);
			}
			//SPM/EQL보급, DC보급추출
			else
			{
				jrRtn = procF1YFL027_IN(rcvMsg);
			}
	...
	/**
	 *      [A] 오퍼레이션명 : 분동코일작업지시요구(procF1YFL027_DC)
	 *                   분동코일보급요구 
	 * 		@ejb.interface-method EJBDoclet을 생성하는 태그입니다.
	 *      @param JDTORecord rcvMsg
	 *      @return JDTORecord
	 *      @throws DAOException
	*/
	public JDTORecord procF1YFL027_IN(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "분동코일작업지시요구:분동코일보급요구[ACoilRcvL2SeEJB.procF1YFL027_IN] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		...
		try {
			...
			String sCOIL_NO                = commUtils.trim(rcvMsg.getFieldString("COIL_NO"               )); //코일번호               
			String sYD_UP_WO_LOC           = commUtils.trim(rcvMsg.getFieldString("YD_UP_WO_LOC"          )); //야드권상지시위치
			String sYD_UP_WO_LYR		   = commUtils.trim(rcvMsg.getFieldString("YD_UP_WO_LYR"        )); //야드권상지시단
			String sYD_UP_WO_LOC_XAXIS     = commUtils.trim(rcvMsg.getFieldString("YD_UP_WO_LOC_XAXIS"    )); //야드권상지시X축
			String sYD_UP_WO_LOC_YAXIS     = commUtils.trim(rcvMsg.getFieldString("YD_UP_WO_LOC_YAXIS"    )); //야드권상지시Y축
			String sYD_UP_WO_LOC_ZAXIS     = commUtils.trim(rcvMsg.getFieldString("YD_UP_WO_LOC_ZAXIS"    )); //야드권상지시Z축
			String sYD_UP_WO_LOC_ZAXIS_SYM = commUtils.trim(rcvMsg.getFieldString("YD_UP_WO_LOC_ZAXIS_SYM")); //야드권상지시Z축부호
			String sYD_DN_WO_LOC           = commUtils.trim(rcvMsg.getFieldString("YD_DN_WO_LOC"          )); //야드권하지시위치
			String sYD_DN_WO_LYR           = commUtils.trim(rcvMsg.getFieldString("YD_DN_WO_LYR"        )); //야드권하지시단
			String sYD_DN_WO_LOC_XAXIS     = commUtils.trim(rcvMsg.getFieldString("YD_DN_WO_LOC_XAXIS"    )); //야드권하지시X축
			String sYD_DN_WO_LOC_YAXIS     = commUtils.trim(rcvMsg.getFieldString("YD_DN_WO_LOC_YAXIS"    )); //야드권하지시Y축
			String sYD_DN_WO_LOC_ZAXIS     = commUtils.trim(rcvMsg.getFieldString("YD_DN_WO_LOC_ZAXIS"    )); //야드권하지시Z축
			String sYD_DN_WO_LOC_ZAXIS_SYM = commUtils.trim(rcvMsg.getFieldString("YD_DN_WO_LOC_ZAXIS_SYM")); //야드권하지시Z축부호
			String sYD_STL_WT              = commUtils.trim(rcvMsg.getFieldString("YD_STL_WT"             )); //야드재료중량
			String sYD_STL_T               = commUtils.trim(rcvMsg.getFieldString("YD_STL_T"              )); //야드재료두께
			String sYD_STL_W               = commUtils.trim(rcvMsg.getFieldString("YD_STL_W"              )); //야드재료폭
			String sCOIL_OUTDIA            = commUtils.trim(rcvMsg.getFieldString("COIL_OUTDIA"           )); //Coil외경
			String sCOIL_INDIA             = commUtils.trim(rcvMsg.getFieldString("COIL_INDIA"            )); //Coil내경
			
			String sYD_SCH_CD     = ""; 
			
			//DC컨베이어는 보급/추출 모두 여기서 처리
			if("1CDC".equals(sYD_UP_WO_LOC.substring(0, 4))
					||"1CDC".equals(sYD_DN_WO_LOC.substring(0, 4)))
			{
				sYD_SCH_CD = "1CDC41MM";
			}
			//SPM보급
			else if("1EKD0103".equals(sYD_DN_WO_LOC))
			{
				sYD_SCH_CD = "1EKE41UM";
			}
			//EQL보급
			else if("1FQD0103".equals(sYD_DN_WO_LOC))
			{
				sYD_SCH_CD = "1FQE41UM";
			}
			...
			jrParam.setField("DTL_ITEM1"       , sYD_STL_WT  );
			jrParam.setField("DTL_ITEM2"       , sYD_STL_T   );
			jrParam.setField("DTL_ITEM3"       , sYD_STL_W   );
			jrParam.setField("DTL_ITEM4"       , sCOIL_OUTDIA);
			jrParam.setField("DTL_ITEM5"       , sCOIL_INDIA );
			...
			**********************************************************/
			...
			String sYD_GP      = sYD_UP_WO_LOC.substring(0, 1);
			String sYD_BAY_GP  = sYD_UP_WO_LOC.substring(1, 2);
			sYD_UP_WO_LYR      = sYD_UP_WO_LYR.substring(1, 3);
			sYD_DN_WO_LYR      = sYD_DN_WO_LYR.substring(1, 3);
			
			/*********************************************************
			 * 1.크레인 선택
			 **********************************************************/
			JDTORecordSet jrEqpInfo = commDao.select(jrParam, getWrkCrnForBC);
			if (jrEqpInfo.size() <= 0) {
			...
			**********************************************************/
			JDTORecordSet jsBDSchlist = commDao.select(jrParam, getBDSchList, logId, methodNm, "분동스케줄 조회");
			if (jsBDSchlist.size() > 0) {
				throw new Exception("해당 분동코일["+sCOIL_NO+"] 스케줄 존재함");
			}
			
			commDao.update(jrParam, updBDCoilInfo, logId, methodNm, "분동코일 정보 수정");
			...
			**********************************************************/
			...
			recInCrn.setField("YD_GP",            	sYD_UP_WO_LOC.substring(0,1));
			recInCrn.setField("YD_BAY_GP",        	sYD_UP_WO_LOC.substring(1,2));
			...
			recInCrn.setField("YD_TO_LOC_GUIDE"   ,	sYD_DN_WO_LOC);
			recInCrn.setField("YD_TO_LOC_DCSN_MTD", "S"); // S W
			recInCrn.setField("YD_EQP_WRK_SH"     , "1");
			recInCrn.setField("YD_EQP_WRK_MAX_L"  , "0");

			recInCrn.setField("YD_UP_WO_LOC"          , sYD_UP_WO_LOC);
			recInCrn.setField("YD_UP_WO_LYR"          , sYD_UP_WO_LYR);
			recInCrn.setField("YD_UP_WO_LOC_XAXIS"    , sYD_UP_WO_LOC_XAXIS); 
			recInCrn.setField("YD_UP_WO_XAXIS_GAP_MAX", "1000");              
			recInCrn.setField("YD_UP_WO_XAXIS_GAP_MIN", "1000");              
			recInCrn.setField("YD_UP_WO_LOC_YAXIS"    , sYD_UP_WO_LOC_YAXIS); 
			recInCrn.setField("YD_UP_WO_YAXIS_GAP_MAX", "1000");              
			recInCrn.setField("YD_UP_WO_YAXIS_GAP_MIN", "1000");              
			recInCrn.setField("YD_UP_WO_LOC_ZAXIS"    , sYD_UP_WO_LOC_ZAXIS); 
			recInCrn.setField("YD_UP_WO_ZAXIS_GAP_MAX", "100");               
			recInCrn.setField("YD_UP_WO_ZAXIS_GAP_MIN", "100");   
			recInCrn.setField("YD_DN_WO_LOC"          , sYD_DN_WO_LOC);
			recInCrn.setField("YD_DN_WO_LYR"          , sYD_DN_WO_LYR);
			recInCrn.setField("YD_DN_WO_LOC_XAXIS"    , sYD_DN_WO_LOC_XAXIS); 
			recInCrn.setField("YD_DN_WO_XAXIS_GAP_MAX", "1000");
			recInCrn.setField("YD_DN_WO_XAXIS_GAP_MIN", "1000");
			recInCrn.setField("YD_DN_WO_LOC_YAXIS"    , sYD_DN_WO_LOC_YAXIS);
			recInCrn.setField("YD_DN_WO_YAXIS_GAP_MAX", "1000");
			recInCrn.setField("YD_DN_WO_YAXIS_GAP_MIN", "1000");
			recInCrn.setField("YD_DN_WO_LOC_ZAXIS"    , sYD_DN_WO_LOC_ZAXIS);
			recInCrn.setField("YD_DN_WO_ZAXIS_GAP_MAX", "100");
			recInCrn.setField("YD_DN_WO_ZAXIS_GAP_MIN", "100");
			recInCrn.setField("UP_ROTATION_ANGLE"     , "0");
			recInCrn.setField("DOWN_ROTATION_ANGLE"   , "0");
			
			String szLogMsg = "";
			int intRtnVal = commDao.insert(recInCrn, insYmCrnsch, logId, methodNm, "TB_YF_CRNSCH 생성");
			if (intRtnVal < 1) {
			...
			**********************************************************/			
			...
			recInCrnMtl.setField("STL_NO"      , sCOIL_NO);
			recInCrnMtl.setField("YD_STK_LYR_NO", sYD_UP_WO_LYR);

			intRtnVal = commDao.insert(recInCrnMtl, insYmCrnwrkmtl, logId, methodNm, "TB_YM_CRNWRKMTL 생성");
			if (intRtnVal <= 0) {
			...
			* 5.크레인작업지시 호출
			...
			jrYdMsg.setField("JMS_TC_CD"         , "YFF1L004"               ); //JMSTC코드
			jrYdMsg.setField("JMS_TC_CREATE_DDTT", commUtils.getDateTime14()); //JMSTC생성일시
			jrYdMsg.setField("YD_CRN_SCH_ID"     , sYD_CRN_SCH_ID           ); //
			jrYdMsg.setField("MSG_GP"            , "I"           ); //전문구분
			jrYdMsg.setField("COIL_OUTDIA"       , sCOIL_OUTDIA); //외경
			jrYdMsg.setField("COIL_INDIA"        , sCOIL_INDIA );  //내경
			jrYdMsg.setField("YD_STL_WT"         , sYD_STL_WT  );
			jrYdMsg.setField("YD_STL_T"          , sYD_STL_T   );
			jrYdMsg.setField("YD_STL_W"          , sYD_STL_W   );
			jrYdMsg.setField("STL_NO"            , sCOIL_NO    );
			
			jrRtn = commUtils.addSndData(jrRtn,commDao.getMsgL2("YFF1L004WC", jrYdMsg));
	...
	/**
	 *      [A] 오퍼레이션명 : 분동코일작업지시요구(procF1YFL027)
	 *                   분동코일추출
	 * 		@ejb.interface-method EJBDoclet을 생성하는 태그입니다.
	 *      @param JDTORecord rcvMsg
	 *      @return JDTORecord
	 *      @throws DAOException
	*/
	public JDTORecord procF1YFL027_OUT(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "분동코일작업지시요구:분동코일추출[ACoilRcvL2SeEJB.procF1YFL027_OUT] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		
		JDTORecord jrRtn = null;	//전문 Return
		try {
			commUtils.printLog(logId, methodNm, "S+");
			//적치단 상태 U가 아니면, 권상불가 함. 권상권하 실적이 필요한지도 확인 필요 KBS 
			//수신 항목 값
			String msgId           = commUtils.getMsgId(rcvMsg); //EAI, JMS, HTTP(출하관리 등) 수신 전문 I/F ID
			String sMODIFIER       = commUtils.trim(rcvMsg.getFieldString("MODIFIER" )); //수정자(Backup Only)
			String sCOIL_NO                = commUtils.trim(rcvMsg.getFieldString("COIL_NO"               )); //코일번호               
			...
			**********************************************************/
			...
			String ydBayGp		= "";
			//SPM추출
			if("1FKD".equals(sYD_UP_WO_LOC.substring(0, 4)))
			{
				sYD_SCH_CD = "1FKD41LM";
				ydBayGp	= "E";
			}
			//EQL추출
			else if("1GQD".equals(sYD_UP_WO_LOC.substring(0, 4)))
			{
				sYD_SCH_CD = "1GQD41LM";
				ydBayGp	= "F";
			}
			...
			**********************************************************/
			...
			jrParam.setField("DTL_ITEM4"      , sCOIL_OUTDIA);
			jrParam.setField("DTL_ITEM5"      , sCOIL_INDIA );
			jrParam.setField("YD_STK_COL_GP"  	, sYD_UP_WO_LOC.substring(0, 6)); 
			jrParam.setField("YD_STK_BED_NO"  	, sYD_UP_WO_LOC.substring(6, 8)); 
			jrParam.setField("YD_STK_LYR_NO" 	, "01"); 
			jrParam.setField("YD_STK_LYR_STAT" 	, "C"); 
			
			commDao.insert(jrParam, updateStackLayerStat, logId, methodNm, "TB_YF_STKLYR 생성");
			...
			**********************************************************/
			...
			wkParam.setField("YD_STK_COL_GP"  	, sYD_UP_WO_LOC.substring(0, 6)); 
			wkParam.setField("YD_STK_BED_NO"  	, sYD_UP_WO_LOC.substring(6, 8)); 
			wkParam.setField("YD_STK_LYR_NO" 	, "01"); 
			wkParam.setField("YD_SCH_CD"		, sYD_SCH_CD); 
			//wkParam.setField("YD_EQP_ID"		, "L");
			wkParam.setField("STL_NO"			, sCOIL_NO);
			//wkParam.setField("YD_TO_LOC_GUIDE"	, ydToLocGuide);
			wkParam.setField("YD_AIM_BAY_GP"	, ydBayGp);
			wkParam.setField("MODIFIER"     	, sMODIFIER);
			String ydWbookId = yfComm.procWkBookInsert(wkParam);
			...
			**********************************************************/
			...
			schParam.setField("YD_SCH_CD"    , sYD_SCH_CD  ); //야드스케쥴코드
			schParam.setField("YD_SCH_ST_GP" , "O"); //야드스케쥴기동구분
			schParam.setField("YD_SCH_REQ_GP", "L"      ); //야드스케쥴요청구분(인출)
			schParam.setField("MODIFIER"     , sMODIFIER ); //수정자

			jrRtn = commUtils.addSndData(yfComm.getCrnSchMsg(schParam));
...
//				}
			...
			**********************************************************/
			...
			SELECT CS.*
			 ...
			 WHERE 1 = 1
			   ...
			   AND SUBSTR(CS.YD_DN_WO_LOC, 1, 6) IN ('3ESC02','3ESC03')
			   AND CM.STL_NO LIKE 'S%'
			   AND '1' = (SELECT HMI_STAT FROM TB_YM_EQUIP WHERE EQUIP_GP = '3EGT01')
			 UNION ALL
			SELECT CS.*
			     , CM.STL_NO
			  FROM TB_YF_CRNSCH    CS
			 ...
			 WHERE 1 = 1
			   ...
			   AND CM.DEL_YN = 'N'
			   AND CS.YD_WRK_PROG_STAT IN ('W')
			   AND SUBSTR(CS.YD_DN_WO_LOC, 1, 6) IN ('3ESC01')
			   AND SUBSTR(CS.YD_DN_WO_LOC, 7, 2) BETWEEN '01' AND '08' --임시적치구역
			   AND CM.STL_NO LIKE 'S%'
			   AND '0' = (SELECT HMI_STAT FROM TB_YM_EQUIP WHERE EQUIP_GP = '3EGT01')
			 */
			JDTORecordSet jsScrSchList = commDao.select(jrParam, "com.inisteel.cim.ym.bcoil.dao.BcoilDAO.getScrpSchList", logId, methodNm, "스크랩스케줄 조회");
			String sYD_SCH_CD = "";
			if (jsScrSchList.size() > 0) {
				for (int ii = 0; ii < jsScrSchList.size(); ++ii) {
					sYD_SCH_CD = jsScrSchList.getRecord(ii).getFieldString("YD_SCH_CD");
					
					jrParam.setField("STL_NO"     , jsScrSchList.getRecord(ii).getFieldString("STL_NO"));
					jrParam.setField("YD_SCH_CD"    , jsScrSchList.getRecord(ii).getFieldString("YD_SCH_CD"));
					jrParam.setField("YD_CRN_SCH_ID", jsScrSchList.getRecord(ii).getFieldString("YD_CRN_SCH_ID"));
					jrParam.setField("YD_WBOOK_ID"  , jsScrSchList.getRecord(ii).getFieldString("YD_WBOOK_ID") );
					
					if ("3EYD01MM".equals(sYD_SCH_CD)) { //동내이적
						
						/**********************************************************
						* 크레인스케줄 취소
						**********************************************************/
						jrParam.setField("WRK_CNCL_YN", "Y"); //작업취소 여부
						EJBConnector ejbConnSCnCl = new EJBConnector("default", "ACoilJspBakSeEJB", this);
						JDTORecord jrRstSCncl = (JDTORecord)ejbConnSCnCl.trx("trtCrnSchCncl", new Class[] { JDTORecord.class }, new Object[] { jrParam });
						
						jrRtn = commUtils.addSndData(jrRtn, jrRstSCncl);	

						/**********************************************************
						* 작업예약 취소
						**********************************************************/
						EJBConnector ejbConnWCnCl = new EJBConnector("default", "ACoilJspBakSeEJB", this);
						JDTORecord jrRstWCncl = (JDTORecord)ejbConnWCnCl.trx("trtWrkBookCncl", new Class[] { JDTORecord.class }, new Object[] { jrParam });
						
						jrRtn = commUtils.addSndData(jrRtn, jrRstWCncl);
						...
						* 권하위치 변경
						**********************************************************/
						JDTORecordSet jsLOCInfo = commDao.select(jrParam, "com.inisteel.cim.ym.bcoil.dao.BcoilDAO.getYdPrimaryWorkSearchScrapNewRe", logId, methodNm, "권하위치 재조회");
						
						if (jsLOCInfo.size() > 0) {
							jrParam.setField("YD_DN_WO_LOC"   , jsLOCInfo.getRecord(0).getFieldString("TAG_YD_STK_COL_GP")+jsLOCInfo.getRecord(0).getFieldString("TAG_YD_STK_BED_NO"));
							jrParam.setField("YD_STK_LYR_NO" , jsLOCInfo.getRecord(0).getFieldString("TAG_YD_STK_LYR_NO"));
							
							/*
							UPDATE TB_YF_STKLYR
							   SET STL_NO         = ''
							     , STACK_LYR_STAT = 'E'
							 WHERE YD_STK_COL_GP LIKE '3ESC0_'
							   AND STL_NO = :V_STL_NO
							 */
							commDao.update(jrParam, "com.inisteel.cim.ym.bcoil.dao.BcoilDAO.updLyrScrpDnLocClear", logId, methodNm, "기존 권하위치 삭제");
							
							/*
							UPDATE TB_YF_STKLYR
							   SET STL_NO         = :V_STL_NO
							     , STACK_LYR_STAT = 'D'
							 WHERE YD_STK_COL_GP||YD_STK_BED_NO = :V_YD_DN_WO_LOC
							   AND YD_STK_LYR_NO             = :V_YD_STK_LYR_NO
							 */
							commDao.update(jrParam, "com.inisteel.cim.ym.bcoil.dao.BcoilDAO.updLyrScrpDnLocSet", logId, methodNm, "새 권하위치 수정");
							
							/*
							UPDATE TB_YF_CRNSCH
							   SET YD_DN_WO_LOC   = :V_YD_DN_WO_LOC
							     , YD_DN_WO_LYR = :V_YD_STK_LYR_NO
							 WHERE YD_CRN_SCH_ID = :V_YD_CRN_SCH_ID
							 */
							commDao.update(jrParam, "com.inisteel.cim.ym.bcoil.dao.BcoilDAO.updSchScrpDnLoc", logId, methodNm, "크레인스케줄 권하위치 수정");
			...
			/***********************************************************
			 * 4. 스크랩차량 나갔을 때 1열에서 2~3열 스크랩 이적지시 
			 ***********************************************************/
			// 2~3열 비우기 지시때 1열 임시적치장에 놓인 스크랩을 2~3열로 자동이적 시킨다.
			
			if ("0".equals(sCAR_YN)) {
				
				//스크랩 자동이적 적용여부 변수 - 디폴트 값 : 적용
				String sAPP033_YN = "Y"; 
				
				//스크랩자동이적 적용여부 확인 
				jrParam.setField("REPR_CD_GP"	, "APP033");
				jrParam.setField("CD_GP"		, "1");
				
				if("3EGT01".equals(sYD_EQP_ID)) {
					//E동 SPM2 SCARP 자동이적 적용여부
					jrParam.setField("ITEM"			, "SPM2");
					
				} else if("3AGT01".equals(sYD_EQP_ID)) {
					//A동 SPM1 SCARP 자동이적 적용여부
					jrParam.setField("ITEM"			, "SPM1");
					
				} else {
					
					jrParam.setField("ITEM"			, "XXX");
				}
				JDTORecordSet jsChk = commDao.select(jrParam, "com.inisteel.cim.ym.bcommon.dao.YmCommDAO.getYmRule", logId, methodNm, "야드 기준 조회"); 
				
				if(jsChk.size() > 0) {
					sAPP033_YN = jsChk.getRecord(0).getFieldString("DTL_ITM1");
					
					commUtils.printLog(logId, "=======:::: sAPP033_YN : " +  sAPP033_YN , "SL");
				}				
				
				//스크랩 자동이적 적용시만 자동이적 지시 생성
				if("Y".equals(sAPP033_YN)) { 
					...
					SELECT SL.STL_NO
					 ...
					 WHERE 1 = 1
					   ...
					   AND SL.YD_STK_COL_GP = '3ESC01' --1열에서 2, 3열로만 이적가능
					   AND SL.STACK_LYR_STAT = 'C'
					   AND SL.YD_STK_BED_NO IN (SELECT ITEM AS YD_STK_BED_NO
					                             FROM USRYMA.TB_YM_RULE
					                            WHERE REPR_CD_GP = 'SCRAP'
					                              AND DTL_ITM1   = 'A' --임시적치구역
					                              AND CD_GP      = '3ESC01'
					                          )
					   AND ST.STL_NO NOT IN (SELECT WM.STL_NO
					   ...
					   AND 1 = (CASE WHEN SL.YD_STK_LYR_NO = '01' AND (SELECT STACK_LYR_STAT
					                                                      FROM TB_YF_STKLYR TEMP
					                                                     WHERE TEMP.YD_STK_COL_GP = SL.YD_STK_COL_GP
					                                                       AND TEMP.YD_STK_BED_NO = SL.YD_STK_BED_NO
					                                                       AND YD_STK_LYR_NO    = '02') IN ('D', 'U') THEN 0
					                 WHEN SL.YD_STK_LYR_NO = '01' AND (SELECT STACK_LYR_STAT
					                                                      FROM TB_YF_STKLYR TEMP
					                                                     WHERE TEMP.YD_STK_COL_GP = SL.YD_STK_COL_GP
					                                                       AND TEMP.YD_STK_BED_NO = LPAD(TO_NUMBER(SL.YD_STK_BED_NO)-1, 2, '0')
					                                                       AND YD_STK_LYR_NO    = '02') IN ('D', 'U') THEN 0
					                 ELSE 1 END
					           )                              
					 ORDER BY SL.YD_STK_LYR_NO DESC
					        , SL.YD_STK_BED_NO 
					 */
					JDTORecordSet jsScrList = commDao.select(jrParam, "com.inisteel.cim.ym.bcoil.dao.BCoilDAO.getMvStkScrpList", logId, methodNm, "스크랩자동이적 대상재 조회");
					
					if (jsScrList.size() > 0) {
						jrParam.setField("YD_SCH_CD"     , "3EYD01MM"); //동내이적
						jrParam.setField("WRK_FLAG"      , "SCRAP"   ); //
						jrParam.setField("CAR_YN" 	     , "0"       );
						
						/**************************
						 * 작업예약 생성 / 스케줄 생성
						 **************************/
						EJBConnector ejbConn = new EJBConnector("default", "ACoilJspBakSeEJB", this);
						JDTORecord jrWrkBook = (JDTORecord)ejbConn.trx("insMvstkWrkBookScrap", new Class[] { JDTORecord.class, JDTORecordSet.class }, new Object[] { jrParam, jsScrList});
						jrRtn = commUtils.addSndData(jrRtn, jrWrkBook);
						...
						String sALT_WPROG_STAT   = jrEqpInfo.getRecord(0).getFieldString("ALT_WPROG_STAT"); //
						String sALT_WORK_MODE    = jrEqpInfo.getRecord(0).getFieldString("ALT_WORK_MODE" ); //
						String sWRK_YD_EQP_ID = sYD_WRK_CRN;
						String sEQP_STAT  = sWRK_WPROG_STAT;
						
						if ("B".equals(sWRK_WPROG_STAT) || "2".equals(sWRK_WORK_MODE)) {
							if (!"".equals(sYD_ALT_CRN) && !"B".equals(sALT_WPROG_STAT) && !"2".equals(sALT_WORK_MODE)) {
								sWRK_YD_EQP_ID = sYD_ALT_CRN;
							}
						}
						JDTORecord jrYdMsg = commUtils.getParam(logId, methodNm, "A7YML007");
						jrYdMsg.setField("JMS_TC_CD"       , YmConstant.A7YML007);	//크레인작업지시요구
						jrYdMsg.setField("YD_EQP_ID"       , sWRK_YD_EQP_ID    );	//야드설비ID

						EJBConnector sndConn = new EJBConnector("default", "BCoilL2RcvBakSeEJB", this);
						JDTORecord jrA7YML007 = (JDTORecord)sndConn.trx("rcvA7YML007", new Class[] { JDTORecord.class }, new Object[] { jrYdMsg });
						jrRtn = commUtils.addSndData(jrRtn, jrA7YML007);
					}
					
				}
				
			} // end "Y".equals(sAPPLY033)
    	...
    	try{
			...
			**********************************************************/
			...
			if (resYn) {
				...
				resMsg.setField("A", "1ACRA1"); //A동 대표크레인
			...
			if (resYn) {
				try {
					...
					//크레인작업실적응답 전문 전송
					EJBConnector resConn = new EJBConnector("default", "YfCommBakSeEJB", this);   //yfComm.ACoilApplyYn
	...
	*/
	...
	/**
	 *      [A] 오퍼레이션명 : 코일 분기Conveyor COIL 추출 요구(F1YFL041)
	 *     					-CF1PB14
	 * 		@ejb.interface-method EJBDoclet을 생성하는 태그입니다.
	 *      @param JDTORecord rcvMsg
	 *      @return JDTORecord
	 *      @throws DAOException
	*/
	public JDTORecord rcvF1YFL041_old(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "코일 분기Conveyor COIL 추출 요구[ACoilRcvL2SeEJB.F1YFL041] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		...
		try{
			...
			//수신 항목 값
			String msgId        = commUtils.getMsgId(rcvMsg); //EAI, JMS, HTTP(출하관리 등) 수신 전문 I/F ID
			String sCoilNo      = commUtils.trim(rcvMsg.getFieldString("STL_NO"     ));
			String location		= commUtils.trim(rcvMsg.getFieldString("LOCATION" 	));
			String bizGp	  	= commUtils.trim(rcvMsg.getFieldString("BIZ_GP" 	)); //T:Take out H:Heat Coil(난방코일) I:TAKE IN 
			...
			**********************************************************/
			...
			jrParam.setResultCode(logId);	//Log ID
			jrParam.setResultMsg(methodNm);	//Log Method Name
			jrParam.setField("STL_NO"	, sCoilNo        ); //야드설비ID
			jrParam.setField("MODIFIER" , modifier       ); //수정자
			JDTORecordSet jsChk = commDao.select(jrParam, getYmwBookYN, logId, methodNm, "작업예약 등록여부");

			if (jsChk != null && jsChk.size() > 0) 
			{
				if ("Y".equals(jsChk.getRecord(0).getFieldString("WB_STL_YN"))) 
				{
					throw new Exception("이미 작업예약에 등록된 재료:" + sCoilNo);				
				}
			}				

			
			String ydStackColGp = ""; //권상위치
			String ydStackBedGp = "00"; //권상위치
			String ydStackDanGp = "01"; //권상위치
			if(!"I".equals(bizGp)) //TAKE IN이 아니면
			{
				/*************************************** 
				 * 3. 적치가능 베드 조회
				 **************************************/ 
				jrParam.setField("STL_NO"     		, sCoilNo); 
				JDTORecordSet jsBedNo = commDao.select(jrParam, getDCBedNo, logId, methodNm, "요구정보 조회");
				if (jsBedNo.size() < 1) 
				{
					throw new Exception("트레킹 정보 이상: ["+sCoilNo+"]");
				}
				
				
				ydStackColGp = jsBedNo.getRecord(0).getFieldString("YD_STK_COL_GP");	
				
				if ("".equals(ydStackColGp)) 
				{
					throw new Exception("트레킹 정보 이상: ["+sCoilNo+"]");
				}
				else if(!"DC".equals(ydStackColGp.substring(2, 4)))
				{
					throw new Exception("트레킹 정보 이상: ["+ ydStackColGp+"]");
				}
				 ...
				 * 4. MAP에 존재하는 저장품 삭제	 
				 *************************/
				commDao.update(jrParam, updStackLayerByStockId, logId, methodNm, "MAP에 존재하는 저장품 삭제");
				...
				jrInRecord.setField("YD_STK_COL_GP"			, ydStackColGp); 
				jrInRecord.setField("YD_STK_BED_NO"			, "00");	 
				jrInRecord.setField("YD_STK_LYR_NO"			, "01");	 
				jrInRecord.setField("STL_NO"				, sCoilNo);	 
				jrInRecord.setField("YD_STK_LYR_STAT"		, "C");  // 
				jrInRecord.setField("MODIFIER" , modifier       ); //수정자
				commDao.update(jrInRecord, updStackLayerStat, logId, methodNm, "LAYOUT 갱신");	
			}
			else if("I".equals(bizGp)) //TAKE IN이면
			{
				jrParam.setField("STL_NO"     		, sCoilNo); 
				JDTORecordSet jsBedNo = commDao.select(jrParam, getStackLayerInfoWithStockId_03, logId, methodNm, "요구정보 조회");
				ydStackColGp = jsBedNo.getRecord(0).getFieldString("YD_STK_COL_GP");	
				ydStackBedGp = jsBedNo.getRecord(0).getFieldString("YD_STK_BED_NO");
				ydStackDanGp = jsBedNo.getRecord(0).getFieldString("YD_STK_LYR_NO");
				if(jsBedNo.size() < 1)
				{
					throw new Exception("야드 맵에 존재하지 않는 재료 입니다 : "+sCoilNo);
				}
				if(!"B".equals(ydStackColGp.substring(1, 2)) && 
						!"C".equals(ydStackColGp.substring(1, 2)))
				{
					throw new Exception("대상재료의 현재동을 확인하세요 : "+ydStackColGp.substring(1, 2));
				}
			}
			...
			/*************************************** 
			 * 6. 스케줄코드로 스케줄기준Table조회
			 **************************************/
			String suffixCd = "";
			String ydAimBayGp = "";
			String item = "";
			if("H".equals(bizGp)) //난방코일 LINE OFF
			{
				suffixCd = "DC12LM";
				item = "H";
			}
			else if("T".equals(bizGp))//TAKE OUT
			{
				suffixCd = "DC02LM";
				item = "T";
			}
			else if("I".equals(bizGp)) //TAKE IN
			{
				suffixCd = "DC02UM";
			}
			else //LINE OFF
			...
			ydAimBayGp = yfComm.ACoilApplyYn("YF0006","1",item);
			
			
			
			
			String ydSchCd    = "1" + ydStackColGp.substring(1,2) + suffixCd;  
			String ydSchPrior = "";
			
			JDTORecord recInTemp = JDTORecordFactory.getInstance().create();
			recInTemp.setField("YD_SCH_CD", ydSchCd);
			JDTORecordSet jsResult = commDao.select(recInTemp, getYdSchrule, logId, methodNm, "스케줄 기준 조회"); 
			...
			{
				throw new Exception("박판열연 코일 스케쥴 코드 이상 : [" + ydSchCd + "]");
			}
			...
			**********************************************************/
			...
			recInTemp.setField("YD_AIM_BAY_GP"     , ydAimBayGp           ); //난방코일용 목적동
			
			int ins_cnt = commDao.insert(recInTemp, insWrkBook, logId, methodNm, "TB_YM_WRKBOOK");
			if (ins_cnt <= 0) 
			{
				throw new Exception("작업예약 등록실패");
			}
			...
			**********************************************************/
			...
			recInTemp.setField("YD_WBOOK_ID"      	, ydWbookId     ); //야드작업예약ID
			recInTemp.setField("MODIFIER"         	, modifier      ); //수정자
			recInTemp.setField("YD_STK_COL_GP"     	, ydStackColGp       ); //야드구분
			recInTemp.setField("YD_STK_BED_NO"     	, ydStackBedGp    ); // TAKE IN이 아니면 00
			recInTemp.setField("YD_STK_LYR_NO"   	, ydStackDanGp        ); //단구분
			ins_cnt = commDao.insert(recInTemp, insWrkBookMtlByStkLyr, logId, methodNm, "TB_YM_WRKBOOKMTL");
			if (ins_cnt <= 0) 
			{
				throw new Exception("작업예약 재료 등록실패");
			}
			...
			**********************************************************/
			JDTORecord jrYdMsg = JDTORecordFactory.getInstance().create();
			String sStockMv	= yfComm.getStockMv(logId, methodNm, sCoilNo);
			
			jrYdMsg.setResultCode(logId);	//Log ID
			jrYdMsg.setResultMsg(methodNm);	//Log Method Name
			jrYdMsg.setField("MODIFIER"     		, modifier ); //수정자
			jrYdMsg.setField("STOCK_MOVE_TERM" 		, sStockMv);  //작업행선
			jrYdMsg.setField("STOCK_ITEM" 			, "CM");  //작업행선
			jrYdMsg.setField("STL_NO" 	    		, sCoilNo);
			jrYdMsg.setField("HEATING_COIL_YN" 	    , ("H".equals(bizGp)?"Y":""));
			commDao.update(jrYdMsg, insStock, logId, methodNm, "저장품 재료행선 UPDATE");
	...
	*/
	public JDTORecord rcvF1YFL041(JDTORecord rcvMsg) throws DAOException {
		...
		try{
			...
			String modifier     = commUtils.trim(rcvMsg.getFieldString("MODIFIER"   ));
			String ydStackColGp = ""; //권상위치
			String ydStackBedGp = ""; //권상위치
			String ydStackDanGp = ""; //권상위치
			String heatYn       = "";
			String frStlNo		= "";
			...
			**********************************************************/
			if ("".equals(location)) 
			{
				throw new Exception("line off 권상위치 누락");
			}
			...
			**********************************************************/
			...
			{
				ydStackColGp = "1CDC01"; //권상위치
				ydStackBedGp = "14"; //권상위치
				ydStackDanGp = "01"; //권상위치
			}
			else
			{
				ydStackColGp = "1BDC01"; //권상위치
				ydStackBedGp = "05"; //권상위치
				ydStackDanGp = "01"; //권상위치
			}
			...
			/**********************************************************
			* 3. Line off 대상 재료 검색
			*    - 화면 백업시 재료번호가 파라미터로 들어오는 경우는 skip 
			**********************************************************/
			JDTORecord jrParam = JDTORecordFactory.getInstance().create();
			JDTORecord jrStl = JDTORecordFactory.getInstance().create();
			JDTORecordSet jsStd = null;
			JDTORecordSet jsStl = null;
			if("".equals(sCoilNo) || sCoilNo ==null)
			{
				
				/**********************************************************
				* 3-1. C동 lineoff 경우
				**********************************************************/
				jsStd = commDao.select(jrParam, getLineOffTurnTableStlNo, logId, methodNm, "turn table 고장 재료 검색");
				jrParam.setField("DTL_ITEM2"	, jsStd.getRecord(0).getFieldString("DTL_ITEM2")        ); //코일번호
				if("C".equals(location))
				{
					jsStl = commDao.select(jrParam, getLineOffTargets2, logId, methodNm, "C동 LINE OFF 대상 재료 검색");
				}
				/**********************************************************
				* 3-2. B동 lineoff 경우
				**********************************************************/
				...
				{
					jsStl = commDao.select(jrParam, getLineOffTargets, logId, methodNm, "B동 LINE OFF 대상 재료 검색");
				}
				
				/**********************************************************
				* 3-3. 정보 set
				**********************************************************/
				if(jsStl == null || jsStl.size()<1 )
				{
					throw new Exception("line off 대상재 찾기 실패");
				}
				jrStl = jsStl.getRecord(0);
				sCoilNo      = commUtils.trim(jrStl.getFieldString("STL_NO"     ));
				heatYn       = commUtils.trim(jrStl.getFieldString("HEATING_COIL_YN"     ));
			}
			...
			jrParam.setField("STL_NO"	, sCoilNo        ); //야드설비ID
			jrParam.setField("MODIFIER" , modifier       ); //수정자
			JDTORecordSet jsChk = commDao.select(jrParam, getLinOffYN, logId, methodNm, "작업예약 등록여부");
			...
			if(!"I".equals(bizGp)) //TAKE IN이 아니면
			{
				...
				JDTORecord jrInRecord = JDTORecordFactory.getInstance().create();
				jrInRecord.setField("YD_STK_COL_GP"			, ydStackColGp); 
				jrInRecord.setField("YD_STK_BED_NO"			, ydStackBedGp);	 
				jrInRecord.setField("YD_STK_LYR_NO"			, ydStackDanGp);	 
				jrInRecord.setField("STL_NO"				, sCoilNo);	 
				jrInRecord.setField("YD_STK_LYR_STAT"		, "C");  // 
				jrInRecord.setField("MODIFIER" , modifier       ); //수정자
				commDao.update(jrInRecord, updStackLayerStat, logId, methodNm, "LAYOUT 갱신");	
			...
			else if("I".equals(bizGp)) //TAKE IN이면
			{
				...
				if(!"C".equals(ydStackColGp.substring(1, 2)))
				{
					throw new Exception("대상재료의 현재동을 확인하세요 : "+ydStackColGp.substring(1, 2));
				}
				
				//직전 코일 번호 get
				JDTORecordSet befStlSet = commDao.select(jrParam, getLineOffBefStlNo, logId, methodNm, "요구정보 조회");
				if(befStlSet != null && befStlSet.size()>0)
				{
					frStlNo = befStlSet.getRecord(0).getFieldString("STL_NO");
				}
			...
			String ydAimBayGp = "";
			String item = "";
			if("Y".equals(heatYn)) //난방코일 LINE OFF
			{
				suffixCd = "DC12LM";
				item = "H";
			}
			...
			**********************************************************/
			...
			jrYdMsg.setField("STOCK_ITEM" 			, "CM");  //작업행선
			jrYdMsg.setField("STL_NO" 	    		, sCoilNo);
			jrYdMsg.setField("HEATING_COIL_YN" 	    , heatYn);
			jrYdMsg.setField("DC_TKIN_FR_STL_NO" 	, frStlNo);
			
			commDao.update(jrYdMsg, insStockLineOff, logId, methodNm, "저장품 재료행선 UPDATE");
	...
	*/
	public JDTORecord rcvF1YFL042(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "CTS대차위치정보[ACoilRcvL2SeEJB.rcvF1YFL042] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		...
		try {
			...
			String msgId    = commUtils.getMsgId(rcvMsg); //EAI, JMS, HTTP(출하관리 등) 수신 전문 I/F ID
			String modifier = commUtils.trim(rcvMsg.getFieldString("MODIFIER"     )); //수정자(Backup Only)
			String ydTcarLocBay1 	= commUtils.trim(rcvMsg.getFieldString("YD_TCAR_LOC_BAY1"     )); //재료번호
			String ydTcarLocX1 		= commUtils.trim(rcvMsg.getFieldString("YD_TCAR_LOC_X1"     )); //야드설비ID
			String ydTcarLocBay2 	= commUtils.trim(rcvMsg.getFieldString("YD_TCAR_LOC_BAY2"     )); //야드권하실적위치
			String ydTcarLocX2 		= commUtils.trim(rcvMsg.getFieldString("YD_TCAR_LOC_X2"     )); //대차상차하차구분
			...
			**********************************************************/
			...
			param.setResultMsg(methodNm);	//Log Method Name
			param.setField("MODIFIER"     		, modifier);	//수정자
			param.setField("YD_EQP_ID"     		, "1XTC01" );  
			param.setField("YD_CURR_BAY_GP"     , ydTcarLocBay1);  
			commDao.update(param, updCTSLoc, logId, methodNm, "01번 대차 수정");
			...
			**********************************************************/
			...
			param.setResultMsg(methodNm);	//Log Method Name
			param.setField("MODIFIER"     		, modifier);	//수정자
			param.setField("YD_EQP_ID"     		, "1XTC02" );  
			param.setField("YD_CURR_BAY_GP"     , ydTcarLocBay2 );
			commDao.update(param, updCTSLoc, logId, methodNm, "02번 대차 수정");
	...
	*/
	public JDTORecord rcvF1YFL043(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "CTS작업실적[ACoilRcvL2SeEJB.rcvF1YFL043] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		...
		try {
			...
			String ydEqpId 		= commUtils.trim(rcvMsg.getFieldString("YD_EQP_ID"     )); //야드설비ID
			String ydDnWrLoc 	= commUtils.trim(rcvMsg.getFieldString("YD_DN_WR_LOC"     )); //야드권하실적위치
			String tcUpDnGp 	= commUtils.trim(rcvMsg.getFieldString("TC_UP_DN_GP"     )); //대차상차하차구분(상차 :U 하차 :D)
			String ydBayGp		= commUtils.trim(rcvMsg.getFieldString("YD_DN_WR_LOC"     )).substring(1, 2); //동
			
			if ("".equals(modifier)) { modifier = msgId; }
			methodNm = msgId.substring(0, 2) + methodNm;
			
			//작업 상태에 따라 에러 체크 필요 KBS
			/**********************************************************
			* 1. 대차상차
			**********************************************************/
			if("U".equals(tcUpDnGp))
			{
				...
				**********************************************************/
				...
				param.setResultMsg(methodNm);	//Log Method Name
				param.setField("MODIFIER"     		, modifier);	//수정자
				param.setField("YD_EQP_ID"     		, ydEqpId );  	 
				param.setField("YD_EQP_PROG_STAT"   , "2" ); 
				param.setField("YD_WRK_PROG_STAT"   , "2" );
				param.setField("YD_EQP_ID"     		, ydEqpId );
				param.setField("STL_NO"   			, stlNo );  		
				param.setField("YD_CARLD_WR_LOC"   	,ydDnWrLoc );  //상차실적위치(대차위치)
				param.setField("YD_CURR_BAY_GP"   	,ydDnWrLoc.substring(1, 2) );  //현재동
				param.setField("YD_STK_LYR_STAT"   	,"C" ); 
				param.setField("YD_STK_COL_GP"   	,ydEqpId ); 
				param.setField("YD_STK_BED_NO"   	,"01" ); 
				param.setField("YD_STK_LYR_NO"   	,"01" ); 
				
				JDTORecordSet lyrSet = commDao.select(param, chkStackLayerStat, logId, methodNm, "대차상태 확인"); 
				if(lyrSet.size()>0)
				{
					if("Y".equals(lyrSet.getRecord(0).getFieldString("B_EXSIST")))
					{
						throw new Exception("현 대차에 재료가 존재하여, 상차 할수 없습니다.");
					}
				}
				...
				* 1.2 CTS 대차 상차 실적 수정
				**********************************************************/
				commDao.update(param, updEqpOprnStat2, logId, methodNm, "설비 상태 수정(CTS 대차 상차)");
				
				commDao.update(param, updCTSSchUpModify, logId, methodNm, "CTS스케줄 상차 실적 수정");
				
				commDao.update(param, updStackLayerByStockId2, logId, methodNm, "SADDLE 적치단 초기화");
				
				commDao.update(param, updStackLayerStat, logId, methodNm, "CTS 대차로 이적");
				
				
				/**********************************************************
				* 1.3 SADDLE이 가득차 CTS대차가 휴식중 이라면 다시 CTS대차를 기동시킨다. 
				**********************************************************/
				JDTORecord ctsParam = JDTORecordFactory.getInstance().create();
				ctsParam.setResultCode(logId);	//Log ID
				ctsParam.setResultMsg(methodNm);	//Log Method Name
				ctsParam.setField("MODIFIER" , modifier     ); 
				ctsParam.setField("BAY_GP" 	 , ydDnWrLoc.substring(1, 2)); 
				JDTORecordSet ctsSet = commDao.select(ctsParam, getCTSEqpId, logId, methodNm, "cts대차 설비 조회 "); 
		    	if(ctsSet.size()>0)
		    	{
		    		ctsParam.setField("YD_EQP_ID" 	, ctsSet.getRecord(0).getFieldString("YD_EQP_ID")); 
			...
			**********************************************************/
			else if("D".equals(tcUpDnGp))
			{
				...
				**********************************************************/
				...
				param.setField("MODIFIER"     		, modifier);	//수정자
				param.setField("YD_EQP_ID"     		, ydEqpId );  	 
				param.setField("YD_EQP_PROG_STAT"   , "W" ); 
				param.setField("YD_WRK_PROG_STAT"   , "4" );
				param.setField("YD_EQP_ID"     		, ydEqpId );
				param.setField("STL_NO"   			, stlNo );  		
				param.setField("YD_CARUD_WR_LOC"   	,ydDnWrLoc );  //하차실적위치(SADDLE위치)
				param.setField("YD_CURR_BAY_GP"   	,ydDnWrLoc.substring(1, 2) );  //현재동
				param.setField("YD_STK_LYR_STAT"   	,"C" ); 
				param.setField("YD_STK_COL_GP"   	,ydDnWrLoc );  
				param.setField("YD_STK_BED_NO"   	,"01" ); 
				param.setField("YD_STK_LYR_NO"   	,"01" ); 
				...
				* 2.2 CTS 대차 하차 실적 수정
				**********************************************************/
				commDao.update(param, updEqpOprnStat2, logId, methodNm, "설비 상태 수정(CTS 대차 하차)");
				
				commDao.update(param, updCTSSchDnModify, logId, methodNm, "CTS스케줄 하차 실적 수정(CTS 대차 하차)");
				
				commDao.update(param, updStackLayerByStockId2, logId, methodNm, "CTS 대차위치 초기화");
				
				commDao.update(param, updStackLayerStat, logId, methodNm, "SADDLE로 이적");
				
				
				/**********************************************************
				* 2.3 CTS 스케줄 정보 조회
				**********************************************************/
				JDTORecordSet schSet = commDao.select(param, selectCTSSch, logId, methodNm, "CTS스케줄 조회");
				JDTORecord schRow    = schSet.getRecord(0);
				String ydCtsRelayYn 	= schRow.getFieldString("YD_CTS_RELAY_YN");
				String ydCtsRelayBayGp	= schRow.getFieldString("YD_CTS_RELAY_BAY_GP");
				String ydToLocGuide	 	= schRow.getFieldString("YD_TO_LOC_GUIDE");
				
				
				/**********************************************************
				* 2.4 중계동 도착이라면 
				**********************************************************/
				if("Y".equals(ydCtsRelayYn)
						&&ydCtsRelayBayGp.equals(ydBayGp))
				{
					...
					**********************************************************/
					...
					param.setField("YD_CTS_SCH_ID"   	,ydSchId ); 
					commDao.insert(param, insCTSRelaySch, logId, methodNm, "중계 CTS 스케줄 생성");
					...
					**********************************************************/
					...
					jrYdMsg.setField("YD_EQP_ID" , "1XTC02".equals(ydEqpId)?"1XTC01":"1XTC02");
					jrRtn = commUtils.addSndData(jrRtn, yfComm.getCTSToLoc(jrYdMsg));
					
				}
				
				/**********************************************************
				* 2.5 목적동 도착이라면 크레인 작업예약/스케줄 호출
				**********************************************************/
				else
				{
					...
					**********************************************************/
					...
					{
						nLtRtRule   = rsResult.getRecord(0).getFieldString("YD_RL_GP");
						heatYn      = rsResult.getRecord(0).getFieldString("HEATING_COIL_YN");
					}
					...
					/*else if("BDSP001".equals(stlNo)) //spm분동코일
					{
						ydSchCd = ydDnWrLoc.substring(0, 2)+"TC41LM";
						ydToLocGuide = "1E05021101";
					}
					else if("BDEQ001".equals(stlNo)) //eql분동코일
					{
						ydSchCd = ydDnWrLoc.substring(0, 2)+"TC41LM";
						ydToLocGuide = "1F12021001";
					}*/
					else
					{
				...
				* 2.6 CTS 스케줄 삭제
				**********************************************************/
				commDao.update(param, deleteCTSSch, logId, methodNm, "CTS 스케줄 삭제");
