...
//import com.inisteel.cim.yf.acoilOld.ThreadExceptionHandler;
import com.inisteel.cim.yf.acoilBak.YfCommUtils;
import com.inisteel.cim.yf.acoilBak.YfConstant;
import com.inisteel.cim.yf.acoilBak.YfParseAndValidateTC;
 ...
 *      [A] 클래스명 :  박판열연 IF 수신
 *
 * @ejb.bean name="YfRcvBakFaEJB" jndi-name="YfRcvBakFaEJB" type="Stateless"
...
public class YfRcvFaEJBSBean extends BaseSessionBean 
{
	...
	public String rcvInterface(JDTORecord into) throws DAOException 
	{
		...
		try
		{
			...
			//4. 메소드 호출
			EJBConnector ejbConn = new EJBConnector("default", "YfRcvBakFaEJB", this);  
	...
	public JDTORecord rcvPOYMJ001(JDTORecord rcvMsg) throws DAOException 
	{
		...
		try 
		{
			...
			EJBConnector ejbCon = new EJBConnector("default", "ACoilRcvL3BakSeEJB", this);
	...
	public JDTORecord rcvPOYMJ007(JDTORecord rcvMsg) throws DAOException 
	{
		...
		try 
		{
			...
			EJBConnector ejbCon = new EJBConnector("default", "YfCommBakSeEJB", this);
	...
	/**	
	 * [A] 오퍼레이션명 : 코일 SPM / HFL 작업 요구 정보를 수신 (POYMJ010)
	 *
	 * @ejb.interface-method EJBDoclet을 생성하는 태그입니다.
	 * @param JDTORecord rcvMsg
	 * @return JDTORecord
	...
	{
		String		methodNm	= "코일 SPM / HFL 작업 요구 정보를 수신[YfRcvFaEJB.rcvPOYMJ010] < " + rcvMsg.getResultMsg();
		String		logId		= rcvMsg.getResultCode();
		...
		try 
		{
			...
			//박판에는 SPM2 설비가 없어서 사용안함
			//EJBConnector ejbCon = new EJBConnector("default", "ACoilRcvL3BakSeEJB", this);
			//jrRtn = (JDTORecord)ejbCon.trx("rcvPOYMJ010", new Class[]{ JDTORecord.class }, new Object[]{ rcvMsg });
		...
		{
			logger.println("==================rcvPOYMJ010 catch 1");
			throw e;
		}
		catch (Exception e) 
		{
			logger.println("==================rcvPOYMJ010 catch 2");
			throw new DAOException(yfCommUtils.makeErrorLog(logId, methodNm, e));
		}
	}
	...
	{
		String 		methodNm	= "코일제품보류확정[YfRcvFaEJB.rcvDMYDR002] < " + rcvMsg.getResultMsg();
		String 		logId		= rcvMsg.getResultCode();
		JDTORecord	jrRtn		= JDTORecordFactory.getInstance().create();
		...
		try 
		{
			...
			//EJBConnector ejbCon = new EJBConnector("default", "ACoilRcvL3BakSeEJB", this);
			//((Boolean)ejbCon.trx("rcvDMYDR002", new Class[]{ JDTORecord.class }, new Object[]{ rcvMsg })).booleanValue();
		...
		{
			logger.println("==================rcvDMYDR002 catch 1");
			throw e;
		}
		catch (Exception e) 
		{
			logger.println("==================rcvDMYDR002 catch 2");
			throw new DAOException(yfCommUtils.makeErrorLog(logId, methodNm, e));
		}
	}
	...
	{
		String 		methodNm	= "코일제품고간이송지시 [YfRcvFaEJB.rcvDMYDR011] < " + rcvMsg.getResultMsg();
		String 		logId		= rcvMsg.getResultCode();
		JDTORecord	jrRtn		= JDTORecordFactory.getInstance().create();
		...
		try 
		{
			...
			else
			{
				//지시('N')
				EJBConnector ejbCon = new EJBConnector("default", "ACoilRcvL3BakSeEJB", this);
				((Boolean)ejbCon.trx("rcvDMYDR011", new Class[]{ JDTORecord.class }, new Object[]{ rcvMsg })).booleanValue();
			}
		...
		{
			logger.println("==================rcvDMYDR011 catch 1");
			throw e;
		}
		catch (Exception e) 
		{
			logger.println("==================rcvDMYDR011 catch 2");
			throw new DAOException(yfCommUtils.makeErrorLog(logId, methodNm, e));
		}
	}
	...
	{
		String 		methodNm	= "코일 결번 실적[YfRcvFaEJB.rcvDMYDR060] < " + rcvMsg.getResultMsg();
		String 		logId		= rcvMsg.getResultCode();
		JDTORecord	jrRtn		= JDTORecordFactory.getInstance().create();
	...
	public JDTORecord rcvYFYFJ001(JDTORecord rcvMsg) throws DAOException 
	{
		...
		try 
		{
			...
			EJBConnector ejbCon = new EJBConnector("default", "ACoilSchBakSeEJB", this);
