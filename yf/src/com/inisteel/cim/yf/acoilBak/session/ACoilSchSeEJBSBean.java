 ...
 *
 * @ejb.bean name="ACoilSchBakSeEJB" jndi-name="ACoilSchBakSeEJB" type="Stateless"
...
public class ACoilSchSeEJBSBean extends BaseSessionBean implements YfQueryIFOld, YfQueryIFOld2 {
...
//    		stockV = dao.getCoilCommonInfo(sCoilNo); // 쿼리가 변경됨. 최규성 운영계 반영시 주의. 
	...
	{	
		...
		try {
            ...
            scrane_up_loc		= "모 종료"; 
            scrane_put_loc		= "모 종료";
            scrane_up_func		= YfConstant.CRANE_FUNC_N; 
            scrane_put_func		= YfConstant.CRANE_FUNC_N;
            scrane_register		= "SYSTEM"; 
            scrane_modifier		= "SYSTEM"; 
            scrane_yd_gp		= sYdGp;
            scrane_sch_code 	= YfConstant.NEW_SCH_WORK_KIND_CKLI;
        	
            scrane_equip_gp = sYdGp + "X" + YfConstant.EQUIP_KIND_CR + "00";
	        
	        scrane_work_duty		= commUtils.getWorkDuty();
			scrane_work_party		= commUtils.getWorkParty();				        
			sch_wdemand_duty		= commUtils.getWorkDuty();
		    sch_wdemand_party		= commUtils.getWorkParty();		
	...
	*/
	public JDTORecord procYFYFJ302(JDTORecord rcvMsg) throws DAOException {
		...
		try {
			...
			**********************************************************/
			jrParam.setField("YD_WBOOK_ID", ydWbookId); //야드작업예약ID
			EJBConnector tranConn = new EJBConnector("default", "ACoilSchBakSeEJB", this);
			...
			* 12. 대차스케줄 공대차출발지시 처리 (별도 Transaction 으로 처리)
			...
			if ("9".equals(commUtils.trim(jrLocSrcRngRtn.getFieldString("RTN")))) 
			{
				...
				try {
					EJBConnector tranConn1 = new EJBConnector("default", "ACoilSchBakSeEJB", this);
	...
	*/
	public JDTORecord rcvYFYFJ303(JDTORecord rcvMsg) throws DAOException {
		...
		try {
			...
			EJBConnector ejbConn = new EJBConnector("default", "ACoilSchBakSeEJB", this); //추가
	...
	*/
    	...
    	try
    	{
			...
			* 3. 크레인스케줄의 권하지시위치 결정
		    ...
		    for (int i = 0; i < schSet.size(); i++) 
		    {
        		...
        		/*********************************************************
        		 * 3-1. 스크랩 - 업무 협의,분석 필요 kbs
        		 ********************************************************/
        		if ("S".equals(stlNo.substring(0,1))) 
        		{       				
        			commUtils.printLog(logId, "["+ i+"]번째 크레인 스케줄[" + ydCrnSchId + "]은  Scrap To위치 결정 시작", "SL");
        			this.procScrapToLoc(logId, methodNm, wbRow, schRow);
            	}
        		/*********************************************************
        		 * 3-2. 보조작업
        		 ********************************************************/
        		else if ("W".equals(szToLocDcsnMtd)) 
       			{            		
        		 ...
        		 * 3-3. 설비보급
              			 ...
              			 &&("FE".equals(szSchCd.substring(2,4))			//HFL 보급
   	           			     || "KE".equals(szSchCd.substring(2,4))		//SPM 보급
   	           			     || "QE".equals(szSchCd.substring(2,4))		//EQL 보급
   	           			   )) 
       			{
        		...
        		/*********************************************************
        		 * 3-4. 차량 상차: 차량이 정지한 적치열 조회 ==> TO위치가 됨  ==> TO 위치 가이드에 등록 처리
       			 * 확인 필! (차량방향, 복수동..) kbs
       			 ********************************************************/
        		...
        		/*********************************************************
        		 * 3-5. 분동코일 대기장 권하(cts하차)
        		 ********************************************************/
       			else if ("TC".equals(szSchCd.substring(2,4)) && "L".equals(szSchCd.substring(6,7))		//대차하차스케줄
   	           			     && "BD".equals(stlNo.substring(0,2))	//재료가 분동코일
   	           			   ) 
       			{
       				commUtils.printLog(logId, methodNm +"["+ i+"]번째 크레인 스케줄[" + ydCrnSchId + "]은 분동코일 스케줄의  To위치 결정 시작", "SL");
   		       		this.procToLocBundong(logId, methodNm, wbRow, schRow);
            	}
        		/*********************************************************
        		 * 3-6. 고정위치
        		 ********************************************************/
       			else if ("1FQE10UM".equals(szSchCd) //EQL재작업(F동)
       					||"1CDC02UM".equals(szSchCd)//DC TAKE IN
       					) 
       			{
       				commUtils.printLog(logId, methodNm +"["+ i+"]번째 크레인 스케줄[" + ydCrnSchId + "]은 설비위 스케줄의  To위치 결정 시작", "SL");
   		       		this.procToLocFix(logId, methodNm, wbRow, schRow);
            	}
        		/*********************************************************
        		 * 3-6. 사용자 지정
        		 *      - 대차상차는 제외, 대차상차는 일반작업에서 처리 
        		 *      - 대차상차라면 동간이적이기 때문에 to위치가이드로 to위치 찾으면 안된다. 
        		 *      - 대차에 올리는것이 먼저임. 
        		 *      - to위치가이드는 대차이동시 참고함.
        		 *      
       			 ********************************************************/
       			else if (szToLocGuide.length() >= 4 && 
       					!("TC".equals(szSchCd.substring(2,4)) && "U".equals(szSchCd.substring(6,7)))) 
       			{
	 ...
	 * @throws JDTOException
	 */
    public String procTcToLoc(String logId, String methodNms, JDTORecord wbRow, JDTORecord schRow) throws JDTOException {
    	String methodNm = "TO 위치 결정: 대차작업[ACoilSchSeEJB.procTcToLoc] < " + methodNms;
    	try 
    	{
    		String ydAimBayGp		= commUtils.trim(wbRow.getFieldString("YD_AIM_BAY_GP"));	//야드 목적동
    		String ydWrkPlanTcar 	= commUtils.trim(wbRow.getFieldString("YD_WRK_PLAN_TCAR"));	//예약등록시 지정계획대차
    		...
    		/**********************************************************
			* 1. to위치 결정에 필요한 정합성 체크
			*  - 목적동에 빈자리, 상차작업가능한 상태에 대차스케줄, 크레인스케줄편성여부
			*    의 합으로 판정하는 것이 합리적인지?? kbs 
  			**********************************************************/
    		jrParam.setField("YD_EQP_ID"	, ydWrkPlanTcar);
			jrParam.setField("YD_AIM_BAY_GP", ydAimBayGp);
			JDTORecordSet jsTcarInfo = commDao.select(jrParam, getTcBayWrk, logId, methodNm, "대차정보조회");
			if (jsTcarInfo.size() <= 0) 
			{
				commUtils.printLog(logId, methodNm + " : 대차 to위치결정 정합성 검증 실패!", "SL");
			...
			* 2. 대차정보 조회
			...
			if (!szSchCd.substring(1,2).equals(ydTcarCurrBayGp) && !"0".equals(tcarUdCmplYn)) 
			{
				commUtils.printLog(logId, methodNm + " : 대차가 다른동에서 작업중임", "SL");
				return YfConstant.RETN_CD_FAILURE;
			}
			...
			* 3. to위치가이드 조회
  			**********************************************************/
			jrParam.setField("YD_STK_COL_GP", ydWrkPlanTcar.substring(0,1) + szSchCd.substring(1,2) + ydWrkPlanTcar.substring(2,6));
			JDTORecordSet jsTcarBed = commDao.select(jrParam, getYdStkBedBybedTc, logId, methodNm, "대차TOSQL 조회");
			if (jsTcarBed.size() <= 0) 
    		{
			...
			* 4. to위치결정 모듈 호출
			...
			String StackColGp	= commUtils.trim(jrTcarBed.getFieldString("YD_STK_COL_GP"));//대차정지위치 적치열
			String StackBedGp	= commUtils.trim(jrTcarBed.getFieldString("YD_STK_BED_NO"));//대차정지위치 적치베드
			String StackLayerGp= commUtils.trim(jrTcarBed.getFieldString("YD_STK_LYR_NO"));//대차정지위치 적치단
			...
			{	
				commUtils.printLog(logId,  " : 대차 위치가 작업동과 같은동에 없는 경우 대차 호출", "SL");
				return "Y";
			}
	 ...
	 * @throws JDTOException
	 */
    public String procCTSToLoc(String logId, String methodNms, JDTORecord wbRow, JDTORecord schRow) throws JDTOException {
    	String methodNm = "TO 위치 결정: CTS작업[ACoilSchSeEJB.procCTSToLoc] < " + methodNms;
    	try 
    	{
    		String szSchCd        	= schRow.getFieldString("YD_SCH_CD");
			...
			* 1. SADDLE to위치가이드 조회
  			**********************************************************/
			jrParam.setField("BAY_GP", szSchCd.substring(1,2)) ;
			JDTORecordSet jsTcarBed = commDao.select(jrParam, getYdStkBedBybedSaddle, logId, methodNm, "대차TOSQL 조회");
			if (jsTcarBed.size() <= 0) 
    		{
				commUtils.printLog(logId, methodNm + " : CTS SADDLE READ 실패!", "SL");
				return YfConstant.RETN_CD_FAILURE;
    		} 
			...
			* 2. to위치결정 모듈 호출
			...
			JDTORecord jrTcarBed  = jsTcarBed.getRecord(0);
			String StackColGp	  = commUtils.trim(jrTcarBed.getFieldString("YD_STK_COL_GP"));//대차정지위치 적치열
			String StackBedGp	  = "01";//새들 적치베드
			String StackLayerGp   = "01";//새들 적치단
    ...
    public String procScrapToLoc(String logId, String methodNms, JDTORecord jrWbook, JDTORecord jrCrnSch) throws JDTOException {
    	String methodNm = "TO 위치 결정:Scrap작업[ACoilSchSeEJB.procScrapToLoc] < " + methodNms;
		...
		String StockId	   	= commUtils.trim(jrCrnSch.getFieldString("STL_NO"));			//크레인작업재료
		...
		String ydUpWoLayer 	= commUtils.trim(jrCrnSch.getFieldString("YD_UP_WO_LYR"  ));		
		...
		try 
		{
			...
			jrTemp.setField("STL_NO"			, StockId);											//권상 STOCK
			...
			else 
			{
				...
				/**********************************************************
				 * ★★★★검색조건은 스크랩거 사용 1EKE02LM  -> 1ESC01LM ★★★★
				 **********************************************************/ 
				...
				JDTORecordSet outjsResult = null;
 				if(ydToLocGuide.length() >= 6) 
 				{
 					if(ydToLocGuide.length() == 6) 
 					{
 						jrTemp.setField("YD_STK_COL_GP"			, ydToLocGuide.substring(0,6));		//적치열									
 						jrTemp.setField("YD_STK_BED_NO"			, "%");		//적치BED									
 					} 
 					else if(ydToLocGuide.length() >= 8) 
 					{
 						jrTemp.setField("YD_STK_COL_GP"			, ydToLocGuide.substring(0,6));		//적치열									
 						jrTemp.setField("YD_STK_BED_NO"			, ydToLocGuide.substring(6,8));		//적치BED									
 					}
 					outjsResult =	commDao.select(jrTemp, getYdPrimaryWorkSearchScrapNewToLocGuide, logId, methodNm, "TO위치 가이드로 ScrapTOSQL 베드 조회");
 				} 
 				else 
 				{
 					outjsResult =	commDao.select(jrTemp, getYdPrimaryWorkSearchScrapNew, logId, methodNm, "ScrapTOSQL 베드 조회");
 				}
 				...
 				if (outjsResult.size() <= 0) 
 				{
 					...
 					if ("Y".equals(sAPP005_YN)) 
 					{
		    			...
		    			EJBConnector SchLog = new EJBConnector("default", "ACoilSchBakSeEJB", this);
 				...
 				else 
 				{
 					...
 					szStackColGp  	= commUtils.trim(outjsResult.getRecord(0).getFieldString("TAG_YD_STK_COL_GP"));
 					szStackBedGp  	= commUtils.trim(outjsResult.getRecord(0).getFieldString("TAG_YD_STK_BED_NO"));
 					szStackLayerGp  = commUtils.trim(outjsResult.getRecord(0).getFieldString("TAG_YD_STK_LYR_NO"));
			...
			jrSetLoc.setField("YD_UP_WO_LYR",	ydUpWoLayer);	 
			jrSetLoc.setField("YD_DN_WO_LOC", 	sRtnBedDan.substring(0,8));
			jrSetLoc.setField("YD_DN_WO_LYR", sRtnBedDan.substring(8,10));
	...
	public String procUpdateLoc(String logId, String methodNms, JDTORecord jrSetLoc, JDTORecord jrCrnWrk) throws JDTOException {
		...
		try {
			...
			jrParam.setField("YD_STK_BED_NO", 	szYD_DN_WO_LOC.substring(6));
			jrParam.setField("YD_STK_LYR_NO", 	commUtils.stringPlusInt(szYD_DN_WO_LAYER,0));
			jrParam.setField("STL_NO",       	szSTOCK_ID);
			jrParam.setField("YD_STK_LYR_STAT", ("00".equals(szYD_DN_WO_LOC.substring(6)))?"":"D"); //설비 보급위치는 예약(D)하지 않는다.
			if ("Y".equals(sAPP000_YN)) {
				commUtils.printLog(logId, "TB_YF_STKLYR 권하위치 수정", "[INFO]");
				if (yfComm.execQueryId(jrParam, updYdStkLyrYdStkColBedGp)) {
					intRtnVal = 1;
				} else {
					intRtnVal = 0;
				}
			} else {
				intRtnVal = commDao.update(jrParam, updYdStkLyrYdStkColBedGp, logId, methodNm, "TB_YM_STKLYR 갱신");
			}
	...
	*/
	 ...
	 *      @ejb.transaction type="RequiresNew"
	*/
	public void insSchLogs(JDTORecordSet set, String prefix, String schId, String schCd) throws DAOException {
		String methodNm = "";
		String logId = "";
		String strContents = "";
		JDTORecord row = JDTORecordFactory.getInstance().create();
    ...
    public JDTORecordSet procToLocDummy(String logId, String methodNms, JDTORecord wbRow, JDTORecord schRow) throws JDTOException {
		...
		try {
			...
			if ("Y".equals(sAPP005_YN))
			{
				...
				{
					EJBConnector SchLog = new EJBConnector("default", "ACoilSchBakSeEJB", this);
	    			SchLog.trx( "insSchLogs" , new Class[] { JDTORecordSet.class, String.class , String.class, String.class}
	    			, new Object[] { bedSet,"",szYD_CRN_SCH_ID, szYD_SCH_CD });
				}
				else
				{
					EJBConnector SchLog = new EJBConnector("default", "ACoilSchBakSeEJB", this);
    ...
    public String procToLocConveyor(String logId, String methodNms, JDTORecord jrWbook, JDTORecord jrCrnSch) throws JDTOException {
		...
		try {
			...
			if ("1BFE01UM".equals(szYD_SCH_CD)) //HFL
			{
				sRtnBed = "1BFE0100";
			}
			else if ("1DKE01UM".equals(szYD_SCH_CD))//SPM
			{
				sRtnBed = "1DKE0100";
			}
			else if ("1EKE10UM".equals(szYD_SCH_CD)) //SPM
			{
				sRtnBed = "1EKE0200"; //재작업 위치(방향) 확인 필요 kbs
			}
			else if ("1EQE01UM".equals(szYD_SCH_CD))//EQL
			{
				sRtnBed = "1EQE0100";
			}
			else if ("1FQE01UM".equals(szYD_SCH_CD)) //EQL
			{
				sRtnBed = "1FQE0100"; 
			}
	 ...
	 * @throws JDTOException
	 */
    public String procToLocBundong(String logId, String methodNms, JDTORecord jrWbook, JDTORecord jrCrnSch) throws JDTOException {
    	String methodNm = "TO위치결정:분동코일 대기장 TO위치결정[ACoilSchSeEJB.procToLocBundong] < " + methodNms;
    	String szLogMsg					= null;
		JDTORecord		jrTemp			= null;
		...
		try {
			...
			if ("BDSP001".equals(szSTOCK_ID)) //SPM
			{
				sRtnBed = "1E050211";
			}
			else if ("BDEQ001".equals(szSTOCK_ID))//EQL
			{
				sRtnBed = "1F120210";
			}
    ...
    public JDTORecordSet procToLocPrimaryWork(String logId, String methodNms, JDTORecord wbRow, JDTORecord schRow) throws JDTOException {
		...
		try {
			...
			/** 
			 * 3. ZONE 적용 여부 체크
			 *   -CTS 하차, HFL추출(후물재만) 일때만 ZONE을 적용한다.
			 *   
			 * */
			if ("TC01L".equals(szYD_SCH_CD.substring(2,7)) 
        			||"TC11L".equals(szYD_SCH_CD.substring(2,7)) 
        			||"FD01L".equals(szYD_SCH_CD.substring(2,7))
        			||"KD01L".equals(szYD_SCH_CD.substring(2,7))
        			||"QD01L".equals(szYD_SCH_CD.substring(2,7)) 
        			
        			||"QD10U".equals(szYD_SCH_CD.substring(2,7)) //재작업 추가
        			||"KD10U".equals(szYD_SCH_CD.substring(2,7)) //재작업 추가
        			) 
        	{
				JDTORecord jrParam = JDTORecordFactory.getInstance().create();
				jrParam.setField("STL_NO", szSTOCK_ID);
				JDTORecordSet rsResult = commDao.select(jrParam, getYfZoneGp, logId, methodNm, "ZONE 구분 조회");
				
				if(rsResult != null && rsResult.size()>0)
				{
					szYD_ZONE_GP   = rsResult.getRecord(0).getFieldString("YD_ZONE_GP");
				}
				
				
				//HFL추출 후물재 제외 테스트 위해 임시 막음 20 01 13
				//if("FD01L".equals(szYD_SCH_CD.substring(2,7)) && !"HZ".equals(szYD_ZONE_GP))
				//{
				//	szYD_ZONE_GP ="";
				//}
        	}
			...
			jrTemp.setField("YD_EQP_ID"		, szYD_EQP_ID);		//설비ID
			jrTemp.setField("YD_CRN_SCH_ID"	, szYD_CRN_SCH_ID);	//크레인 스케쥴 ID
			jrTemp.setField("YD_ZONE_GP"  	, szYD_ZONE_GP); 	//야드존구분
			//jrTemp.setField("BIZ_GP"  		, "1"); //1:주작업 2:더미 3:사용자 4:차공정 5:진도 6:zone
			jrTemp.setField("BIZ_GP"  		, "".equals(szYD_ZONE_GP)?"1":"6"); //1:주작업 2:더미 3:사용자 4:차공정 5:진도 6:zone	
			JDTORecordSet bedSet = commDao.select(jrTemp, getYfToLocMainSql, logId, methodNm, "동일한 적치가능한 베드 조회");
			
	
			
			/** 
		     * 5. 크레인현황조회 화면이라면 적재위치 list 출력하고 리턴.
		     * 
		     * */
			JDTORecord bedRow	= JDTORecordFactory.getInstance().create();
			JDTORecord udpRow = JDTORecordFactory.getInstance().create();
			...
			for (int i = 0; i < bedSet.size(); i++) 
			{
				...
				if("PASS".equals(bedRow.getFieldString("LOC_ABLE_CHK")))
				{
					...
					szNEXT_PROC			= "".equals(bedRow.getFieldString("C_NEXT_PROC"))?"GN":bedRow.getFieldString("C_NEXT_PROC");
			...
			if (!"".equals(finalToLoc)) 
			{
				...
				udpRow.setField("YD_DN_WO_LYR",		finalToLoc.substring(8,10));
				udpRow.setField("YD_WBOOK_ID", 		szYD_WBOOK_ID);
				udpRow.setField("YD_ROUTE_GP", 		"".equals(szYD_ZONE_GP)?szNEXT_PROC:szYD_ZONE_GP);
				this.procUpdateLoc(logId,methodNm, udpRow, schRow  );
				
				//스케줄 코드가 대차 상차가 아닌데, TO위치가 대차로 정해졌다면, 작업예약에 목적동,to위치가이드, 계획대차를 갱신한다.
				if(("TC".equals(finalToLoc.substring(2,4))
						||"SR".equals(finalToLoc.substring(2,4))
						||"SL".equals(finalToLoc.substring(2,4)))
						&& 
						!("TC".equals(szYD_SCH_CD.substring(2,4)) && "U".equals(szYD_SCH_CD.substring(6,7)))
					)
				{
					szLogMsg = methodNm+ "TO위치가 대차이므로, 작업예약에 목적동,계획대차를 갱신함 :" +finalToLoc + ", "+szYD_SCH_CD;
					commUtils.printLog(logId, szLogMsg, "SL");
				
					if("".equals(commUtils.trim(szYD_AIM_BAY_GP)))
					{
						szLogMsg = methodNm+ "대차 목적동 누락";
						commUtils.printLog(logId, szLogMsg, "SL");
						throw new Exception(szLogMsg);
					}

					udpRow.setField("MODIFIER", 		schRow.getFieldString("MODIFIER")); 
					udpRow.setField("YD_AIM_BAY_GP", 	szYD_AIM_BAY_GP); 
					udpRow.setField("YD_WRK_PLAN_TCAR", szYD_WRK_PLAN_TCAR);
					commDao.update(udpRow, updWrkBookAimBayGp, logId, methodNm, "작업예약 목적동 갱신");
				}
			}
	...
	*/
	public JDTORecord updTcarSchLevWo(JDTORecord jrParam) throws DAOException {
		String methodNm = "대차스케줄 공대차출발지시 처리[ACoilSchSeEJB.updTcarSchLevWo] < " + jrParam.getResultMsg();
		String logId = jrParam.getResultCode();

		try {

			return commUtils.addSndData(yfComm.trtTcarSchLevWo(jrParam));
    	...
    	try{
		    ...
		    for(int i = 0; i < jsCrnsch.size(); i++) 
		    {
        		...
        		commUtils.printLog(logId, szMsg, "SL");
       			...
       			if ("S".equals(StockId.substring(0,1))) 
       			{       				
            		/**********************************************************
    				* SPM2 Scrap 인 경우  : 적치 가능 여부 CHECK 안함
    				**********************************************************/            		
            		szMsg = "["+ i+"]번째 크레인 스케줄[" + ydCrnSchId + "]은  Scrap To위치 결정 시작";
        			commUtils.printLog(logId, szMsg, "SL");
    ...
    public String procStockIdBaseCheckNew(String logId, String methodNms,JDTORecord jrLayer) throws JDTOException {
		...
		try {
			...
			if ("Y".equals(sAPP005_YN) && bedSet.size() > 0) 
			{
				EJBConnector SchLog = new EJBConnector("default", "ACoilSchBakSeEJB", this);
    			SchLog.trx( "insSchLogs" , new Class[] { JDTORecordSet.class, String.class , String.class, String.class}
    			, new Object[] { bedSet,"",ydCrnSchId, ydSchCd });
			}
			else if (sRtnBedDan.length() < 10) 
			{
				EJBConnector SchLog = new EJBConnector("default", "ACoilSchBakSeEJB", this);
    			SchLog.trx( "insSchLogs" , new Class[] { JDTORecordSet.class, String.class , String.class, String.class}
    			, new Object[] { bedSet,"대상코일선택실패: ",ydCrnSchId, ydSchCd });
