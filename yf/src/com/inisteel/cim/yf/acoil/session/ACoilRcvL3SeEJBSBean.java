 ...
 *
 * @ejb.bean name="ACoilRcvL3SeEJB" jndi-name="ACoilRcvL3SeEJB" type="Stateless"
...
public class ACoilRcvL3SeEJBSBean extends BaseSessionBean implements YfQueryIF, YfQueryIF2
{
	...
	*/
	...
	public JDTORecord rcvDMYDR005(JDTORecord rcvMsg)
	{
		...
		try
		{
			...
			String yd_gp	= commUtils.paraRecChkNull(rcvMsg, "YD_GP");
			...
			if( "Y".equals(commUtils.trim(rsResult.getRecord(0).getFieldString("LOC_YN"))) )
			{
				//======================================================
		...
		try
		{
			...
			**********************************************************/
			...
			if("*".equals(stlAppearGp))
			{
				...
				{
					jrParam.setField("YD_CAR_SCH_ID",		commUtils.trim(loadCarsch.getRecord(0).getFieldString("YD_CAR_SCH_ID")));
	...
	public JDTORecord rcvDMYDR033(JDTORecord rcvMsg)
	{
		...
		try
		{
			...
			jrParam.setField("MODIFIER", modifier); //수정자 셋팅
//PIDEV			
//			String sApplyYnPI = commDao.ApplyYnPI("", "코일제품반품", "APPPI0", "*", "*");
			...
			String distGoodsGp  	= commUtils.trim(rcvMsg.getFieldString("DIST_GOODS_GP"));   //출하제품구분 H:열연코일, L:냉연코일
...
// PIDEV_SJH
//	    	String sApplyYnPIChk = commDao.ApplyYnPI("", methodNm, "APPPI0", "1", "*");
//			if ("Y".equals(sApplyYnPIChk)) {
//				commUtils.printLog(logId, "PI 적용여부가 Y이라서 수신처리 불가", "SL");
//				return jrRtn;
//			}
			...
			String szTRANS_FRTOMOVE_GP	= commUtils.trim(rcvMsg.getFieldString("TRANS_FRTOMOVE_GP"));	//운송이송구분
...
//					}
			...
			if (YfConstant.YD_GP_1.equals(szYD_GP) || YfConstant.YD_GP_3.equals(szYD_GP) || YfConstant.YD_GP_H.equals(szYD_GP) || YfConstant.YD_GP_J.equals(szYD_GP))
			{
				...
				{
					String CoilGp = commUtils.trim(rsResult.getRecord(i).getFieldString("COIL_GP"));
...
//					if("Y".equals(sApplyYnPI)) {
						//전송 Data 생성
						jrParam.setField("MQ_TC_CD", "M10YDLMJ1051");
...
//			String sApplyYnPI = commDao.ApplyYnPI("", methodNm, "APPPI0", "1", "*");
			...
			jrParam.setField("CAR_NO",			szCAR_NO);			//카드번호
...
//				if ("Y".equals(sApplyYnPI)) {
					...
					String szCOIL_GP = commUtils.trim(rsResult.getRecord(0).getFieldString("COIL_GP")); //코일구분
...
//PIDEV			
//			String sApplyYnPI = commDao.ApplyYnPI("", "코일이송상차대기장도착", "APPPI0", "*", "*");
...
//					}
					...
					//차량스케줄 생성 ////////////////////////////////////////////////////////////////////////
					//차량정보 존재여부 체크 /////////////////////////////////////////////////////////////////
					jrParam.setField("STL_NO",			commUtils.trim(rcvMsg.getFieldString("STL_NO1"))); 	//1번재료
					jrParam.setField("YD_CARPNT_CD",	CarldPntCd);
					JDTORecordSet jsCarPnt = commDao.select(jrParam, getYdCarPoint1, logId, methodNm, "TB_YD_CARPOINT 조회");
					...
					if(jsCarPnt.size() > 0)
					{
						...
						CarldPntCd	= commUtils.trim(jsCarPnt.getRecord(0).getFieldString("YD_CARPNT_CD"));
...
//			String sApplyYnPI = commDao.ApplyYnPI("", "코일이송상차도착PDA 수신", "APPPI0", "*", "*");
...
//			}
			...
			{
				commUtils.printLog(logId, methodNm + "DMYDR073전문...차량 : " + ydCarNo + "에 CARUD_PNT_CD(하차포인트) 값이 없습니다." , "SL");
				...
				CarudPntCd = commUtils.trim(jsYdCarPntCd.getRecord(0).getFieldString("YD_CARPNT_CD"));	//조회된 YD_CARPNT_CD
			}
...
//				}
			...
			if("9".equals(WorkGp))
			{
				...
				String bzGp = commUtils.trim(bzInfo.getRecord(0).getFieldString("BZ_GP")); //복포존운영체크
				...
				String ydCarpntCd	= commUtils.trim(jrCarPnt.getFieldString("YD_CARPNT_CD"));
...
//						}
	...
	public JDTORecord rcvDMYDR075(JDTORecord rcvMsg) throws DAOException
	{
		...
		try
		{
			...
			// PIDEV			
//			String sApplyYnPI = commDao.ApplyYnPI("", "코일이송하차완료PDA 수신 시작", "APPPI0", "*", "*");
			//수신 항목 값
			String ydGp			= commUtils.trim(rcvMsg.getFieldString("YD_GP"));
			String stlAppearGp	= commUtils.trim(rcvMsg.getFieldString("STL_APPEAR_GP"));   //재료외형
			String Stl_no 		= commUtils.trim(rcvMsg.getFieldString("STL_NO"));    		//재료번호
...
//			}
	...
	{
		String		methodNm	= "냉연소재이송지시[ACoilRevL3SeEJB.rcvCRYFRJ003] < " + rcvMsg.getResultMsg();
		String		logId		= rcvMsg.getResultCode();
	    JDTORecord	jrParam		= JDTORecordFactory.getInstance().create();	//Query 실행시 파라메터 전달용 JDTORecord
	    JDTORecord	jrRtn		= JDTORecordFactory.getInstance().create();	//method 종료시 리턴하는 JDTORecord
		...
		try
		{
			...
			commUtils.printLog(logId, "=============냉연소재이송지시 시작========", "SL");
			...
			szCARLD_SH				= commUtils.trim(rcvMsg.getFieldString("CARLD_SH"));			//상차매수
			szCOIL_MTL_OUT_SHP_TP	= commUtils.trim(rcvMsg.getFieldString("COIL_MTL_OUT_SHP_TP"));	//코일재료외형구분
			szCANCLE_YN				= commUtils.trim(rcvMsg.getFieldString("CANCLE_YN"));			//취소여부
			
			if("Y".equals(szCANCLE_YN))
			{
				//냉연소재이송지시 취소
				/*
				 * 냉연소재이송지시 취소는 차량이 도착 전에만 취소 할 수 있다.(차량이 도착 후에는 취소 불가)
				 * 1. 차량상태 확인하여 도착이후면 오류 
				 * 2. TB_YF_STOCK 초기화
				 */
				String svYD_CAR_PROG_STAT	= "0";
				String svYD_PNT_CT			= "0";
				
				jrParam.setField("TRN_EQP_CD",	szTRN_EQP_CD);
				rsResult = commDao.select(jrParam, getCarSchByTrnEqpCd, logId, methodNm, "운송설비코드로 차량스케쥴 조회");

				if(rsResult != null && rsResult.size() > 0)
				{
					svYD_CAR_PROG_STAT	= rsResult.getRecord(0).getFieldString("YD_CAR_PROG_STAT");	//야드차량진행상태(0:상차대기, 1:상차출발, 2:상차도착, 3:상차검수, 4:상차개시, 5:상차완료)
					svYD_PNT_CT			= rsResult.getRecord(0).getFieldString("YD_PNT_CT");		//TB_YD_CARPOINT에 TRN_EQP_CD로 차량이 점유하고 있는 포인트 확인
				}
				
				if(!"0".equals(svYD_PNT_CT))
				{
					//구내운송차량이 TB_YD_CARPOINT에 점유중인 포인트가 한건이라도 있으면 취소 불가 에러 발생!
					throw new Exception("야드차량(" + szTRN_EQP_CD + ")은 입동지시 상태입니다. 냉연소재이송지시 취소 불가!");
				}
								
				for(int i = 1; i <= Integer.parseInt(szCARLD_SH) ; i++)
				{
					szSTL_NO	= commUtils.trim(rcvMsg.getFieldString("STL_NO" + i));
					
					if(!"".equals(szSTL_NO))
					{
						//TB_YF_STOCK에 차량정보 착지개소코드 정보 초기화
						jrParam.setField("STL_NO",		szSTL_NO);
			    		jrParam.setField("TRN_EQP_CD",	"");
			    		jrParam.setField("WLOC_CD",		"");
			    		commDao.update(jrParam, updStockCrMTPinfo, logId, methodNm, "TB_YF_STOCK 수정");
					}
				}
			...
			else
			{
				//냉연소재이송지시
				/*
				 * 1. 소재이송지시에 상차매수가 0이면 오류 발생
				 * 2. 발지개소코드가 박판창고(D2Y45)가 아니면 오류 발생
				 * 3. 상차매수 만큼 FOR문을 돌며 TB_YF_STOCK에 차량정보, 착지개소코드를 저장
				 */
				if("0".equals(szCARLD_SH))
				{
					throw new Exception("소재이송지시 상차매수가 '0' 입니다!");
				}
				
				if(!YfConstant.WLOC_CD_A_HR_NO2_COIL_YARD.equals(szSPOS_WLOC_CD))
				{
					throw new Exception("발지개소코드가 박판창고가 아닙니다! 발지개소코드 : " + szSPOS_WLOC_CD);
				}
				...
				for(int i = 1; i <= Integer.parseInt(szCARLD_SH) ; i++)
				{
					...
					if(!"".equals(szSTL_NO))
					{
						//TB_YF_STOCK에 차량정보 착지개소코드 정보 등록
						//구내운송공차 출발(TSYFJ004), 포인트요구(TSYFJ002)호출시  차량정보로 조회하여 상차대상을 찾는다.
						jrParam.setField("STL_NO",		szSTL_NO);
			    		jrParam.setField("TRN_EQP_CD",	szTRN_EQP_CD);
				...
				jrParam.setField("TRN_EQP_CD",		szTRN_EQP_CD);
	    		jrParam.setField("SPOS_WLOC_CD",	szSPOS_WLOC_CD);
				rsResult = commDao.select(jrParam, getYdCarArrYn, logId, methodNm, "운송설비코드로 차량스케쥴 이전에 대기장 도착여부 조회");

				if(rsResult != null && rsResult.size() > 0)
				{
					//CRYFJ003(냉연소재이송지시)전에 TSYFJ005(소재차량 대기장도착(냉연))발생여부 확인
					sARR_YN = rsResult.getRecord(0).getFieldString("ARR_YN");
				}
				
				if("Y".equals(sARR_YN))
				{
					//TSYFJ005(소재차량 대기장도착(냉연)) 발생
					...
					jrMsg.setField("JMS_TC_CD",				"TSYFJ005");				//TSYFJ005(소재차량 대기장도착(냉연))
					jrMsg.setField("JMS_TC_CREATE_DDTT",	commUtils.getDateTime14());	//JMSTC생성일시
					jrMsg.setField("CR_PLNT_GP",			"");						//냉연공장구분(사용안함)
					jrMsg.setField("TRN_EQP_CD",			szTRN_EQP_CD);				//운송장비코드
					jrMsg.setField("ARR_WLOC_CD",			szSPOS_WLOC_CD);			//착지개소코드
					jrMsg.setField("ARR_YD_PNT_CD",			"");						//착지야드포인트코드(사용안함)
					jrMsg.setField("TRN_WRK_FULLVOID_GP",	"E");						//운송작업영공구분(F영차 E공차)
					jrMsg.setField("TRN_EQP_STK_CAPA",		"80000");					//운송장비적재능력...그냥 80000으로 셋팅함(사용안함)
					jrMsg.setField("CAR_ARR_DT",			commUtils.getDateTime14());	//차량도착일시
					jrMsg.setField("MGS_GP",				"");						//MGS_GP

					jrRtn = commUtils.addSndData(jrRtn, jrMsg);
				}
			}
			
			commUtils.printLog(logId, "=============냉연소재이송지시 종료========", "SL");
	...
	{
		String		methodNm		= "냉연제품이송정보(순천)[ACoilRevL3SeEJB.rcvCRYFRJ004] < " + rcvMsg.getResultMsg();
		String		logId			= rcvMsg.getResultCode();
	    JDTORecord	jrParam			= JDTORecordFactory.getInstance().create();	//Query 실행시 파라메터 전달용 JDTORecord
	    JDTORecord	jrRtn			= JDTORecordFactory.getInstance().create();	//method 종료시 리턴하는 JDTORecord
...
//	    	String sApplyYnPI = commDao.ApplyYnPI("", methodNm, "APPPI0", "*", "*");
			...
			commUtils.printLog(logId, "=============냉연제품이송정보(순천) 시작========", "SL");

			//수신 항목 값
			szCAR_NO				= commUtils.trim(rcvMsg.getFieldString("CAR_NO"));				//차량번호
			szCR_CAR_NO				= commUtils.trim(rcvMsg.getFieldString("CR_CAR_NO"));			//냉연차량번호
			szTRANS_ORD_DT			= commUtils.trim(rcvMsg.getFieldString("TRANS_ORD_DT"));		//운송지시일자
			szTRANS_ORD_SEQNO		= commUtils.trim(rcvMsg.getFieldString("TRANS_ORD_SEQNO"));		//운송지시순번
			szCARLD_SH				= commUtils.trim(rcvMsg.getFieldString("CARLD_SH"));			//상차매수
			szCOIL_MTL_OUT_SHP_TP	= commUtils.trim(rcvMsg.getFieldString("COIL_MTL_OUT_SHP_TP"));	//코일재료외형구분
			szCANCLE_YN				= commUtils.trim(rcvMsg.getFieldString("CANCLE_YN"));			//취소여부
			...
			if("Y".equals(szCANCLE_YN))
			{
				//냉연제품이송정보(순천) 취소
				/*
				 * 냉연제품이송정보(순천) 취소는 차량이 도착 전에만 취소 할 수 있다.(차량이 도착 후에는 취소 불가)
				 * 1. 차량상태 확인하여 하차도착이후 스케줄이 있으면 취소 불가
				 * 2. 차량스케줄에 발지를 순천공장으로 만들어서 반품화면에서 조회할수 없게 만듬
				 * 3. TB_YF_STOCK 삭제
				 */
				String			svYD_CAR_PROG_STAT	= "0";
				JDTORecordSet	rsResult	= null;
				...
				if(rsResult != null && rsResult.size() > 0)
				{
					...
					throw new Exception("야드차량 진행상태 값이 " + svYD_CAR_PROG_STAT + " 입니다! 냉연제품이송정보(순천) 취소 불가!");
				}
				
				//발지개소코드를 바꿔서 화면에서 조회되지 않도록 수정함
				jrParam.setField("SPOS_WLOC_CD",	"LA00");	//순천공장코드 : LA00
				jrParam.setField("CAR_NO",			szCAR_NO);
			...
			else
			{
		    	...
		    	//PIDEV				
//				if("N".equals(sApplyYnPI)) {				
//					jrParam.setField("CARD_NO",				szCR_CAR_NO);					//냉연차량 카드번호
//				}
				...
				jrParam.setField("SPOS_WLOC_CD",		"S0001");						//발지개소코드(상차지)-S0001(순천공장)
...
//				} else {
//					commDao.insert(jrParam, insCarSchUdS, logId, methodNm, "차량스케쥴 하차출발(A) INSERT ");
//				}
			...
			commUtils.printLog(logId, "=============냉연제품이송정보(순천) 종료========", "SL");
	...
	{
		String		methodNm		= "사외창고배차정보(냉연)[ACoilRevL3SeEJB.rcvDMYFRJ004] < " + rcvMsg.getResultMsg();
		String		logId			= rcvMsg.getResultCode();
	    JDTORecord	jrParam			= JDTORecordFactory.getInstance().create();	//Query 실행시 파라메터 전달용 JDTORecord
	    JDTORecord	jrRtn			= JDTORecordFactory.getInstance().create();	//method 종료시 리턴하는 JDTORecord
...
//	    	String sApplyYnPI = commDao.ApplyYnPI("", methodNm, "APPPI0", "*", "*");			
			...
			commUtils.printLog(logId, "=============사외창고배차정보(냉연) 시작========", "SL");
...
//				}
			...
			commUtils.printLog(logId, "=============사외창고배차정보(냉연) 종료========", "SL");
	...
	/**
	 * [A] 오퍼레이션명 : 코일 HFL결속장 작업 요구 정보를 수신(POYMJ004)
	 *
	 * @ejb.interface-method EJBDoclet을 생성하는 태그입니다.
	 * @param JDTORecord rcvMsg
	 * @return JDTORecord
	...
	{
		String		methodNm	= "코일HFL결속장 작업 요구 정보 정보를 수신[ACoilRcvL3SeEJB.rcvPOYMJ004] < " + rcvMsg.getResultMsg();
		String		logId		= rcvMsg.getResultCode();
	    JDTORecord	jrParam		= JDTORecordFactory.getInstance().create();	//Query 실행시 파라메터 전달용 JDTORecord
		...
		try
		{
			...
			commUtils.printLog(logId, "=============코일 HFL결속장 작업 요구 정보를 수신 시작========", "SL");
			...
			String L3_HMI	 	= commUtils.trim(rcvMsg.getFieldString("L3_HMI"));
			...
			if (YfConstant.PROCESS_ID_1.equals(procId))
			{
				 ...
				 * 보급처리
				 */
				commUtils.printLog(logId, methodNm + "HFL결속장 보급: " + CoilNo , "SL");

				if(WorkId.equals("D")) {       ///HFL 결속대

					/****  스케줄/작업예약 생성/ 스케쥴 호출  *****/
					jrRtn = this.callLineInOut(logId, methodNm, yardId, WorkId, procId, CoilNo, "1GHS01",modifier);
			...
			commUtils.printLog(logId, "=============코일 HFL결속장 작업 요구 정보를 수신 종료========", "SL");
	...
	{
		String		methodNm	= "HFL결속장 보급, 추출, Take-In[ACoilRevL3SeEJB.callLineInOut] < " + methodNms;
	    JDTORecord	jrParam		= JDTORecordFactory.getInstance().create();	//Query 실행시 파라메터 전달용 JDTORecord
		...
		try
		{
			...
			commUtils.printLog(logId, "=============HFL결속장 보급,추출,Take-In 시작========", "SL");
			...
			{
				if (YfConstant.WORK_HFL_S.equals(WorkID))	//HFL결속장
				{
					if (YfConstant.PROCESS_ID_1.equals(ProcessID))		//HFL결속장 보급
					{
						ydSchCd	= ydGp + ydBayGp + "HS01UM" ;
						toLocGuide  = ydGp + ydBayGp + "HS01" ;
					}
					else if (YfConstant.PROCESS_ID_3.equals(ProcessID))	//HFL결속장 추출
					{
						ydSchCd	= ydGp + ydBayGp + "HS01LM" ;
						ydStackLyrNo = "01";
					}
					else if (YfConstant.PROCESS_ID_5.equals(ProcessID))	//HFL결속장 TAKE IN처리
					{
						ydSchCd	= ydGp + ydBayGp + "HS02UM" ;
						toLocGuide = position;
				 	}
				} 
				...
				/***********************************************************************
				 * HFL 보급/추출 기동 중인 스케줄이 1건 이상 있으면 기동하지 않음
				 * 2025.07.10
				 ***********************************************************************/
				String ydCrnSchChk_1GHS 		= "Y";	//보급/추출 기동 중인 스케줄 체크 
				JDTORecordSet jrSchInfo_1GHS = commDao.select(jrParam, getSchCdInfo_1GHS, logId, methodNm, "HFL 기동 중인 보급/추출 스케줄 조회");
				String GHS_YN = jrSchInfo_1GHS.getRecord(0).getFieldString("GHS_YN"); // 0건 - Y, 그외 - N
				if ("1GHS01UM".equals(ydSchCd) || "1GHS01LM".equals(ydSchCd)) {
					if ( "N".equals(GHS_YN)) {
						ydCrnSchChk_1GHS = "N";
					}
				}
				commUtils.printLog(logId, "ydSchCd : "+ ydSchCd + ", GHS_YN : " + GHS_YN + ", ydCrnSchChk_1GHS : " + ydCrnSchChk_1GHS , "SL");

				//if("Y".equals(ydCrnSchChk))
				if("Y".equals(ydCrnSchChk) && "Y".equals(ydCrnSchChk_1GHS)) // 2025.07.10  보급/추출 기동 중인 스케줄 체크 기능 추가 
				{
					/**
					* 3. 크레인스케줄 전문 호출
					**********************************************************/
			...
			commUtils.printLog(logId, "=============HFL결속장 보급,추출,Take-In 종료========", "SL");
