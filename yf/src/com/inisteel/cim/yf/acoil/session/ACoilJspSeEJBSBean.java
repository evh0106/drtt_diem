 ...
 *
 * @ejb.bean name="ACoilJspSeEJB" jndi-name="ACoilJspSeEJB" type="Stateless"
...
*/
public class ACoilJspSeEJBSBean extends BaseSessionBean implements YfQueryIF, YfQueryIF2
{
	...
	public GridData getSelectData(GridData gdReq) throws DAOException{
		String methodNm = "조회[ACoilJspSeEJB.getSelectData(GridData)] < " + gdReq.getNavigateValue();
	...
	{
		String methodNm = "조회[ACoilJspSeEJB.getSelectData(JDTORecord)] < " + recPara.getResultMsg();
	...
	public JDTORecord updateGridData(GridData gdReq) throws DAOException{
		...
		try{
			...
			if( "".equals(sQueryId) ){
				throw new YFUserException("지정된 쿼리아이디가 없어 저장 할 수 없습니다.(개발자오류)");
			}
			
			if( "".equals(sFuncNm) ){
				throw new YFUserException("저장을 수행할 Method가 없어 저장 할 수 없습니다.(개발자오류)");
			}
	...
	{
		String methodNm = "크레인 스케줄취소[ACoilJspSeEJB.trtCrnSchCncl] < " + rcvMsg.getResultMsg();
	...
	{
		String methodNm = "작업예약 취소처리[ACoilJspSeEJB.trtWrkBookCncl] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
	 ...
	 * @throws DAOException
	*/
	public JDTORecord updStrAreaSrchSeqClear(GridData gdReq) throws DAOException {
		String methodNm = "저장영역별검색순서조회 저장전 Clear[ACoilJspSeEJB.updStrAreaSrchSeqClear] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
		try {
			commUtils.printLog(logId, methodNm, "S+", gdReq);
			...
			String sYD_SCH_CD = gdReq.getParam("YD_SCH_CD"  );
			String sYD_ROUTE_GP = gdReq.getParam("YD_ROUTE_GP");
			JDTORecord tmp = null;
			
			tmp = commUtils.getParam(logId, methodNm, commUtils.trim(gdReq.getParam("userid")));
			tmp.setField("YD_SCH_CD", sYD_SCH_CD);
			tmp.setField("YD_ROUTE_GP", sYD_ROUTE_GP);
			commDao.update(tmp, delSchLocSrch, logId, methodNm, "저장위치 삭제");
	 ...
	 * @throws DAOException
	*/
	public JDTORecord updStrAreaSrchSeqBatch(GridData gdReq) throws DAOException {
		String methodNm = "저장영역별검색순서조회 저장[ACoilJspSeEJB.updStrAreaSrchSeqBatch] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
		...
		try {
			...
			for (int i = 0; i < rowCnt; i++) {
				tmp = commUtils.getParam(logId, methodNm, commUtils.trim(gdReq.getParam("userid")));
				tmp.setField("YD_LOC_SRCH_RNG_SEQ", commUtils.getValue(gdReq, "YD_LOC_SRCH_RNG_SEQ", i));
				tmp.setField("YD_SCH_CD", sYD_SCH_CD);
				tmp.setField("YD_ROUTE_GP", sYD_ROUTE_GP);
				tmp.setField("YD_STK_COL_GP", commUtils.getValue(gdReq, "YD_STK_COL_GP", i));
				inRec[i] = commDao.conversionFieldname(tmp, 0);
			}
			
			if(rowCnt > 0){ 
				commDao.addBatch(inRec, updSchLocSrch);
			}
...
//						}
	...
	*/
	public JDTORecord updSchRuleMgt(GridData gdReq) throws DAOException {
		String methodNm = "크레인스케줄 기준 변경[ACoilJspSeEJB.updSchRuleMgt] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord updSchColor(GridData gdReq) throws DAOException {
		String methodNm = "스케줄 색상변경[ACoilJspSeEJB.updSchColor] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord updCraneChange(GridData gdReq) throws DAOException {
		String methodNm = "크레인작업요구현황조회 크레인변경[ACoilJspSeEJB.updCraneChange] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
...
//			    	throw new YFUserException("크레인스케쥴ID[" + ydCrnSchId + "]의 크레인스케줄 정보가 존재하지 않습니다.");
				...
				**********************************************************/
				...
				if ("W".equals(chgYdEqpStat)) {
					...
					EJBConnector sndConn1 = new EJBConnector("default", "ACoilRcvL2SeEJB", this);
	...
	*/
	public JDTORecord updPriorChange(GridData gdReq) throws DAOException {
		String methodNm = "크레인작업요구현황조회 순위변경[ACoilJspSeEJB.updPriorChange] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord updPriorWrkChange(GridData gdReq) throws DAOException {
		String methodNm = "크레인작업관리 긴급작업[ACoilJspSeEJB.updPriorWrkChange] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord updDownLocChange(GridData gdReq) throws DAOException {
		String methodNm = "크레인작업관리 권하위치변경[ACoilJspSeEJB.updDownLocChange] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord updCraneWrkCancel(GridData gdReq) throws DAOException {
		String methodNm = "크레인작업요구현황조회 작업취소[ACoilJspSeEJB.updCraneWrkCancel] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	 ...
	 *        
	 *       * 공통사항
	 *        1. ACoilSchSeEJB.procStockIdBaseCheckNew 모듈을 호출하여 권하위치 변경로직 재 확인
	 *        2. 권하위치 정합성
	...
	*/
	public JDTORecord updCrnSchDnWoLoc(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "크레인스케줄 권하지시위치 변경[ACoilJspSeEJB.updCrnSchDnWoLoc] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
...
//			String sAPP024 = ymComm.BCoilApplyYn("APP024","3","1");
			...
			if (!sSTL_NO.startsWith("S") && sYD_STK_COL_GP.matches("[1][A-H]\\d\\d\\d\\d")) {
				...
				EJBConnector ejbConn = new EJBConnector("default", "ACoilSchSeEJB", this);
				String isSuc = "";
//				if ("Y".equals(sAPP024)) {
...
//				}
	 ...
	 *     - TB_YF_WRKBOOK :: SCH_CNCL_YN 스케쥴취소 컬럼 업데이트
	 *     - TB_YF_SCHRULE :: YD_SCH_AUTO_ST_YN 스케쥴 자동기동 금지 상태변경(N)
	 *     - ACoilRcvL2SeEJB rcvF1YFL007 크레인작업지시 요구 아래의 조건을 만족해야함
	...
	public JDTORecord updCraneSchCancel(GridData gdReq) throws DAOException {
		String methodNm = "크레인작업요구현황조회 스케줄취소[ACoilJspSeEJB.updCraneSchCancel] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
			...
			for (int i = 0; i < rowCnt; i++) {
				...
				**********************************************************/
				...
				if ("RS".equals(sTRT_DTL_GP)) {
					...
					//크레인스케줄기동 전문
					EJBConnector sndConnD = new EJBConnector("default", "ACoilSchSeEJB", this);
	...
	*/
	public JDTORecord reSndCrnSch(GridData gdReq) throws DAOException {
		String methodNm = "크레인작업요구현황조회 스케줄재전송[ACoilJspSeEJB.reSndCrnSch] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord updbtCrnStsSetPp(GridData gdReq) throws DAOException {
		String methodNm = "크레인작업관리 상태 설정변경[ACoilJspSeEJB.updbtCrnStsSetPp] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	 ...
	 * 				1. 권하위치 찾기
	 *                - TO위치결정:사용자지정작업[ACoilSchSeEJB.procToLocUser]으로 대상위치 Get
	 * 
	 * 		</pre>
	 * @ejb.interface-method EJBDoclet을 생성하는 태그입니다.
	 * @param inDto
	...
	public GridData getDownLocChange(GridData gdReq) throws DAOException {
		String methodNm = "조회[ACoilJspSeEJB.getDownLocChange] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord procCoilYdGdsBayInWoSeqChangCoil(GridData gdReq) throws DAOException {
		String methodNm = "차량작업 포인트 현황-입동순서변경[ACoilJspSeEJBSBean.procCoilYdGdsBayInWoSeqChangCoil] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
		String szMsg = null;
	...
	{
		String methodNm					= "차량작업 포인트 현황-포인트 사용 등록[ACoilJspSeEJBSBean.procCoilYdGdsPntUnitCLCoil] < " + gdReq.getNavigateValue();
		String logId					= gdReq.getIPAddress();
		String szMsg					= null;
		...
		try
		{
			...
			{	
				...
				szTRN_EQP_CD			    = commUtils.getValue(gdReq, "TRN_EQP_CD", ii);			//순위]차량번호
				szTRN_EQP_CD_ORG			= commUtils.getValue(gdReq, "TRN_EQP_CD_ORG", ii);		//차량번호
				szYD_FRM_YN                 = commUtils.getValue(gdReq, "YD_FRM_YN", ii);			//차량형상유무
				szYD_CAR_USE_GP				= commUtils.getValue(gdReq, "YD_CAR_USE_GP", ii);		//구내/출하 구분
    			recOutTemp = JDTORecordFactory.getInstance().create();
    			jrParam.setField("YD_STK_COL_GP"		, szYD_STK_COL_GP);
    			// PIDEV PI_YD
    			jrParam.setField("PI_YD", commUtils.getValue(gdReq, "PI_YD", ii));
				...
				{	
					if(szTRN_EQP_CD.indexOf(szTRN_EQP_CD_ORG) > -1 && szTRN_EQP_CD_ORG.length()>0)
					{
						if(szYD_CAR_USE_GP.equals(YfConstant.YD_CAR_USE_GP_DM))
						{
							//출하이면
							szCAR_NO = szTRN_EQP_CD_ORG;
							szTRN_EQP_CD = "";
						}
						else
						{
							//구내운송이면
							szCAR_NO = "";
							szTRN_EQP_CD = szTRN_EQP_CD_ORG;
							
							if(szTRN_EQP_CD.length()> 8){
								throw new Exception("해당 차량장비번호 길이가 틀립니다.");
							}
						}
					}
					else
					{
						jrParam.setField("CAR_NO", szTRN_EQP_CD);
						jrParam.setField("TRN_EQP_CD", szTRN_EQP_CD);
						rsCarSch = commDao.select(jrParam, getYdCarschDaoByYdCarNoOrTrnEqpCd, logId, methodNm, "차량스케줄 조회 조회"); 
						
						if(rsCarSch.size() > 0)
						{
							if(YfConstant.YD_CAR_USE_GP_DM.equals(rsCarSch.getRecord(0).getFieldString("YD_CAR_USE_GP")))
							{
								//출하이면
								szYD_CAR_USE_GP = YfConstant.YD_CAR_USE_GP_DM;								
								
								if(szTRN_EQP_CD.indexOf(szTRN_EQP_CD_ORG) == -1 || szTRN_EQP_CD_ORG.length()==0)
								{
									szCAR_NO = szTRN_EQP_CD;
									szCARD_NO = szCAR_NO.substring(szCAR_NO.length()-4);
								}
							...
							{
								//구내운송이면
								szYD_CAR_USE_GP = YfConstant.YD_CAR_USE_GP_TS;
								
								szTRN_EQP_CD	= szTRN_EQP_CD;
								szCAR_NO 		= "";
								szCARD_NO 		= "";
							}
						}
						...
						{
							throw new Exception("해당 차량스케줄이 존재하지 않습니다.");
						}
					}
				}
				...
				if("C".equals(szYD_STK_COL_ACT_STAT_PARAM) || "L".equals(szYD_STK_COL_ACT_STAT_PARAM) || "R".equals(szYD_STK_COL_ACT_STAT_PARAM))
				{
					if ( "N".equals(szYD_STK_COL_ACT_STAT) )
					{			//사용불가
	...
	*/
	public JDTORecord changeCarLoc(GridData gdReq) throws DAOException {
		String methodNm = "차량입동위치변경[ACoilJspSeEJBSBean.changeCarLoc] < " + gdReq.getNavigateValue();
		JDTORecord 		jrRtn  		= null;
		...
		try {
			...
			if(jsCarSch.size()>0){
				...
				recInTemp.setField("CURR_YD_CARPNT_CD"	, jsCarSch.getRecord(0).getFieldString("YD_CARPNT_CD"));		//야드차량포인트코드_현재
				recInTemp.setField("NEXT_YD_CARPNT_CD"	, jsCarSch.getRecord(0).getFieldString("NEXT_YD_CARPNT_CD"));	//야드차량포인트코드_다음				
				recInTemp.setField("YD_CAR_SCH_ID"		, jsCarSch.getRecord(0).getFieldString("YD_CAR_SCH_ID"));	//차량스케줄ID
				recInTemp.setField("SCH_YN"				, jsCarSch.getRecord(0).getFieldString("SCH_YN"));	//스케줄차량 여부
				
				
				commDao.updateTx(jrParam, updCarStopLoc, logId, methodNm, "차량입동위치 수정");
				...
				schYn =jsCarSch.getRecord(0).getFieldString("SCH_YN");
				
				if("".equals(schYn)){
					schYn ="N";
				}						
				
				//스케줄 차량인 경우 에만 포인트 변경 시 물류진행으로 전문을 전송 한다.
				if("Y".equals(schYn)){
					jrRtn = commUtils.addSndData(jrRtn, commDao.getMsgL3("M10YDLMJ1141", recInTemp));
				} 
			}
	...
	*/
	public JDTORecord CarArrivalNEW(GridData gdReq) throws DAOException {
		String methodNm = "차량작업 포인트 현황-출하차량도착[ACoilJspSeEJBSBean.CarArrivalNEW] < " + gdReq.getNavigateValue();
		JDTORecord 		recInTemp  		= null;
		...
		try{
			...
			for (int ii = 0; ii < rowCnt; ii++) {				
				...
				if(loadCarSch.size() <= 0 ){
					...
					String errMsg = "";
					if(
							(YfConstant.CAR_BAY_TRANS_CARD_NO_9990.equals(inRecord2.getField("CAR_NO"))) || 
							(YfConstant.CAR_BAY_TRANS_CARD_NO_9991.equals(inRecord2.getField("CAR_NO"))) || 
							(YfConstant.CAR_BAY_TRANS_CARD_NO_9992.equals(inRecord2.getField("CAR_NO"))) || 
							(YfConstant.CAR_BAY_TRANS_CARD_NO_9993.equals(inRecord2.getField("CAR_NO"))) || 
							(YfConstant.CAR_BAY_TRANS_CARD_NO_9994.equals(inRecord2.getField("CAR_NO")))		
					
					){
						commUtils.printLog(logId, szMsg, "차량동간이적은 출발처리 제외");	
						
					}else{
						
						if( !commUtils.getValue(gdReq, "TRN_EQP_CD", ii).equals(inRecord2.getField("CAR_NO")) ){
							 
					    	if("".equals(inRecord2.getField("TRANS_ORD_DATE")))
					    	{
					    		errMsg = "운송지시일자[TRANS_ORD_DATE]가 없습니다.";
					    	}
	
					    	if("".equals(inRecord2.getField("TRANS_ORD_SEQNO")))
					    	{
					    		errMsg = "운송지시순번[TRANS_ORD_SEQNO]이 없습니다.";
					    	}
	
					    	if("".equals(inRecord2.getField("CAR_NO")))
					    	{
					    		errMsg = "차량번호[CAR_NO]가 없습니다.";
					    	}
	
					    	if("".equals(inRecord2.getField("SPOS_WLOC_CD")))
					    	{
					    	}			
	
					    	if("".equals(inRecord2.getField("SPOS_YD_PNT_CD")))
					    	{
					    		errMsg = "발지포인트코드[SPOS_YD_PNT_CD]가 없습니다.";
					    	}
							
							if(!"".equals(errMsg)){
								throw new YFUserException( "[" + inRecord2.getField("CAR_NO") + "]" + errMsg);
							}
							...
							commUtils.printLog(logId, szMsg, "구내운송의 출발처리는 [이송차량관리->이송차량백업처리]메뉴에서 진행하세요");
							throw new YFUserException("구내운송의 출발처리는 [이송차량관리->이송차량백업처리]메뉴에서 진행하세요");
						}
					}
				}				
			}
	...
	public JDTORecord getCoilCarMovYn(GridData gdReq) throws DAOException {
		...
		String methodNm			= "차량Point작업현황 - 제품이송우선순위 변경 [ACoilJspSeEJB.updCarStart]";
	...
	*/
	public JDTORecord updateCarExamination(JDTORecord jrecord) throws DAOException {
		String methodNm = "출고검수등록[ACoilJspSeEJB.updateCarExamination] < " + jrecord.getResultMsg();
...
//				if("Y".equals(sApplyYnPI)) {
					count=commDao.update(jrParam, updateCarExaminationGoodsDetjlNEW2_PIDEV, logId, methodNm, "박판열연검수완료처리");
//				} else {
...
//				}
				...
				////운송지시 단위 검수 완료 처리
//				if("Y".equals(sApplyYnPI)) {
					count=commDao.update(jrParam, updateCarExaminationGoodsDetjl2_PIDEV, logId, methodNm, "박판열연검수완료처리");
//				} else {
...
//	                tcRecordDM.setField("TRANS_ORD_SEQNO", sTRANS_ORD_SEQNO);
	...
	*/
	public JDTORecord modiYfRule(GridData gdReq) throws DAOException {
		String methodNm = "YF-RULE 수정[ACoilJspSeEJB.modiYfRule] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	public JDTORecord regCarUdWrk(GridData gdReq) throws DAOException {
		...
		String methodNm = "하차작업등록[ACoilJspSeEJB.regCarUdWrk]s < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	    String szMsg = "";
		...
		try {
			...
			String szTEL_NO			= StringHelper.evl(gdReq.getParam("TEL_NO"),"");
			String szDRIVER_NAME	= StringHelper.evl(gdReq.getParam("DRIVER_NAME"),"");
...
//			}
			...
			recInTemp.setField("DRIVER_NAME"		, szDRIVER_NAME);
			
			if("5".equals(szRETN_WK_GP)){
				recInTemp.setField("FRTOMOVE_PLANT_GP"	, "CR");				
			}else{
				recInTemp.setField("FRTOMOVE_PLANT_GP"	, "HR");		
			}
...
//			}
	 ...
	 * @throws DAOException
	 */	
	public JDTORecord regCarUdWrk5(GridData gdReq) throws DAOException
	{	
		String methodNm = "하차작업등록_순천이송[ACoilJspSeEJB.regCarUdWrk5] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	    String szMsg = "";
		...
		{	
			...
			//화면으로 부터 전달 받은 정보
			String szCAR_NO 		= gdReq.getParam("CAR_NO").toUpperCase();
			String szRETN_WK_GP 	= gdReq.getParam("RETN_WK_GP");  //1:반품, 2:회송, 3:부분하차 , 4:소재반품, 5:순천이송, 6:사외창고
			String szYD_CARPNT_CD 	= gdReq.getParam("YD_CARPNT_CD").toUpperCase();
			String szTEL_NO			= StringHelper.evl(gdReq.getParam("TEL_NO"),"");
			String szDRIVER_NAME	= StringHelper.evl(gdReq.getParam("DRIVER_NAME"),"");
			String szCARD_NO		= StringHelper.evl(gdReq.getParam("CARD_NO"),"").toUpperCase(); //차량번호의 뒤의 4자리를 사용한다.
...
//			String sApplyYnPI = commDao.ApplyYnPI("", methodNm, "APPPI0", "*", "*");			
			...
			recInTemp.setField("CAR_KIND"			, szCAR_KIND);							//차량종류
			recInTemp.setField("YD_EQP_WRK_STAT"	, "L");									//야드설비작업상태
			recInTemp.setField("SPOS_WLOC_CD"		, "S0001");								//발지개소코드(S00001:순천공장)
			recInTemp.setField("ARR_WLOC_CD"		, szWLOC_CD);							//착지개소코드
			recInTemp.setField("YD_CARUD_LEV_DT"	, commUtils.getDateTime14());			//하차출발일시
			recInTemp.setField("YD_PNT_CD3"			, szYD_PNT_CD);							//야드포인트코드3
			recInTemp.setField("YD_CARUD_STOP_LOC"	, szYD_STK_COL_GP);						//야드하차차정지위치
...
//			}
			...
			if("5".equals(szRETN_WK_GP) || "6".equals(szRETN_WK_GP))
			{
				recInTemp.setField("FRTOMOVE_PLANT_GP"	, "CR");
			}
			else
...
//			}
	 ...
	 * @throws DAOException
	 */	
	public JDTORecord regCarUdWrk6(GridData gdReq) throws DAOException
	{	
		String methodNm = "하차작업등록_사외창고이송[ACoilJspSeEJB.regCarUdWrk6] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	    String szMsg = "";
...
//			jrParam.setField("CR_FRTOMOVE_GP" 	, transFrtoMoveGp);
			...
			recInTemp.setField("CAR_KIND"			, szCAR_KIND);							//차량종류
			recInTemp.setField("YD_EQP_WRK_STAT"	, "L");									//야드설비작업상태
			recInTemp.setField("SPOS_WLOC_CD"		, "DZY30");								//발지개소코드(DZY30:사외창고)
			recInTemp.setField("ARR_WLOC_CD"		, szWLOC_CD);							//착지개소코드
			recInTemp.setField("YD_CARUD_LEV_DT"	, commUtils.getDateTime14());			//하차출발일시
			recInTemp.setField("YD_PNT_CD3"			, szYD_PNT_CD);							//야드포인트코드3
			recInTemp.setField("YD_CARUD_STOP_LOC"	, szYD_STK_COL_GP);						//야드하차차정지위치 
...
//			}
	...
	*/
	public JDTORecord regCarUdExplainInfo(GridData gdReq) throws DAOException {
		String methodNm = "차량예정정보 전송[ACoilJspSeEJB.regCarUdExplainInfo] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord procCrnWrkBookStart(GridData gdReq) throws DAOException {
		String methodNm = "크레인작업예약관리 스케줄기동[ACoilJspSeEJB.procCrnWrkBookStart] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
		...
		try {
			...
			//크레인스케줄기동 전문
			EJBConnector sndConn = new EJBConnector("default", "ACoilSchSeEJB", this);
	...
	*/
	public JDTORecord delWrkBook(GridData gdReq) throws DAOException {
		String methodNm = "작업예약관리-삭제[ACoilJspSeEJB.delWrkBook] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord updblMvStkWrkBook(GridData gdReq) throws DAOException {
		String methodNm = "이적작업예약등록[ACoilJspSeEJB.updblMvStkWrkBook] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
		...
		try {
			 ...
			 *  - RULE_BASE : 저장영역 OR 존별 검색순서 기준(사용안함) 
			 *  - USER_BASE : 화면에서 TO위치 지정
			 ******************************************************/
			//저장영역별 검색기준 
			if( "RULE_BASE".equals(sCHK_TO_LOC) ){
...
//			}   
	...
	*/
	public JDTORecord insMvstkWrkBook(JDTORecord jrParam, JDTORecordSet jsWbMtl) throws DAOException {
		String methodNm = "이적작업예약등록[ACoilJspSeEJB.insMvstkWrkBook] < " + jrParam.getResultMsg();
...
//			String sApplyYnPI = commDao.ApplyYnPI("", methodNm, "APPPI0", "*", "*");			
			...
			**********************************************************/
			for (int ii = 0; ii < lotCnt; ii++) {
		    	...
		    	//PIDEV				
//				if("Y".equals(sApplyYnPI)) {
					commDao.insert(jrParam, insWrkBook_PIDEV, logId, methodNm, "TB_YF_WRKBOOK");
//				} else {
...
//				}
	...
	*/
	public Vector setCrnSpecSpr(JDTORecord jrCrnSpec, JDTORecordSet jsWrkMtl) throws DAOException {
		String methodNm = "크레인사양분리[ACoilJspSeEJB.setCrnSpecSpr] < " + jrCrnSpec.getResultMsg();
	...
	*/
	public JDTORecord setCrnSchMsg(JDTORecordSet jsMsg, String logId, String mthdNm) throws DAOException {
		String methodNm = "크레인스케줄전문정리[ACoilJspSeEJB.setCrnSchMsg] < " + mthdNm;
	...
	*/
	...
	{
		String methodNm = "차량이적등록[ACoilJspSeEJB.updColUnitCarMvstkRegNew] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
...
//						}
				    	...
				    	//PIDEV				
//						if("Y".equals(sApplyYnPI)) {
							commDao.insert(jrParam, insWrkBook_PIDEV, logId, methodNm, "작업예약(TB_YF_WRKBOOK) 생성");
//						} else {
...
//						}
	...
	*/
	public JDTORecord updShearInspectMsg(GridData gdReq) throws DAOException{
		String methodNm = "코일공통상세조회-정정검사메시지 수정[ACoilJspSeEJB.updShearInspectMsg] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord updBedActStat(GridData gdReq) throws DAOException {
		String methodNm = "위치별 적치현황조회 - Bed상태 수정[ACoilJspSeEJB.updBedActStat] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord updFtmvWrkCancel(GridData gdReq) throws DAOException {
		String methodNm = "이송지시 취소[ACoilJspSeEJB.updFtmvWrkCancel] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord updFtmvWrkCancel2(GridData gdReq) throws DAOException {
		String methodNm = "이송지시 취소후 후처리작업[ACoilJspSeEJB.updFtmvWrkCancel2] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
...
//								|| "PT22UM".equals(ydSchCd.substring(2,8)) //제품이송상차(R)
	...
	*/
	public JDTORecord updUgntWrk(GridData gdReq) throws DAOException {
		String methodNm = "긴급작업[ACoilJspSeEJB.updUgntWrk] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	public JDTORecord reqCarldSchSt(GridData gdReq) throws DAOException {
		String methodNm = "상차대상순위별조회 - SCH기동[ACoilJspSeEJB.reqCarldSchSt] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
...
//				if("열연".equals(commUtils.getValue(gdReq, "FRTOMOVE_PLANT_GP", ii))){
//					rsResult = commDao.select(jrParam, getListSposYNchk3, logId, methodNm, " 운송장비 코드와 재료번호로 이송대상 차량스케줄 조회");					
//				}else{
					rsResult = commDao.select(jrParam, getTrnEqpCdSch, logId, methodNm, " 운송장비 코드로 이송대상 차량스케줄 조회");										
//				}
...
//					}
	...
	*/
	public JDTORecord mkUdCarSch(GridData gdReq) throws DAOException {
		String methodNm = "하차백업생성[ACoilJspSeEJB.mkUdCarSch] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
		...
		try {
			...
			jrParam.setField("FRTOMOVE_PLANT_GP"	, gdReq.getParam("FRTOMOVE_PLANT_GP")		); //운송장비코드
	...
	*/
	public JDTORecord trtMvCarStatSet2(GridData gdReq) throws DAOException {
		String methodNm = "(열연)이송차량 실적처리 팝업 - 등록 [ACoilJspSeEJB.trtMvCarStatSet2] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
		...
		try {
			...
			String sWLOC_CD 			= commUtils.trim(gdReq.getParam("WLOC_CD"));
			String sYD_PNT_CD 			= null;
			String sYD_BAY_GP			= null;
			...
			if ("TSYDJ003".equals(sJMS_TC_CD)) { //소재차량도착
				...
				EJBConnector sndConn = new EJBConnector("default", "YfCommCarMvSeEJB", this);
				...
				EJBConnector sndConn = new EJBConnector("default", "CarMvHdFaEJB", this);
				jrRtn = (JDTORecord)sndConn.trx("rcvMatlCarLev", new Class[] { JDTORecord.class }, new Object[] { jrYdMsg });
				...
				if(jsCol != null && jsCol.size() > 0) {
					...
					sYD_BAY_GP	= commUtils.trim(jsCol.getRecord(0).getFieldString("YD_BAY_GP"));
				...
				commDao.updateTx(jrParam, updPlnInfoReSet, logId, methodNm, "TB_YD_CARPOINT 차량 포인트 예약으로 잡혀있는정보 Clear ");
				...
				jrYdMsg.setField("YD_BAY_GP"     		, sYD_BAY_GP 	); 
				...
				jrParam.setField("WLOC_CD",			sWLOC_CD);
				...
				ejbConn1.trx("YfCarPointinforeg", new Class[]{String.class,String.class,String.class,String.class,String.class,String.class,String.class,String.class,String.class},
			  	             new Object[]{"4", "", sTRN_EQP_CD, "", sWLOC_CD, sYD_PNT_CD, "L", logId, methodNm});
			}else if("TSYDJ005".equals(sJMS_TC_CD)) {
				jrYdMsg.setField("JMS_TC_CD"         	, sJMS_TC_CD	); //"TSYFJ005"
				jrYdMsg.setField("JMS_TC_CREATE_DDTT"	, currDate  	); //JMSTC생성일시				
				jrYdMsg.setField("TRN_EQP_CD"     	 	, sTRN_EQP_CD		);
				jrYdMsg.setField("ARR_WLOC_CD"     		, sWLOC_CD	); 
				jrYdMsg.setField("TRN_WRK_FULLVOID_GP"  , sTRN_WRK_FULLVOID_GP);
				jrYdMsg.setField("TRN_EQP_STK_CAPA"     , YfConstant.TRN_EQP_STK_CAPA ); 
				jrYdMsg.setField("CAR_ARR_DT"  			, currDate    		);
				jrYdMsg.setField("MGS_GP"     			, "I" 		); 
				
				EJBConnector sndConn = new EJBConnector("default", "YfCommCarMvSeEJB", this);
				jrRtn = (JDTORecord)sndConn.trx("rcvTSYDJ005", new Class[] { JDTORecord.class }, new Object[] { jrYdMsg });
			}
	 ...
	 *      @throws DAOException
	*/
	public JDTORecord trtMvCarStatSet3(GridData gdReq) throws DAOException {
		String methodNm = "(냉연)이송차량 실적처리 팝업 - 등록 [ACoilJspSeEJB.trtMvCarStatSet3] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
		...
		try {
			...
			String sYD_CARUD_STOP_LOC 	= commUtils.trim(gdReq.getParam("YD_CARUD_STOP_LOC"));	
			String sTO_LOC 				= commUtils.trim(gdReq.getParam("TO_LOC"));
			String sCOIL_MTL_OUT_SHP_TP = commUtils.trim(gdReq.getParam("COIL_MTL_OUT_SHP_TP")); //Z:제품 , Y:제공품
			String sCARLD_SH			= commUtils.nvl(commUtils.trim(gdReq.getParam("CARLD_SH")), "0");	//상차매수(영차시 사용)
			String sCoilNo 				= "";
			String sWLOC_CD 			= commUtils.trim(gdReq.getParam("WLOC_CD"));
			String sYD_PNT_CD 			= null;
			String sYD_BAY_GP			= null;
			...
			if ("TSYFJ003".equals(sJMS_TC_CD)) { //소재차량도착
				...
				EJBConnector sndConn = new EJBConnector("default", "YfCommCarMvSeEJB", this);
				jrRtn = (JDTORecord)sndConn.trx("rcvTSYFJ003", new Class[] { JDTORecord.class }, new Object[] { jrYdMsg });
				
			} else if("TSYFJ004".equals(sJMS_TC_CD)) { //소재차량출발
				
				jrYdMsg.setField("TRN_EQP_CD"   	  		, sTRN_EQP_CD);
				jrYdMsg.setField("TRN_WRK_FULLVOID_GP"		, sTRN_WRK_FULLVOID_GP);
				jrYdMsg.setField("COIL_MTL_OUT_SHP_TP"		, sCOIL_MTL_OUT_SHP_TP);
				jrYdMsg.setField("TRN_EQP_STK_CAPA"		    , YfConstant.TRN_EQP_STK_CAPA);
				jrYdMsg.setField("CARLD_SH"		    		, sCARLD_SH);
				...
				for (int ii = 0; ii < rowCnt; ii++) {
					...
					if(!"".equals(sCoilNo)){
						...
						//이송작업재료등록
						jrYdMsg.setField("STL_NO"+rowNo			, sCoilNo); 
						jrYdMsg.setField("GDS_CARLD_LOC"+rowNo	, commUtils.getValue(gdReq, "YD_STK_BED_NO", ii)); 
					}
				}
				
				EJBConnector sndConn = new EJBConnector("default", "YfCommCarMvSeEJB", this);
				jrRtn = (JDTORecord)sndConn.trx("rcvTSYFJ004", new Class[] { JDTORecord.class }, new Object[] { jrYdMsg });

			} else if("YFTSJ007".equals(sJMS_TC_CD)) { //소재차량상차개시
			...
			} else if("YFTSJ008".equals(sJMS_TC_CD)) { //소재차량상차완료
			...
			} else if("YFTSJ009".equals(sJMS_TC_CD)) { //소재차량하차개시
			...
			} else if("YFTSJ010".equals(sJMS_TC_CD)) { //소재차량하차완료
			...
			} else if("YFTSJ011".equals(sJMS_TC_CD)) { //소재차량Point지시
				...
				//차량스케줄의 개소코드, 야드포인트, 정지위치를 UDPATE 한다.
				if("E".equals(sTRN_WRK_FULLVOID_GP)) { //공차:상차
					sYD_CAR_PROG_STAT	= YfConstant.YD_CAR_PROG_STAT_2;
					sSPOS_WLOC_CD 		= sWLOC_CD;
					sYD_PNT_CD1			= sYD_PNT_CD;	
					sYD_CARLD_STOP_LOC 	= sTO_LOC;
					sYD_CARLD_PNT_WO_DT = currDate;
				} else { //영차:하차
					sYD_CAR_PROG_STAT	= YfConstant.YD_CAR_PROG_STAT_B;
					sARR_WLOC_CD 		= sWLOC_CD;
					sYD_PNT_CD3			= sYD_PNT_CD;	
					sYD_CARUD_STOP_LOC 	= sTO_LOC;
					sYD_CARUD_PNT_WO_DT = currDate;
				...
				//이송차량스케줄 수정 
				jrParam.setField("YD_CAR_PROG_STAT"		, sYD_CAR_PROG_STAT);  //""이면 이전 상태 유지된다.
				jrParam.setField("SPOS_WLOC_CD"			, sSPOS_WLOC_CD);
				jrParam.setField("YD_CARLD_PNT_WO_DT"	, sYD_CARLD_PNT_WO_DT);
				jrParam.setField("YD_PNT_CD1"			, sYD_PNT_CD1);
				jrParam.setField("YD_CARLD_STOP_LOC"	, sYD_CARLD_STOP_LOC); 
			...
			}else if("TSYFJ005".equals(sJMS_TC_CD)) {
				jrYdMsg.setField("JMS_TC_CD"         	, sJMS_TC_CD	); //"TSYFJ005"
				jrYdMsg.setField("JMS_TC_CREATE_DDTT"	, currDate  	); //JMSTC생성일시
				
				String sCR_PLNT_GP = "0000";
				
				if("DXY99".equals(sSPOS_WLOC_CD)){
					sCR_PLNT_GP = "2000";
				}else if("DYY99".equals(sSPOS_WLOC_CD)){
					sCR_PLNT_GP = "8000";
				}
				
				jrYdMsg.setField("CR_PLNT_GP"        	, sCR_PLNT_GP	); 
				jrYdMsg.setField("TRN_EQP_CD"     	 	, sTRN_EQP_CD		);
				jrYdMsg.setField("ARR_WLOC_CD"     		, sWLOC_CD	); 
				jrYdMsg.setField("TRN_WRK_FULLVOID_GP"  , sTRN_WRK_FULLVOID_GP);
				jrYdMsg.setField("TRN_EQP_STK_CAPA"     , YfConstant.TRN_EQP_STK_CAPA ); 
				...
				EJBConnector sndConn = new EJBConnector("default", "YfCommCarMvSeEJB", this);
				jrRtn = (JDTORecord)sndConn.trx("rcvTSYFJ005", new Class[] { JDTORecord.class }, new Object[] { jrYdMsg });
			}
	...
	*/
	public JDTORecord updCarFtMvMtl(GridData gdReq) throws DAOException {
		String methodNm = "이송작업재료등록[ACoilJspSeEJB.updCarFtMvMtl] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
		...
		try {
			...
			for (int ii = 0; ii < rowCnt; ii++) {
				...
				if(!"".equals(sCoilNo)){
					...
					commDao.insert(jrParam, insCarFtMvMtl, logId, methodNm, "이송작업재료등록");					
				}
			}
	...
	*/
	public JDTORecord delCarFtMvMtl(GridData gdReq) throws DAOException {
		String methodNm = "이송작업재료삭제[ACoilJspSeEJB.delCarFtMvMtl] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord chgCarFtMvMtl(GridData gdReq) throws DAOException {
		String methodNm = "이송작업재료위치변경[ACoilJspSeEJB.chgCarFtMvMtl] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	public JDTORecord insStockInfo(JDTORecord rcvMsg) throws DAOException {
		...
		String methodNm = "산적위치수정-저장품생성[ACoilJspSeEJB.insStockInfo] <" + rcvMsg.getResultMsg();
		String logId 	= rcvMsg.getResultCode();
		...
		try {
			...
			//ora에러 에서 입력값 누락 현상 발견(필수 값 체크)2024.08.26
			if("".equals(sSTL_NO)){
				throw new YFUserException("산적위치 수정 시 코일번호가 존재 안합니다.");
			} 
	...
	public JDTORecord updStkLoc(GridData gdReq) throws DAOException {
		...
		String methodNm = "산적위치수정-수정[ACoilJspSeEJB.updStkLoc] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
...
//				트랜지션 분리
				EJBConnector ejbConnS = new EJBConnector("default", "ACoilJspSeEJB", this);
			 ...
			 * (저장품이 사라졌을 경우 생성후 종료)
			 *  - 2020.11.21 동일위치일때 skip하는 로직은 제외함
			 * 
			 **************************************************/
...
//			} 
			...
			if (jsLyrInfo.size() == 0) {
				throw new YFUserException("산적위치 이상 [" + sYD_YD_STK_COL_GP + sYD_BED_GP + sYD_LYR_GP + "]");
			}
...
//				if ("Y".equals(sApplyYnPI)) {
					JDTORecord tcRecord2 = JDTORecordFactory.getInstance().create();
					tcRecord2.setField("COIL_NO", sSTL_NO);
					JDTORecord stlRecord2 = commDao.select(tcRecord2, getCOILCOMM, logId, methodNm, "코일공통조회").getRecord(0);
					String CoilGp = StringHelper.evl(stlRecord2.getFieldString("COIL_GP"), "");

					tcRecordDM.setField("MQ_TC_CD", "M10YDLMJ1011");
					tcRecordDM.setField("MQ_TC_CREATE_DDTT", commUtils.getDateTime14());
					tcRecordDM.setField("YD_GP", sYardGp);
					tcRecordDM.setField("DIST_GOODS_GP", "HR".equals(CoilGp) ? "H" : "CR".equals(CoilGp) ? "L" : "");
...
//				}
	...
	public JDTORecord changeCoilLocationInfo(JDTORecord jparam) throws Exception {
		...
		String methodNm = "산적위치수정[ACoilJspSeEJB.changeCoilLocationInfo] < " + jparam.getResultMsg();
...
//			String sApplyYnPI = commDao.ApplyYnPI("", methodNm, "APPPI0", "*", "*");
			...
			if (stockV.size() > 0) {
				...
				sCOIL_GP	 = StringHelper.evl(stockV.getRecord(0).getFieldString("COIL_GP"), "");
			} else {
...
//					
			...
			if (jsPutLocInfo.size() > 0){
				...
				sToCoilGp	= commUtils.nvl(jsPutLocInfo.getRecord(0).getFieldString("COIL_GP"), "");
				sYdLrGp 	= commUtils.nvl(jsPutLocInfo.getRecord(0).getFieldString("YD_LR_GP"), "");
			}
			
			/************************************
			 * 4.To 위치에 존재하는 코일제품이적작업실적 전문 송신 
			 * 5.TO 위치에 존재하는 기존 저장품 이력 저장 
			 *************************************/
...
//					if ("Y".equals(sApplyYnPI)) {
						...
						tcRecordDM.setField("MOVENSTACK_DATE", commUtils.getDate8());
						tcRecordDM.setField("MOVENSTACK_TIME", commUtils.getTime6());
//					} else {
//						tcRecordDM.setField("JMS_TC_CD", "YDDMR004");
//						tcRecordDM.setField("TC_CODE", "YDDMR004");
//						tcRecordDM.setField("MOVENSTACK_DATE", commUtils.getDate8());
//						tcRecordDM.setField("MOVENSTACK_TIME", commUtils.getTime6());
//						tcRecordDM.setField("JMS_TC_CREATE_DDTT", commUtils.getDateTime14());
//						tcRecordDM.setField("GOODS_NO", sToStlNo);
...
//					}
	    	 	...
	    	 	}else if("CR".equals(sToCoilGp)){
	    	 		commDao.update(jRecord, updYfCrCoilCommLodLoc, logId, methodNm, "TB_YF_CR_COILCOMM 냉연코일 위치수정");
	    	 		
	    	 		commUtils.printLog(logId, "[YFCRJ002] 코일제품이적작업실적 전문 전송", "[INFO]");
	    	 		
	    	 		JDTORecord tcRecordDM = JDTORecordFactory.getInstance().create(); 
					tcRecordDM.setField("JMS_TC_CD"         , "YFCRJ002");
					tcRecordDM.setField("JMS_TC_CREATE_DDTT", commUtils.getDateTime14());
					tcRecordDM.setField("COIL_NO"          	, sToStlNo);
					tcRecordDM.setField("YD_GP"          	, sTempLayer.substring(0,1));
					tcRecordDM.setField("BAY_GP"          	, sTempLayer.substring(1,2));
					tcRecordDM.setField("SECT_GP"          	, sTempLayer.substring(2,4));
					tcRecordDM.setField("COL_GP"          	, sTempLayer.substring(4,6));
					tcRecordDM.setField("TO_STORE_LOC"      , sTempLayer);
					tcRecordDM.setField("YD_LR_GP"    		, sYdLrGp);
			 ...
			 *******************************************/
    	 	...
    	 	if("HR".equals(sCOIL_GP)){
    	 		EJBConnector ejbConnTO = new EJBConnector("default", "ACoilRcvL2SeEJB", this);
    	 	...
    	 	}else if("CR".equals(sCOIL_GP)){
    	 		commDao.update(jRecordTO, updYfCrCoilCommLodLoc, logId, methodNm, "TB_YF_CR_COILCOMM 냉연코일 위치수정");
    	 	}
...
//				if ("Y".equals(sApplyYnPI)) {
					...
					tcParam.setField("MQ_TC_CD", "M10YDLMJ1031");
					tcParam.setField("MQ_TC_CREATE_DDTT", commUtils.getCurDate("yyyyMMddHHmmss"));
					tcParam.setField("YD_GP", "1");
					tcParam.setField("DIST_GOODS_GP", "H");
					tcParam.setField("YARD_GP", "");
					tcParam.setField("GOODS_NO", sToStlNo);
					tcParam.setField("STORE_LOC_CD_FROM", sPutStackColGp + sPutStackBedGp + sPutStackLayerGp);
					tcParam.setField("STORE_LOC_CD_TO", sTempLayer);
					tcParam.setField("MOVENSTACK_DATE", commUtils.getDate8());
					tcParam.setField("MOVENSTACK_TIME", commUtils.getTime6());

//				} else {
//					tcParam.setField("JMS_TC_CD", "YDDMR004");
//					tcParam.setField("TC_CODE", "YDDMR004");
//					tcParam.setField("MOVENSTACK_DATE", commUtils.getDate8());
//					tcParam.setField("MOVENSTACK_TIME", commUtils.getTime6());
//
...
//				}
				...
				jrRtn = commUtils.addSndData(jrRtn, tcParam);		
    	 		...
    	 		JDTORecord tcParam = JDTORecordFactory.getInstance().create();
    	 		tcParam.setField("STL_NO"          , sSTL_NO);
    	 		
    	 		commUtils.printLog(logId, "[YFCRJ002] 코일제품이적작업실적 전문 전송", "[INFO]");
				jrRtn = commUtils.addSndData(jrRtn, commDao.getMsgL3("YFCRJ002" , tcParam));
    	 	}
	...
	*/
	public JDTORecord delStkLoc(GridData gdReq) throws DAOException {
		String methodNm = "산적위치수정-삭제[ACoilJspSeEJB.delStkLoc] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	public JDTORecord updStkLocBackUp(GridData gdReq) throws DAOException {
		...
		try {
			...
			if ("STOCK".equals(sSND_FLAG)) {
				...
				// 있다 -> 저장품 삭제 여부 확인하여 del='N'처리
				else 
				{
					// 저장품에서 삭제되있다면 다시 살려놓는다.
					if ("Y".equals(rst.getRecord(0).getFieldString("DEL_YN"))) {
	    				jrParam.setField("DEL_YN"  , "N");
						jrParam.setField("STL_NO", sSTL_NO);
						commDao.update(jrParam, updateStockDelYnInfo, logId, methodNm, "TB_YF_STOCK 저장품삭제");
					} else {
						throw new YFUserException("이미 저장품 정보가 존재합니다.");
					}
				}
	...
	public JDTORecord carLiftPosSet(GridData gdReq) throws DAOException {
		String methodNm = "상차위치변경 [ACoilJspSeEJB.carLiftPosSet] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
...
//				commDao.update(jrParam, updateCarMapLayer, logId, methodNm, "TB_YF_STKLYR 적치단 상태변경 C, 재료번호 등록");
	...
	*/
	public JDTORecord updAdvSchRuleMgt(GridData gdReq) throws DAOException {
		String methodNm = "크레인스케줄 고도화기준 변경[ACoilJspSeEJB.updAdvSchRuleMgt] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
...
//					}
	...
	public GridData getTableData(GridData gdReq) throws DAOException{
		String methodNm = "테이블Data조회[ACoilJspSeEJB.getTableData(GridData)] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	public GridData updTableData(GridData gdReq) throws DAOException{
		String methodNm = "야드기준 [ACoilJspSeEJB.updTableData(GridData)] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
...
//			commUtils.printLog(logId, methodNm, "변환된 쿼리 :: \n"+ sMergeQuery, gdReq);
	 ...
	 * @throws Exception
	 */
	private JDTORecord getQueryParam(String sQuery) throws Exception {
		...
		if(cutLen > -1 ){ 
			...
			for( int i=0; i<len; i++){
//				char currentChar = sScript.charAt(i);
				
//				if(    
//						( ( currentChar == '=' 
//							|| ( (i+3) < len 
//							&& currentChar == 'L' 
//							&& sScript.charAt(i+1) == 'I' 
//							&& sScript.charAt(i+2) == 'K' 
//							&& sScript.charAt(i+3) == 'E' )) && "U".equals(sQTyp))	
//						
//						|| (currentChar == '(' && "I".equals(sQTyp))
//				){
					startPoint = false;
...
//					oP[i] = sKeyVal.replaceAll("\'","");
	...
	public GridData executeQuery(GridData gdReq) throws DAOException{
		String methodNm = "야드기준 실행[ACoilJspSeEJB.executeQuery(GridData)] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
...
//				commUtils.printLog(logId, methodNm, "변환된 쿼리 :: \n"+ callScript, gdReq);
	...
	*/
	public JDTORecord insEqpTrblReg(GridData gdReq) throws DAOException {
		String methodNm = "야드설비정비등록[ACoilJspSeEJB.insEqpTrblReg] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord updYfRule(GridData gdReq) throws DAOException {
		String methodNm = "기준관리 - 세부항목수정[ACoilJspSeEJB.updYfRule] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	*/
	public JDTORecord updEqpOprnStat(GridData gdReq) throws DAOException {
		String methodNm = "설비상태 변경[ACoilJspSeEJB.updEqpOprnStat] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	public JDTORecord updYdStkColActiveStat(GridData gdReq) throws DAOException {
		String methodNm = "적치단 활성상태 수정[ACoilJspSeEJB.updYdStkColActiveStat] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	 ...
	 * @throws DAOException
	 */
	public JDTORecord updYdStkColZoneGp(GridData gdReq) throws DAOException {
		String methodNm = "적치단 활성상태 수정[ACoilJspSeEJB.updYdStkColZoneGp] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
		...
		try {
			...
			jrParam.setField("YD_STK_COL_GP"	,commUtils.trim(gdReq.getParam("YD_STK_COL_GP")));
			jrParam.setField("YD_ZONE_GP"	    ,commUtils.trim(gdReq.getParam("YD_ZONE_GP")));

			//적치단 UPDATE
			commDao.update(jrParam, updYfStkColZoneGp, logId, methodNm, "적치단의 ZONE UPDATE");
	...
	public JDTORecord updCoilYdStkPosSet(GridData gdReq) throws DAOException {
		String methodNm = "야드및설비 열정보수정[ACoilJspSeEJB.updCoilYdStkPosSet] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
		...
		try {
			...
			inRecord.setField("YD_ZONE_GP"	                ,commUtils.getValue(gdReq, "YD_ZONE_GP", 0));
	...
	public JDTORecord updCoilYdStkPosSetBed(GridData gdReq) throws DAOException {
		String methodNm = "야드및설비 베드정보수정[ACoilJspSeEJB.updCoilYdStkPosSetBed] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
	...
	public JDTORecord procAllCarPntYnReg(GridData gdReq) throws DAOException {
		String mthdNm = "전체입동제한[ACoilJspSeEJB.procAllCarPntYnReg] < " + gdReq.getNavigateValue();
		String logId  = gdReq.getIPAddress();
	 ...
	 * @throws DAOException
	*/
	public JDTORecord updPurposeSpanList(GridData gdReq) throws DAOException {
		String methodNm = "저장위치 용도관리 - 저장 [ACoilJspSeEJB.updPurposeSpanList] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
		...
		try {
			...
			String useYN = "";
			String span = "";
			span = gdReq.getParam("SPAN_CNT");
			int spanCnt = Integer.parseInt(span);
			
			int rowCnt = gdReq.getHeader("BAY_GP").getRowCount();
			for(int i=0; i<rowCnt;i++){
				for(int j=1 ; j <=spanCnt ; j++){
					if(j<10) sSpanNo = "0" + j;
					else sSpanNo = "" + j;
					
					purposeCode = commUtils.getValue(gdReq, "SP" + sSpanNo, i);//용도코드(CR,HR)
					
					if(!purposeCode.equals(gdReq.getHeader("SP" + sSpanNo).getHiddenValue(i)) ){
						item = YfConstant.YD_GP_1 + commUtils.getValue(gdReq, "BAY_GP", i) + sSpanNo;
						
						jrParam.setField( "REPR_CD_GP" 			, "YF0010");
						jrParam.setField( "CD_GP" 				, YfConstant.YD_GP_1);
						jrParam.setField( "ITEM" 				, item);
						jrParam.setField( "DTL_ITEM1" 			, purposeCode);
						jrParam.setField( "DTL_ITEM2" 			, "");
						jrParam.setField( "DTL_ITEM3" 			, "");
						
						commDao.update(jrParam, setCTSAreaBaySeq  , logId, methodNm, "저장위치 용도관리 업데이트");
	...
	*/
	public JDTORecord updStrZoneList(GridData gdReq) throws DAOException {
		String methodNm = "저장영역별검색순서조회 저장[ACoilJspSeEJB.updStrZoneList] < " + gdReq.getNavigateValue();
		String logId = gdReq.getIPAddress();
		
		JDTORecord inRec[] = null;
		List batchData = new java.util.ArrayList();
		...
		try {
			...
			JDTORecord tmp = null;
			GridHeader[] gridHeader = gdReq.getHeaders();
			int colCnt = gridHeader.length;
			String sColumnNm = "";
			String sBayGp = "";
			String sColGp = "";
			String sSpan = "";
			String sYdStkColGp = "";
			String sYdGp = commUtils.nvl(gdReq.getParam("YD_GP"),YfConstant.YD_GP_1);
			String sZoneGp = ""; 
			for (int i = 0; i < rowCnt; i++) {		
				
				sBayGp = commUtils.getValue(gdReq, "BAY_GP", i);
				sColGp = commUtils.getValue(gdReq, "COL_GP", i);
				
				
				for(int j=0; j<colCnt; j++){
					sColumnNm = gridHeader[j].getID();
					if(sColumnNm.indexOf("SP") > -1){
						  
						sZoneGp = commUtils.getValue(gdReq, sColumnNm, i);
						if(!sZoneGp.equals(gdReq.getHeader(sColumnNm).getHiddenValue(i))){
							
							sSpan = sColumnNm.replaceAll("SP", "").trim(); 
							sYdStkColGp = sYdGp + sBayGp + sSpan + sColGp;
							
							tmp = commUtils.getParam(logId, methodNm, commUtils.trim(gdReq.getParam("userid")));
							tmp.setField("YD_LOC_SRCH_RNG_SEQ", "0"); 
							tmp.setField("YD_STK_COL_GP", sYdStkColGp);
							tmp.setField("YD_ZONE_GP", sZoneGp);

							// 비활성화면 삭제로
//							if("N".equals(sZoneGp)){
//								tmp.setField("DEL_YN", "N");
//							}else{
//								tmp.setField("DEL_YN", "Y");
//							}
							// local에서 테스트 할때만
							batchData.add(commDao.conversionFieldname(tmp, 0));
//							batchData.add(tmp);
						}
					}
				}				
			...
			if(batchData != null&& batchData.size() >0){
				int updateSize = batchData.size();
				inRec = new JDTORecord[updateSize];
				for(int i=0; i<batchData.size();i++){
					inRec[i] = (JDTORecord)batchData.get(i);
					// local에서 테스트 할때만
//					commDao.update( (JDTORecord)batchData.get(i), updSchLocSrchZone);
				}
				
				
				commDao.addBatch(inRec, updSchLocSrchZone);
			}
