 ...
 * @ejb.bean name="M10LmExYs41RecieveFaEJB" jndi-name="M10LmExYs41RecieveFaEJB" type="Stateless"
...
*/
public class M10LmExYs41RecieveFaEJBBean extends BaseSessionBean{
	...
	private YsCommUtils piYsUtils = new YsCommUtils();       
...
//		} 
		try {//HS(조업서버), TM(물류서버), PM(진행서버)중 선택
			...
			piYsUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd41RecieveFaEJB(특수강야드수신) ejbCreate IOException error :" + e.getMessage());
		}
		catch (TimeoutException e) {
			piYsUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd41RecieveFaEJB(특수강야드수신) ejbCreate TimeoutException error :" + e.getMessage());
		}		
		catch (Exception e) {
			piYsUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd41RecieveFaEJB(특수강야드수신) ejbCreate Exception error :" + e.getMessage());
		}
	}
	...
	public void ejbRemove() {
		...
		piYsUtils.printLog("MES_PI_RABBIT_MQ", "", "===== M10LmExYd41RecieveFaEJB(특수강야드수신) ejbRemove called");
    	...
    	try {
			...
			piYsUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd41RecieveFaEJB(특수강야드수신) ejbRemove error :" + e1.getMessage());
		}
		catch (TimeoutException e2) {
			piYsUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd41RecieveFaEJB(특수강야드수신) ejbRemove error :" + e2.getMessage());
		}		
		catch (Exception e3) {
			piYsUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd41RecieveFaEJB(특수강야드수신) ejbRemove error :" + e3.getMessage());
		}
	...
	*/
	public void RabbitConnect() throws IOException, TimeoutException, Exception{
		try {
			...
			this.QNAME        = propertyService.getProperty("common.properties","rabbitmq.YD.M10LM-Q-YD-41");
			this.ROUTINGKEY   = propertyService.getProperty("common.properties","rabbitmq.YD.m10lm.yd.41");			
//LOCAL			
//			this.QNAME        = "M10LM-Q-YD-41S"; 
//			this.ROUTINGKEY   = "m10lm.yd.41S";   
			...
			piYsUtils.printLog("MES_PI_RABBIT_MQ","", "=====M10LmExYd41RecieveFaEJB(특수강야드수신) ejbRemove error :" + e.getMessage());
		}
		catch (TimeoutException e) {
			piYsUtils.printLog("MES_PI_RABBIT_MQ","", "=====M10LmExYd41RecieveFaEJB(특수강야드수신) ejbRemove error :" + e.getMessage());
		}		
		...
		piYsUtils.printLog("MES_PI_RABBIT_MQ", "", "===== M10LmExYd41RecieveFaEJB(특수강야드수신)      RabbitConnected OK");
	}
	...
	*/
	public void RabbitDisConnect() throws IOException, TimeoutException{
		...
		piYsUtils.printLog("MES_PI_RABBIT_MQ", "", "===== M10LmExYd41RecieveFaEJB(특수강야드수신) RabbitDisConnect called");
	}
	//메세지 수신된 시간
	...
	*/
		...
		Consumer consumer = new DefaultConsumer(this.channel)
		{
	    	...
	    	public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties basicproperties, byte[] message) throws IOException{
    			...
    			try {
    				...
    				piYsUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd41RecieveFaEJB(특수강야드)수신Message 전문: <<<<<<<<<< : " + recievedMessage);

					//받은 메세지를 처리하는 함수 호출
					MessageProcessYd41(recievedMessage);
					...
					piYsUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd41RecieveFaEJB(특수강야드수신) StartReceive handleErrorMessage error " + e.getMessage());
				}//end of catch
	 ...
	 * @throws Exception
	*/
	public HashMap MessageProcessYd41( String paramMap ) throws JSONException,Exception{
		...
		try {
			...
			ejbConn = new EJBConnector("default", "YsCommEJB", this);
...
//			Thread.sleep(2000);
			...
			piYsUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd41RecieveFaEJB(특수강야드수신) MessageProcessYd41 JSONException " + e.getMessage());
             AlimtalkSendUtil.SendAlimTalk(this.QNAME, "M10LmExYs41RecieveFaEJB", e.getMessage());//알림톡전송
			httpRaggitMqlog.postReceiveLog(trId, recvTime, Util.Now_YYYYMMddHHmmss(), "E", "MessageProcessExam Exception:" +e.getMessage());

		} catch (Exception e) {
			piYsUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd41RecieveFaEJB(특수강야드수신) MessageProcessYd41 Exception " + e.getMessage());
//			Thread.sleep(2000);
			AlimtalkSendUtil.SendAlimTalk(this.QNAME, "M10LmExYs41RecieveFaEJB", e.getMessage());//알림톡전송
			httpRaggitMqlog.postReceiveLog(trId, recvTime, Util.Now_YYYYMMddHHmmss(), "E", "MessageProcessExam Exception:" +e.getMessage());
