...
//*/
public class M10YsExLm54SenderFaEJBBean { //extends BaseSessionBean{
	...
	private YsCommUtils piYdUtils = new YsCommUtils();
...
//	*/
	public HashMap SendMessage(final HashMap paramMap) throws IOException, TimeoutException, Exception{
		...
		try {
			...
			piYdUtils.printLog("MES_PI_RABBIT_MQ", "===== M10YsExLm54SenderFaEJB(특수강임가공)송신Message 전문: >>>>>>>>>> :" + paramMap, "S+");

		} 
		catch (IOException e) {
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10YsExLm54SenderFaEJB(특수강임가공) IOException Error " + e.getMessage());
		}
		catch (Exception e) {
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10YsExLm54SenderFaEJB(특수강임가공) Exception Error " + e.getMessage());
		}
...
//			this.ROUTINGKEY   = "m10yd.lm.51S";  			
			...
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10YsExLm54SenderFaEJB(특수강임가공) RabbitConnect IOException ======" + e.getMessage());
		}
		catch (TimeoutException e) {
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10YsExLm54SenderFaEJB(특수강임가공) RabbitConnect TimeoutException ======" + e.getMessage());
		}
		catch (Exception e) {
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10YsExLm54SenderFaEJB(특수강임가공) Exception ======" + e.getMessage());
		}

		piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10YsExLm54SenderFaEJB(특수강임가공) RabbitConnected ======OK");
...
//	*/
	public void RabbitDisConnect() throws IOException, TimeoutException{
		...
		piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10YsExLm54SenderFaEJB(특수강임가공) RabbitDisConnect called");
	}
