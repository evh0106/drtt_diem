...
public class SbrYsComm implements YsQueryIFSbr {
	...
	*/
	public JDTORecord chkEqpStat(JDTORecord rcvMsg) throws DAOException {
		String methodNm  = "설비상태체크[SbrYsComm.chkEqpStat] < " + rcvMsg.getResultMsg();
	...
	*/
	public JDTORecord chkSchCd(JDTORecord rcvMsg) throws DAOException {
		String methodNm  = "스케줄코드 체크[SbrYsComm.chkSchCd] < " + rcvMsg.getResultMsg();
		String logId     = rcvMsg.getResultCode();
		...
		try {
			...
			**********************************************************/
			...
			if( !"".equals(ydL3Msg) ) {
				jrRtn.setField("YD_L3_HD_RS_CD", ydL3HdRsCd);	//야드L3처리결과코드
				jrRtn.setField("YD_L3_MSG"     , ydL3Msg   );	//야드L3MESSAGE
				commUtils.printLog(logId, "1. 스케줄코드 체크 : ydL3Msg [" + ydL3Msg + "]", "SL");
				throw new Exception(ydL3Msg);
			}
...
//				ydL3HdRsCd = "SC03";
//				ydL3Msg    = "오류:스케줄금지여부[" + ydSchCd + "] 정보 없음";
//			} else if( "Y".equals(ydSchProhExn) ) {
			...
			if (!"".equals(ydL3Msg)) {
				jrRtn.setField("YD_L3_HD_RS_CD", ydL3HdRsCd);	//야드L3처리결과코드
				jrRtn.setField("YD_L3_MSG"     , ydL3Msg   );	//야드L3MESSAGE
				commUtils.printLog(logId, "2. 크레인스케줄 상태 Check : ydL3Msg [" + ydL3Msg + "]", "SL");
				throw new Exception(ydL3Msg);
			}
	...
	*/
	public JDTORecord chkSchCdEqp(JDTORecord jrParam) {
		String methodNm = "스케줄코드, 크레인 체크[SbrYsComm.chkSchCdEqp] < " + jrParam.getResultMsg();
...
//		String methodNm = "스케줄코드 조회[SbrYsComm.getSchCd] < " + jrParam.getResultMsg();
//		String logId = jrParam.getResultCode();
//
//		try {
//			commUtils.printLog(logId, methodNm, "S+");
...
//		String methodNm = "크레인사양Check[SbrYsComm.chkCrnSpec] < " + jrCrnSpec.getResultMsg();
...
//	}
	...
	*/
	public JDTORecord getCrnSchMsg(JDTORecord jrParam) {
		...
		String methodNm = "크레인스케줄전문조회[SbrYsComm.getCrnSchMsg] < " + jrParam.getResultMsg();
		String logId = jrParam.getResultCode();
		...
		try {
			...
			if (!"".equals(ydWbookId) && ("".equals(ydSchCd) || "".equals(ydEqpId))) {
				...
				JDTORecordSet jsChk = commDao.select(jrParam, "com.inisteel.cim.ys.sbr.dao.getCrnSchStartGp", logId, methodNm, "크레인스케줄기동구분 조회");
				// getCrnSchStartGp
			...
			// 크레인스케줄 기동
			jrYdMsg.setField("JMS_TC_CD"		 , "YSYSJ702"); //소형야드 크레인스케줄Main
	...
	*/
	public JDTORecord getYSN2L004(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "크레인작업실적응답 조회[SbrYsComm.getYSN2L004] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
	...
	*/
	public JDTORecord getYSN7L304(JDTORecord rcvMsg) throws DAOException {
        String szMsg	= "";
		String methodNm = "소형야드 크레인작업실적응답 조회[SbrYsComm.getYSN7L304]";
		String logId = rcvMsg.getResultCode();
