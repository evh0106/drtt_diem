 ...
 * @date             2025/07/01
 *
 * @description      특수강 사외통합야드 공통 처리 EJB
 * ------------------------------------------------------------------------------
 * Ver.   수정일자              요청자       수정자      내용
 * =====  ===========  ======  ======  ==========================================
...
public class YsTotYsComm implements YsQueryIFYsTot {
	...
	*/
	public JDTORecord chkEqpStat(JDTORecord rcvMsg) throws DAOException {
		String methodNm  = "설비상태체크[YsTotYsComm.chkEqpStat] < " + rcvMsg.getResultMsg();
	...
	*/
	public JDTORecord chkSchCd(JDTORecord rcvMsg) {
		String methodNm  = "스케줄코드 체크[YsTotYsComm.chkSchCd] < " + rcvMsg.getResultMsg();
		String logId     = rcvMsg.getResultCode();
...
//			}
	...
	*/
	public JDTORecord chkSchCdEqp(JDTORecord jrParam) {
		String methodNm = "스케줄코드, 크레인 체크[YsTotYsComm.chkSchCdEqp] < " + jrParam.getResultMsg();
...
//			} else {
//				jRecode.addField("YD_EQP_ID", "");
//				jRecode.addField("YD_SCH_PRIOR", "");
//				jsChk.addRecord(jRecode);
//			}
...
//			}
	...
	*/
	public JDTORecord getSchCd(JDTORecord jrParam) {
		String methodNm = "스케줄코드 조회[YsTotYsComm.getSchCd] < " + jrParam.getResultMsg();
		String logId = jrParam.getResultCode();
...
//			}
	...
	*/
	public String chkCrnSpec(JDTORecord jrCrnSpec) throws DAOException {
		String methodNm = "크레인사양Check[YsTotYsComm.chkCrnSpec] < " + jrCrnSpec.getResultMsg();
	...
	*/
	public JDTORecord getCrnSchMsg(JDTORecord jrParam) {
		...
		String methodNm = "크레인스케줄전문조회[YsTotYsComm.getCrnSchMsg] < " + jrParam.getResultMsg();
		String logId = jrParam.getResultCode();
...
//				}
	...
	*/
	public JDTORecord getYSN2L004(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "크레인작업실적응답 조회[YsTotYsComm.getYSN2L004] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
	...
	*/
	public JDTORecord getYSN8L004(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "크레인작업실적응답 조회[YsTotYsComm.getYSN8L004] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
	...
	*/
	public JDTORecordSet getMsgL3(String msgId, JDTORecord jrParam) throws DAOException {
		String methodNm = "L3전문생성[YsTotYsComm.getMsgL3] < " + jrParam.getResultMsg();
		String logId = jrParam.getResultCode();
		String trtNm = "";
		...
		try {
			...
			if ("YSTSJ008".equals(msgId)) {
				trtNm = "소재 상차완료";
				jspeed_query_id = "com.inisteel.cim.ys.ystot.dao.TcYSTSJ008";
			} 
			...
			if(!"".equals(jspeed_query_id)) {
				...
				jsRst = commDao.select2(jrParam, jspeed_query_id);
					
				commUtils.printLog(logId, trtNm + jsRst.size(), "DB");
				
				String sApplyYnPI = commDao.ApplyYnPI("", methodNm, "APPPI1", "*", "*");
				...
				if(jsRst.size()>0) 
				{
					...
					JDTORecordSet jsLayOut = commDao.select(jrParam, getIfTestLayout);
