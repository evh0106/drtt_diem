 ...
 * @date             2025/07/01
 *
 * @description      특수강대형옥내야드 Schedule 처리 Session EJB
 * ------------------------------------------------------------------------------
 * Ver.   수정일자              요청자       수정자      내용
 * =====  ===========  ======  ======  ==========================================
 ...
 *
 * @ejb.bean name="CbtYsSchSeEJB" jndi-name="CbtYsSchSeEJB" type="Stateless"
...
public class CbtYsSchSeEJBBean extends BaseSessionBean implements YsQueryIFCbt {
	...
	*/
	public JDTORecord rcvYSYSJ502(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "크레인스케줄[CbtYsSchSeEJB.rcvYSYSJ502] < " + rcvMsg.getResultMsg();
		...
		try {
			...
			String isCallBy204   = commUtils.trim(rcvMsg.getFieldString("IS_CALL_BY_204"    ));	//야드설비ID
			
			commUtils.printLog(logId, "N7YSJ204에 의해 호출여부 IS_CALL_BY_204 [" + isCallBy204 + "]", "SL");
			...
			/**********************************************************
			* 0. 대형옥내야드-출하상(GEPC31/GEPC31/GEPC41/GEPC42/GEPC51) 모든 스위치 OFF 여부 체크
			**********************************************************/
        	//출하상 전체 OFF 이면 E5 크레인 작업지시[대형옥내 입고 (빌렛/각강출하상), 대형옥내 입고 (봉강출하상)] 작성 중지
        	String sApplyYnPI = commDao.ApplyYnPI(logId, methodNm, "APPNEW", "016", "*");
        	if( "Y".equals(sApplyYnPI) && (constant.SCH_CD_GEPC01LA.equals(ydSchCd) || constant.SCH_CD_GEPC02LA.equals(ydSchCd)) ) {	//GEPC01LA GEPC02LA
        		
        		JDTORecordSet rsParam = commDao.select(jrParam, getSwitchOffYn, logId, methodNm, "대형옥내야드-출하상(GEPC31/GEPC31/GEPC41/GEPC42/GEPC51) 모든 스위치 OFF 여부 체크");
    			...
    			if( rsParam.size() > 0 ) {
    				...
    				String sOFF_YN = commUtils.trim(recInPara.getFieldString("OFF_YN"));	//대형옥내야드-출하상(GEPC31/GEPC31/GEPC41/GEPC42/GEPC51) 모든 스위치 OFF 여부

            		if( "GECRE5".equals(ydEqpId) && "Y".equals(sOFF_YN) ) {	//[N:스위치 ON 존재],[Y:스위치 ON 미존재]
            			throw new Exception("오류:대형옥내야드-출하상(GEPC31/GEPC31/GEPC41/GEPC42/GEPC51) 모든 스위치 OFF~!!!");
            		}
    			}
        	}
			...
			**********************************************************/
			...
			if( "".equals(ydWbookId) && !"".equals(ydSchCd) ) {
				...
				JDTORecord jrChk = CbtYsComm.chkSchCd(jrParam);
			...
			sbImpPros.append("[크레인스케줄 > 파라메타정보체크및'작업예약ID'조회 > 작업예약ID조회]::완료 \r\n");
			...
			**********************************************************/
			...
			EJBConnector tranConn = new EJBConnector("default", "CbtYsSchSeEJB", this);
			...
			**********************************************************/
			...
			if( jsChk.size() <= 0 ) {
				...
				//차량하차 시 동일 적치단에 있는 소재가 6개 이상일 경우 작업예약이 n개로 나누어 생성됨
...
//				}
			...
			* 7. 크레인작업지시 호출 [내부 크레인 작업지시 요구 (rcvYSYSJ001)]
			...
			commUtils.printLog(logId, "ydEqpStat >>>>>>>>>> " + ydEqpStat, "");
			...
			boolean isAbleCall001 = false;
			
			sApplyYnPI = commDao.ApplyYnPI(logId, methodNm, "APPNEW", "027", "*");
			
			if("Y".equals(sApplyYnPI)){
	      		commUtils.printLog(logId, "신규기준 적용인경우 isCallBy204 값이 Y 이면 크레인상태 4 에도 YSYSJ001 호출", "");
	      		
	      		if( "Y".equals(isCallBy204) &&( "W".equals(ydEqpStat) || "4".equals(ydEqpStat)) ) {
	      			isAbleCall001 = true;
	      		}
	      		else if("W".equals(ydEqpStat)){
	      			isAbleCall001 = true;
			...
			commUtils.printLog(logId, "isAbleCall001 값 : "+ Boolean.toString(isAbleCall001), "");
			...
			if( isAbleCall001 ) {
				...
				if (!ruleItem.equals("Y")) {
					// 2025.08.25 rcvYSYSJ001 Method 크레인 작업지시 요구 Method N7YSL204 (특수강 정정 야드 크레인작업지시요구), N8YSL004 (대형 봉강 옥외 야드 크레인작업지시요구) 추가				
    ...
    public int LocSrcRngDataSet (String logId, String methodNms, JDTORecord inRecord, StringBuffer sbImpPros)throws JDTOException{
    	String methodNm = "TO위치 결정[CbtYsSchSeEJB.LocSrcRngDataSet] < " + methodNms;
			...
			**********************************************************/
		    ...
		    for( int Loop_i = 1; Loop_i <= rsCrnsch.size(); Loop_i++ ) {
            	...
            	if( szToLocDcsnMtd.equals("W") ) {
        			...
        			//보조작업 TO위치결정
        			szRtnMsg = this.procDummyToLoc(logId, methodNm, rsCrnwrkmtl, recCrnSch, recWbook, sbImpPros);
            	...
            	//3.2.2. "주작업"인 경우
    				...
    				//주작업 TO위치결정
    				szRtnMsg = this.procMainWrkLoc(logId, methodNm, rsCrnwrkmtl, recCrnSch, recWbook, sbImpPros);
				...
				String sParamVal = "CS"+"#"+"G"+"#"+"E"+"#"+szWbookId+"#"+szCrnSchId+"#"+"1"+"#"+sbImpPros.toString()+sbImpPros_ToLoc.toString()+"\r\n"+"#"+sSCH_CONTENTS;
			...
			**********************************************************/
    		...
    		for( int Loop_i = 1; Loop_i <= rsCrnsch.size(); Loop_i++ ) {
				...
				if( "".equals(sYS_DN_WO_LOC) ) {
					...
					recInPara.setField("YS_DN_WO_LOC", "GEXXXXXX");
				...
				//"빌렛정정야드-봉강적치대"에서 "빌렛정정야드-적치대"로 이적이 발생할 경우
				} else if( ("GETY07".equals(sYS_UP_STK_COL_GP) || "GETY08".equals(sYS_UP_STK_COL_GP)) && "GE02".equals(sYS_DN_STK_SPAN)) {
					
					rsRSS = JDTORecordFactory.getInstance().createRecordSet("");
							
					//빌렛정정야드 적치대에 "봉강"이 존재하면 스케줄 생성 금지
					// - 여러 번들을 빌렛정정야드로 가져가서 작업하면서 발생하는 혼합재 발생을 막기 위함
					rsRSS = commDao.select(recInPara, getSRRExistYD, logId, methodNm, "빌렛정정야드 적치대에 '봉강' 존재여부 확인");

	        		if( rsRSS.size() > 0 ) {
	    				commUtils.printLog(logId, "빌렛정정야드 적치대에 '봉강'이 존재하여 스케줄 생성 취소. ", "SL");
	    				new Exception("예외처리 >> 빌렛정정야드 적치대에 '봉강'이 존재하여 스케줄 생성 취소");
	        		}
				}
...
//				}
    ...
    public int CrnSchIns (String logId, String methodNms ,JDTORecordSet outRecset, JDTORecord jrParamSet, StringBuffer sbImpPros) throws JDTOException {
   		String methodNm = "크레인스케줄, 크레인작업재료 등록[CbtYsSchSeEJB.CrnSchIns] < " + methodNms;
	...
	*/
	public void updCrnSchWB(JDTORecord jrParam) throws DAOException {
		String methodNm = "크레인스케줄 작업예약재료 저장위치 수정[CbtYsSchSeEJB.updCrnSchWB] < " + jrParam.getResultMsg();
	...
	public int CrnSchGrp(String logId, String methodNms ,JDTORecord recMainPara, JDTORecordSet rsReturn, StringBuffer sbImpPros)throws JDTOException  {
		String methodNm = "크레인 스케줄 GROUPING PARAMETER DATA SETTING[CbtYsSchSeEJB.CrnSchGrp] < " + methodNms;
...
//    			String szBED_CNT 		= ""+Loop_j;
    			...
    			String szITEMNAME_CD 	= recStkLyr.getFieldString("ITEMNAME_CD");
...
//	    			recInPara9.setField("YD_STK_LOT_CD"		, szYD_STK_LOT_CD);
					...
					//적치단의 소재가 "SRR(봉강)"이면 선택된 번들만 이적하고,
//					if( "SRR".equals(szITEMNAME_CD) ) {
//						recInPara9.setField("WRK_SPR"			, "TARGET");
//					//적치단의 소재가 "SRQ/SRW/SRI(각강/빌렛)"이면 적치단의 모든 소재들을 이적함
//					} else {
...
//YD_UP_COLL_SEQ -> "사용처 & 값" 확인 필요~!!!
	...
	public int CrnSchGrpTr(String logId, String methodNms ,JDTORecord recMainPara, JDTORecordSet rsReturn, StringBuffer sbImpPros)throws JDTOException  {
		String methodNm = "(차량하차) 크레인 스케줄 GROUPING PARAMETER DATA SETTING[CbtYsSchSeEJB.CrnSchGrp] < " + methodNms;
...
//	    		}//end of for
    ...
    public String procDummyToLoc(String logId, String methodNms, JDTORecordSet rsCrnwrkmtl, JDTORecord recCrnSch, JDTORecord recWbook, StringBuffer sbImpPros) throws JDTOException {
    	String methodNm      = "보조작업TO위치결정[CbtYsSchSeEJB.procDummyToLoc] < " + methodNms;
    	String LocalmethodNm = "보조작업TO위치결정[CbtYsSchSeEJB.procDummyToLoc] "; 
...
//		String szYD_MTL_T_GP 	= commUtils.trim(recStock.getFieldString("YD_MTL_T_GP"  ));		//크레인작업 첫번째 재료의 두께구분
		...
		String szITEMNAME_CD    = commUtils.trim(recStock.getFieldString("ITEMNAME_CD"  ));		//크레인작업 첫번째 재료의 품명코드: [봉강:SRR][각강:SRQ][빌렛:SRI,SRW] <- 최종 완성된 형태 ?!
...
//			szYD_STKBED_USG_CD = commUtils.trim(outRecResult.getFieldString("YD_STKBED_USG_CD"  ));		//적치대 상태
	...
	public JDTORecord procLocAbleCheckCnt_Eqp(String logId, String methodNms,JDTORecord jrParamSet, StringBuffer sbImpPros) throws JDTOException {
		String methodNm      = "설비-적재가능 여부 CHECK[CbtYsSchSeEJB.procLocAbleCheckCnt_Eqp] < " + methodNms;
		String LocalmethodNm = "설비-적재가능 여부 CHECK[CbtYsSchSeEJB.procLocAbleCheckCnt_Eqp]";
		...
		**********************************************************/
		...
		if( iCRNWRKMTL_CNT > 8 ) {
			commUtils.printLog(logId, "["+ LocalmethodNm +"] 크레인작업재료 매수가 8개 [빌렛(최대 6개), 각강(최대 8개)] 이상이면 적치불가 ", "SL");
			sbImpPros.append("[크레인스케줄 > TO위치결정 > 크레인스케줄별권하지시위치결정 > 주작업TO위치결정 > 동일한사양의적치가능한'열'조회 > 'TO위치가이드'값이('열'||'열/BED')지정인경우 > TO위치가'설비'일경우 > 적재가능여부CHECK > 설비-적재가능여부CHECK]::(불가)크레인작업재료 매수가 8개 이상이면 적치불가: iCRNWRKMTL_CNT ["+iCRNWRKMTL_CNT+"] \r\n");
		...
		for( int j = 1; j <= RsResultBed.size(); j++ ) {
			...
			//"BED/Max-단"의 해당 BED가 [(빌렛정정 빌렛정정보급 || 빌렛정정 프레스교정보급) && (크레인작업재료 매수 + TO위치-적치된 재료매수 <= 8)] 일 경우 TO위치로 결정
			//if( (constant.SCH_CD_GETF01LB.equals(szYD_SCH_CD) || constant.SCH_CD_GETF02LB.equals(szYD_SCH_CD)) && (iCRNWRKMTL_CNT + iTEMP_STL_CNT <= 8) ) {
			
			//"BED/Max-단"의 해당 BED가 [(빌렛정정 빌렛정정보급 || 빌렛정정 프레스교정보급 || 빌렛정정 빌렛소형보급) 일 경우 TO위치로 결정 <= 연속작업 가능하게 to위치 재료존재여부 확인 안함 constant.SCH_CD_GETF03LB.equals(szYD_SCH_CD)
			if( constant.SCH_CD_GETF01LB.equals(szYD_SCH_CD) || constant.SCH_CD_GETF02LB.equals(szYD_SCH_CD) || constant.SCH_CD_GETF04LB.equals(szYD_SCH_CD) ) {
				...
				sbImpPros.append("[크레인스케줄 > TO위치결정 > 크레인스케줄별권하지시위치결정 > 주작업TO위치결정 > 동일한사양의적치가능한'열'조회 > 'TO위치가이드'값이('열'||'열/BED')지정인경우 > TO위치가'설비'일경우 > 적재가능여부CHECK > 설비-적재가능여부CHECK]::(가능)TO 위치 가이드 처리 *위치 결정1::szYS_DN_WO_LOC:" +  szTEMP_YS_STK_COL_GP + szTEMP_YS_STK_BED_NO + "szYS_DN_WO_LAYER:" + szTEMP_YS_STK_LYR_NO + "\r\n");
			...
			//"BED/Max-단"의 해당 BED의 적치된 재료가 없을 경우 TO위치로 결정
				...
				sbImpPros.append("[크레인스케줄 > TO위치결정 > 크레인스케줄별권하지시위치결정 > 주작업TO위치결정 > 동일한사양의적치가능한'열'조회 > 'TO위치가이드'값이('열'||'열/BED')지정인경우 > TO위치가'설비'일경우 > 적재가능여부CHECK > 설비-적재가능여부CHECK]::(가능)TO 위치 가이드 처리 *위치 결정2::szYS_DN_WO_LOC:" +  szTEMP_YS_STK_COL_GP + szTEMP_YS_STK_BED_NO + "szYS_DN_WO_LAYER:" + szTEMP_YS_STK_LYR_NO + "\r\n");
        ...
        if( !"".equals(szYS_DN_WO_LOC) ) {
        	...
        	sbImpPros.append("[크레인스케줄 > TO위치결정 > 크레인스케줄별권하지시위치결정 > 주작업TO위치결정 > 동일한사양의적치가능한'열'조회 > 'TO위치가이드'값이('열'||'열/BED')지정인경우 > TO위치가'설비'일경우 > 적재가능여부CHECK > 설비-적재가능여부CHECK]::(불가)TO위치-재료매수가 '0'인지 확인 필요함~!!! \r\n");
        }
	...
	public JDTORecord procLocAbleCheckCnt_Car(String logId, String methodNms,JDTORecord jrParamSet, StringBuffer sbImpPros) throws JDTOException {
		String methodNm      = "차량-적재가능 여부 CHECK[CbtYsSchSeEJB.procLocAbleCheckCnt] < " + methodNms;
		String LocalmethodNm = "차량-적재가능 여부 CHECK[CbtYsSchSeEJB.procLocAbleCheckCnt]";
...
//		String szTEMP_YS_STK_LYR_NO = "";
	...
	public JDTORecord procLocAbleCheckCnt(String logId, String methodNms,JDTORecord jrParamSet, StringBuffer sbImpPros) throws JDTOException {
		String methodNm      = "적재가능 여부 CHECK[CbtYsSchSeEJB.procLocAbleCheckCnt] < " + methodNms;
		String LocalmethodNm = "적재가능 여부 CHECK[CbtYsSchSeEJB.procLocAbleCheckCnt]";
...
//		int    iTEMP_STL_CNT        = 0;																			//TO위치-적치된 재료매수
		...
		**********************************************************/
		...
		if( RsResultBed.size() <= 0 ) {
			...
			sbImpPros.append("[크레인스케줄 > TO위치결정 > 크레인스케줄별권하지시위치결정 > 주/보조작업TO위치결정 > 동일한사양의적치가능한'열'조회 > ... >... >  적재가능여부CHECK > 적재가능 여부 CHECK]::(불가)TO위치(열)의 모든 BED들을 대상으로 'Max단'에 존재하는 BED별 재료 매수 조회 실패: getYdStkBedInfo \r\n");
...
//				szYS_DN_WO_LAYER = commUtils.stringPlusInt(""+szTEMP_YS_STK_LYR_NO, 1);
			...
			//TO위치로 결정되었지만 ["TO위치-적치단" > "TO위치-최대 적재 가능단"]일 경우 "불가"로 Return
...
//	        }
	...
	public String procMainWrkLoc(String logId, String methodNms, JDTORecordSet rsCrnwrkmtl, JDTORecord recCrnSch, JDTORecord recWbook, StringBuffer sbImpPros) throws JDTOException {
		String methodNm      = "주작업TO위치결정[CbtYsSchSeEJB.procMainWrkLoc] < " + methodNms;
		String LocalmethodNm = "주작업TO위치결정[CbtYsSchSeEJB.procMainWrkLoc] " ;
...
//		recTemp.setField("YS_STK_BED_NO"     , szYD_UP_STK_BED_NO  );					//권상지시위치-적치BED
		...
		if( (szYD_TO_LOC_GUIDE.length() == 6) || (szYD_TO_LOC_GUIDE.length() == 8) ) {
			...
			if( constant.SCH_CD_GETF01LB.equals(szYD_SCH_CD) || constant.SCH_CD_GETF01MB.equals(szYD_SCH_CD)  ||	//빌렛정정 빌렛정정보급 || (빌렛정정 빌렛정정보급추출 -> 임시BED[GETY05])
				constant.SCH_CD_GETF04LB.equals(szYD_SCH_CD) || constant.SCH_CD_GETF02LB.equals(szYD_SCH_CD)  ||	//빌렛정정 빌렛소형보급 || 빌렛정정 프레스교정보급
				constant.SCH_CD_GETF03LB.equals(szYD_SCH_CD) || 													//빌렛정정 하부비파괴추출테이블보급
				(constant.SCH_CD_GEPC01UB.equals(szYD_SCH_CD) && (szYD_TO_LOC_GUIDE.substring(2,4) == "TF")) ) {	//빌렛정정 빌렛정정추출 -> 소형적재테이블(GETF04)
				...
				//설비-적재가능 여부 CHECK
				//각 설비별 적재가능여부 판별 기준은 ???
				// - 빌렛정정 빌렛정정보급                          : 판별조건 제거 (2025.11.18. L2 양준석)
				// - 빌렛정정 빌렛정정보급추출(->임시BED[GETY05])   : 작업재료매수 <= 8 && TO위치-적치매수 == 0
				// - 빌렛정정 빌렛소형보급							: 작업재료매수 <= 8 && TO위치-적치매수 == 0
				// - 빌렛정정 프레스교정보급						: 판별조건 제거 (2025.11.18. L2 양준석)
				// - 빌렛정정 하부비파괴추출테이블보급              : 작업재료매수 <= 8 && TO위치-적치매수 == 0
				// - 빌렛정정 빌렛정정추출 -> 소형적재테이블(GETF04): 작업재료매수 <= 8 && TO위치-적치매수 == 0
			...
			} else if( constant.SCH_CD_GETR41UA.equals(szYD_SCH_CD) || constant.SCH_CD_GETR41UB.equals(szYD_SCH_CD)  ||	//대형옥내 이송출고 (남4문) || 빌렛정정 이송출고 (남4문)
					   constant.SCH_CD_GETR21UB.equals(szYD_SCH_CD) || constant.SCH_CD_GETR11UB.equals(szYD_SCH_CD)  ||	//빌렛정정 이송출고 (남2문) || 빌렛정정 이송출고 (남1문)
					   constant.SCH_CD_GETR42UB.equals(szYD_SCH_CD) ) {													//빌렛정정 이송출고 (밴드쏘)
...
//				szYS_DN_WO_LAYER = "01";									//"단"
	...
	public String procUpdateLoc(String logId, String methodNms, JDTORecord recCrnwrkmtl, JDTORecord RecSetLoc, StringBuffer sbImpPros) throws JDTOException {
		String methodNm      = "TO위치 UPDATE[CbtYsSchSeEJB.procUpdateLoc] < " + methodNms;
		String LocalmethodNm = "TO위치 UPDATE[CbtYsSchSeEJB.procUpdateLoc]";
		...
		if( !szYS_DN_WO_LOC.equals("") ) {
			...
			**********************************************************/
			...
			if( intRtnVal <= 0 ) {
				...
				sbImpPros.append("[크레인스케줄 > TO위치결정 > 크레인스케줄별권하지시위치결정 > 주/보조작업TO위치결정 > TO위치UPDATE > 크레인스케줄(좌표 등)권하위치정보갱신]::크레인스케줄(좌표 등) 권하위치정보 갱신 중 ERROR 발생 szYS_DN_WO_LOC [" + szYS_DN_WO_LOC + "], szYS_DN_WO_LAYER [" + szYS_DN_WO_LAYER + "]\r\n");
		...
		**********************************************************/
		...
		//해당 설비(빌렛정정 출하상(상단), 프레스교정기 추출대, 글라인드크래들)일 경우 소재들의 "SEQ_NO"를 역순으로 변경함
		//  - 이유: 작업or이동 후 해당 설비 적치대에 쌓이는 순서가 이동전의 물리적인 배치순서와 역순으로 배열 됨
		//  # "L2 <-> I/F [M4YSL220]"에서 처리 시 "TB_YS_STKLYR, TB_YS_STOCK" 테이블에 Update (or Insert) 하고 있는데...
		//                                        L2에서 역순으로 전문을 보내주는지 ?  I/F에서 치리시 2개 테이블에 역순으로 Update or Insert 하는지 확인 후 개발하기~!!!
//		if( constant.SCH_CD_GEPC01UB.equals(szYD_SCH_CD) || constant.SCH_CD_GEPC03UB.equals(szYD_SCH_CD) || constant.SCH_CD_GEPC04UB.equals(szYD_SCH_CD) ) {
//			rsOutBed = commDao.select(recInBed, getStklyrDescInfoByCrnsch, logId, methodNm, "권상위치 작업재료를 'SEQ_NO' 역순으로 조회");
//
//			if( rsOutBed.size() <= 0 ) {
//				commUtils.printLog(logId, "["+ LocalmethodNm +"] 권상위치 작업재료를 'SEQ_NO' 역순으로 조회", "SL");
//			}
//			
//			// "TB_YS_STKLYR, TB_YS_STOCK, TB_YS_CRNWRKMTL" 테이블에  "SEQ_NO"를 역순으로 Update 처리 로직 개발해야 함
//			
//		} else {
