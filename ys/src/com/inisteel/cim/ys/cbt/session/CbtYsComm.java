...
public class CbtYsComm implements YsQueryIFCbt {
	...
	*/
	public JDTORecord chkEqpStat(JDTORecord rcvMsg) throws DAOException {
		String methodNm  = "설비상태체크[CbtYsComm.chkEqpStat] < " + rcvMsg.getResultMsg();
	...
	*/
	public JDTORecord chkSchCd(JDTORecord rcvMsg) {
		String methodNm  = "스케줄코드 체크[CbtYsComm.chkSchCd] < " + rcvMsg.getResultMsg();
		String logId     = rcvMsg.getResultCode();
	...
	*/
	public JDTORecord chkSchCdEqp(JDTORecord jrParam) {
		String methodNm = "스케줄코드, 크레인 체크[CbtYsComm.chkSchCdEqp] < " + jrParam.getResultMsg();
...
//			}
	...
	*/
	public JDTORecord getSchCd(JDTORecord jrParam) {
		String methodNm = "스케줄코드 조회[CbtYsComm.getSchCd] < " + jrParam.getResultMsg();
		String logId = jrParam.getResultCode();
	...
	*/
	public String chkCrnSpec(JDTORecord jrCrnSpec) throws DAOException {
		String methodNm = "크레인사양Check[CbtYsComm.chkCrnSpec] < " + jrCrnSpec.getResultMsg();
	...
	*/
	public JDTORecord getCrnSchMsg(JDTORecord jrParam) {
		...
		String methodNm = "크레인스케줄전문조회[CbtYsComm.getCrnSchMsg] < " + jrParam.getResultMsg();
		String logId = jrParam.getResultCode();
		...
		try {
			...
			String isCallBy204   = commUtils.trim(jrParam.getFieldString("IS_CALL_BY_204"     ));	//작업지시요구로부터 호출 여부
			
			commUtils.printLog(logId, "작업예약으로부터 작업생성 호출여부:" + isCallBy204, "SL");
			...
			// 크레인스케줄 기동
			jrYdMsg.setField("JMS_TC_CD"		 , "YSYSJ502"); //대형옥내 크레인스케줄Main
			...
			jrYdMsg.setField("IS_CALL_BY_204"     , isCallBy204); //야드스케쥴요청구분
			
			String sApplyYnPI = commDao.ApplyYnPI(logId, methodNm, "APPNEW", "026", "*");
			if("Y".equals(sApplyYnPI)){
				commUtils.printLog(logId, "크레인스케줄 전문송신 아닌 EJB 직접호출 변경.", "SL");
				
	    		EJBConnector ejbConn = new EJBConnector("default", "CbtYsSchSeEJB", this);
	    		jrRtn = (JDTORecord)ejbConn.trx("rcvYSYSJ502", new Class[] { JDTORecord.class }, new Object[] { jrYdMsg });
			}
			else{
	...
	*/
	public JDTORecord getYSN2L004(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "크레인작업실적응답 조회[CbtYsComm.getYSN2L004] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
	...
	/**
	 *      [A] 오퍼레이션명 : 크레인작업실적응답(YSN7L204) 전문 조회  : 2025.07.28 소영조 Method 추가
	 *
	 * 		@ejb.interface-method EJBDoclet을 생성하는 태그입니다.
	 *      @param JDTORecord rcvMsg
	 *      @return JDTORecord
	 *      @throws DAOException
	*/
	public JDTORecord getYSN7L204(JDTORecord rcvMsg) throws DAOException {
        String szMsg	= "";
		String methodNm = "크레인작업실적응답 조회[CbtYsComm.getYSN7L204] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		...
		try {
            ...
            szMsg = "\n\t YD_EQP_ID   		: " 	+ ydEqpId 
                  + "\n\t YD_L2_WR_GP      	: " 	+ ydL2WrGp 
                  + "\n\t YD_L3_HD_RS_CD    : " 	+ ydL3HdRsCd 
                  + "\n\t YD_L3_MSG   		: " 	+ ydL3Msg 
                  ;
