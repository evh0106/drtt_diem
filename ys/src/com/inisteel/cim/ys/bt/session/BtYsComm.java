...
public class BtYsComm {
	...
	*/
	public JDTORecord chkEqpStat(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "설비상태Check[BtYsComm.chkEqpStat] < " + rcvMsg.getResultMsg();
	...
	*/
	public JDTORecord chkSchCd(JDTORecord rcvMsg) {
		String methodNm = "스케줄코드Check[BtYsComm.chkSchCd] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
		JDTORecord jrRtn = JDTORecordFactory.getInstance().create(); //결과
...
//			}
	...
	*/
	public JDTORecord chkSchCdEqp(JDTORecord jrParam) {
		String methodNm = "스케줄코드 및 크레인 Check[BtYdComm.chkSchCdEqp] < " + jrParam.getResultMsg();
...
//			}
	...
	*/
	public JDTORecord getCrnSchMsg(JDTORecord jrParam) {
		...
		String methodNm = "크레인스케줄전문조회[btYdComm.getCrnSchMsg] < " + jrParam.getResultMsg();
		String logId = jrParam.getResultCode();
...
//				JDTORecordSet jsChk = commDao.select(jrParam, "com.inisteel.cim.ys.common.dao.YsCommDAO.getCrnSchStartGp", logId, methodNm, "크레인스케줄기동구분 조회");
			...
			// 크레인스케줄 기동
			jrYdMsg.setField("JMS_TC_CD", "YSYSJ202"); //
			...
			//신 크레인스케줄 EJB Call
//			EJBConnector sndConn = new EJBConnector("default", "BtYsSchSeEJB", this);
//			JDTORecord jrRst = (JDTORecord)sndConn.trx("rcvYSYSJ202", new Class[] { JDTORecord.class }, new Object[] { jrYdMsg });
	...
	*/
	public JDTORecord getYSN2L004(JDTORecord rcvMsg) throws DAOException {
		String methodNm = "크레인작업실적응답 조회[BlYsComm.getYSN2L004] < " + rcvMsg.getResultMsg();
		String logId = rcvMsg.getResultCode();
