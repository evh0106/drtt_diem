 ...
 * @date			2012/11/14
 * 
 * @description		이클래스는  Util 클래스입니다.
 * ------------------------------------------------------------------------------
 * Ver.   수정일자              요청자       수정자      내용
 * =====  ===========  ======  ======  ==========================================
 * V1.01  2013/04/03   허철호      허철호      누가 뭘 변경했는지 모름 
 */
...
public class YdDeleComm {
    ...
    private static int  	EAI_SVR_PORT  			= 6806;
	...
	public int socketSender(String szRcvMsg) {
		...
		try {
			...
			nPort = YdCommonUtils.getPortByUsingCode(szTcCode);
		...
		try {
			...
			if(szFaName==null || szFaName.equals("") ||
			   ...
			   szTcCode==null || szTcCode.equals("")    ){
				...
				ydUtils.putLog(szSessionName, szFaName, szMsg, YdConstant.ERROR);
