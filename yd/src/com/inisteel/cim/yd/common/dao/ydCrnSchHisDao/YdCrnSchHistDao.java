package com.inisteel.cim.yd.common.dao.ydCrnSchHisDao;
...
public class YdCrnSchHistDao {
	...
	//update query id
	private String szQueryIdUpd1 = "com.inisteel.cim.yd.dao.ydcrnschhistdao.YdCrnschhistDao.updYdCrnschhist";

	
/*------------------------------------- SELECT -------------------------------------------*/

	/**
	 *      [A] 오퍼레이션명 : 크레인스케줄이력 SELECT
	 *      
	 ...
	 */	
	public int getYdCrnschhist(JDTORecord inRec, JDTORecordSet outRecSet, int intGp) throws DAOException, JDTOException {
		String szMethodName = "getYdCrnschhist";
		...
		try {
			...
			//parameter check
			blnChk_Field = this.chkPara_getYdCrnschhist(recPara, intGp);
	...
	public boolean chkPara_getYdCrnschhist(JDTORecord inRec, int intGp) throws JDTOException  {
		...
		try {
			if (intGp == 0) {
				...
				szFieldName = "V_YD_WRK_HDS_DD_FROM";
				blnErr = ydDaoUtils.chkField(inRec, szFieldName, 8, 2, 'S', 0, 0);
				if (!blnErr) return blnErr;
				
				szFieldName = "V_YD_WRK_HDS_DD_TO";
				blnErr = ydDaoUtils.chkField(inRec, szFieldName, 8, 2, 'S', 0, 0);
				if (!blnErr) return blnErr;
		...
		return blnErr;
	} // end of chkPara_getYdCrnschhist
...
/*------------------------------------- INSERT -------------------------------------------*/
	 ...
	 * @throws JDTOException 
	 */	
	public int insYdCrnschhist(JDTORecord inRec) throws DAOException, JDTOException {
		int intRtnVal = 0;
		boolean blnChk_Field = true;
...
/*------------------------------------- UPDATE -------------------------------------------*/
	 ...
	 * @throws JDTOException 
	 */		
	public int updYdCrnschhist(JDTORecord inRec, int intGp) throws DAOException, JDTOException {
		String szMethodName = "updYdCrnschhist";
		String szMsg = null;
		...
		//update data select
		intRtnVal = this.getYdCrnschhist(inRec, outRecSet, 0);
