package com.inisteel.cim.yd.common.dao.ydCarSpecDao;
	...
	public boolean chkPara_getYdCarspec(JDTORecord inRec, int intGp) throws JDTOException  {
		...
		try {
			if (intGp == 0) {
				...
				szFieldName = "V_YD_CAR_USE_GP1";
				blnErr = ydDaoUtils.chkField(inRec, szFieldName, 1, 1, 'S', 0, 0);
				if (!blnErr) return blnErr;
				
				szFieldName = "V_YD_CAR_USE_GP2";
				blnErr = ydDaoUtils.chkField(inRec, szFieldName, 1, 1, 'S', 0, 0);
			} else if (intGp == 6 || intGp == 7) {				
				szFieldName = "V_CAR_NO";
