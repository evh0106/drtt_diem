...
/**
 * Y1 (C연주슬라브야드L2) 송신 용 전문 생성
 * @author YHWHman
 *
 */
public class MakeTcY1 {
	...
	// 클래스명
	private static final String szClassName  = MakeTcY1.class.getName();
	 ...
	 */
	public static int makeY1L001(JDTORecord inRec, JDTORecordSet outRecSet){
		//		1.	전문 ID					MSG_ID					VARCHAR2(8)		YDY1L001
		...
		// 변수선언
		String szMethodName     	 = "makeY1L001";
		String szOperationName       = "C연주슬라브L2 저장위치제원";
		String szMsg        	     = "";
		String szTemp        	     = "";
		String szConv                = "";
		...
		try{
			...
			for(int nIdx=0; nIdx<intRtnVal; nIdx++) {
				...
				outRec.setField("MSG_ID" , "YDY1L001");
				String ydGp = StringHelper.evl(recGetVal.getFieldString("YD_GP"), "");				
				if (ydGp.equals("M"))	outRec.setField("MSG_ID"   , "YDE7L001"); //항만슬라브야드 기능추가 - 2016.01.07 LeeJY
				...
				// Debug MSG
				ydUtils.putLog(szClassName, szMethodName, "\n======================makeY1L001() OUT("+ Integer.toString(nIdx) + ")==========================\n", YdConstant.DEBUG);	
				ydUtils.displayRecord(szOperationName, outRec);
				ydUtils.putLog(szClassName, szMethodName, "\n================================================================\n", YdConstant.DEBUG);
			}					
		}catch(Exception e){
			szMsg = "Y1(C연주슬라브야드L2) 송신  저장위치제원  데이터 반환 중 예외발생! 예외메세지: " + e.getMessage();
			ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.ERROR);		
			return -1;
		}
	 ...
	 * @return JDTORecord msgRec
	 */
	public static int makeY1L002(JDTORecord inRec, JDTORecordSet outRecSet){
		//		1.	전문 ID						MSG_ID					VARCHAR2(8)		YDY1L002
		//		2.	생성일						DATE					VARCHAR2(10)	YYYY-MM-DD
		//		3.	생성시간						TIME					VARCHAR2(8)		24HH-MM-SS
		//		4.	전문구분						MSG_GP					VARCHAR2(1)		I(신규), U(수정), D(취소,삭제), R(재 전송)
		//		5.	전문길이						MSG_LEN					NUMBER  (4)		
		...
		String szOperationName  = "C연주슬라브L2 저장품제원";
		String szMsg            = "";
		...
		try{
			...
			if(intRtnVal < 0) {
				szMsg = "저장품 + 적치BED + 적치단  테이블 조회오류 YD_INFO_SYNC_CD(" + szYD_INFO_SYNC_CD + ") YD_STK_COL_GP(" + szYD_STK_COL_GP + ") YD_STK_BED_NO(" + szYD_STK_BED_NO + ") STLNO(" + szSTLNO +  ") [Ret : " + intRtnVal + "]";
				ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.ERROR);			
				return 0;
			} else if(intRtnVal == 0) {
				szMsg = "저장품 + 적치BED + 적치단  테이블 조회건수 없음  YD_INFO_SYNC_CD(" + szYD_INFO_SYNC_CD + ") YD_STK_COL_GP(" + szYD_STK_COL_GP + ") YD_STK_BED_NO(" + szYD_STK_BED_NO + ") STLNO(" + szSTLNO +  ") [Ret : " + intRtnVal + "]";
				ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.DEBUG);			
				return 0;
			} else {
				szMsg = "저장품 + 적치BED + 적치단  테이블 조회성공 YD_INFO_SYNC_CD(" + szYD_INFO_SYNC_CD + ") YD_STK_COL_GP(" + szYD_STK_COL_GP + ") YD_STK_BED_NO(" + szYD_STK_BED_NO + ") STLNO(" + szSTLNO +  ") [Ret : " + intRtnVal + "]";
				ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.DEBUG);				
			}
			...
			for(int nIdx=0; nIdx<intRtnVal; nIdx++) {
				...
				// 헤더부
				outRec = JDTORecordFactory.getInstance().create();									
				outRec.setField("MSG_ID" , "YDY1L002");			
				//항만슬라브야드 기능추가 - 2016.01.07 LeeJY
				String ydGp = StringHelper.evl(recGetVal.getFieldString("YD_GP"), "");				
				
				if("".equals(ydGp)) ydGp = StringHelper.evl(recGetVal.getFieldString("YD_AIM_YD_GP"), "");				
				if ("M".equals(ydGp)) {
					outRec.setField("MSG_ID"   , "YDE7L002"); 
					szMsg = "   -----> " + szSTLNO + "  : 항만야드 제원정보연결";
					ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.DEBUG);				
				}
				...
				szSCARFING_DEPTH = YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal2, "SCARFING_DEPTH"), 2, 1);		        		
				...
				// 압연공장구분 [조업공장구분]
				szMILL_PLNT_GP = YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal, "PTOP_PLNT_GP"), 2, 1);	
				...
				outRec.setField("CT_LOT_SCH_SERNO", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal, "CT_LOT_SCH_SERNO"), 22, 1));
				...
				// Debug MSG
				ydUtils.putLog(szClassName, szMethodName, "\n======================makeY1L002() OUT("+ Integer.toString(nIdx) + ")==========================\n", YdConstant.DEBUG);	
				ydUtils.displayRecord(szOperationName, outRec);
				ydUtils.putLog(szClassName, szMethodName, "\n================================================================\n", YdConstant.DEBUG);
			} 			
		}catch(Exception e){
			szMsg = "Y1(C연주슬라브야드L2) 송신  저장품제원  데이터 반환 중 예외발생! 예외메세지: " + e.getMessage();
			ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.ERROR);		
			return -1;
		}
	 ...
	 * @return JDTORecord msgRec
	 */
	public static int makeY1L003(JDTORecord inRec, JDTORecordSet outRecSet){
		//		1.	전문 ID						MSG_ID					VARCHAR2(8)		YDY1L003
		//		2.	생성일						DATE					VARCHAR2(10)	YYYY-MM-DD
		//		3.	생성시간						TIME					VARCHAR2(8)		24HH-MM-SS
		//		4.	전문구분						MSG_GP					VARCHAR2(1)		I(신규), U(수정), D(취소,삭제), R(재 전송)
		//		5.	전문길이						MSG_LEN					NUMBER  (4)		
		...
		//		11.	조업공장구분					PTOP_PLNT_GP			VARCHAR2(2)
		...
		// 변수선언
		String szMethodName    = "makeY1L003";
		String szMsg           = "";
		String szOperationName = "C연주슬라브L2 크레인작업계획";
		...
		try{
			...
			// 압연공장구분 (2자리로 변경 되었음 P B C => 2자리로 바뀌었음) : C연주슬라브 => A야드 => 조업공장코드CC 혹은 CZ
			szYdGp = ydDaoUtils.paraRecChkNull(inRec, "YD_GP").trim();
			/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			 * 임춘수 수정 2009.06.17
			 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
			if(!szYdGp.equals("A")){
				szMsg = "[크레인작업계획]C연주슬라브야드[A]가 아닙니다. - 전달된 야드구분[" + szYdGp + "]";
				ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.DEBUG);
				return 0;
			}
			szYD_INFO_SYNC_CD = ydDaoUtils.paraRecChkNull(inRec, "YD_INFO_SYNC_CD");
			if( szYD_INFO_SYNC_CD.equals("H") ) {				//H : C열연가열로보급 - C연주슬라브야드인 경우
				szPtopPlntGp = "HC";	//C열연
				intGp = 87;
				szMsg = "[크레인작업계획]Y1(C연주슬라브야드L2) 송신  크레인작업계획  데이터 전송 시 C열연가열로보급인 야드정보동기화코드[" + szYD_INFO_SYNC_CD + "] 입니다.";
				ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.DEBUG);
			}else if( szYD_INFO_SYNC_CD.equals("P") ) {			//P : A후판가열로보급 - A후판슬라브야드인 경우 - 발생안함
				szPtopPlntGp = "PA";	//A후판
				intGp	= 13;
				szMsg = "[크레인작업계획]Y1(C연주슬라브야드L2) 송신  크레인작업계획  데이터 전송 시 A후판가열로보급인 야드정보동기화코드[" + szYD_INFO_SYNC_CD + "] 입니다.";
				ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.DEBUG);
			}else if( szYD_INFO_SYNC_CD.equals("Q") ) {			//Q : B후판가열로보급 - B후판슬라브야드인 경우 - 발생안함
				szPtopPlntGp = "PB";	//B후판
				intGp	= 13;
				szMsg = "[크레인작업계획]Y1(C연주슬라브야드L2) 송신  크레인작업계획  데이터 전송 시 B후판가열로보급인 야드정보동기화코드[" + szYD_INFO_SYNC_CD + "] 입니다.";
				ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.DEBUG);
			}else{
				szMsg = "[크레인작업계획]Y1(C연주슬라브야드L2) 송신  크레인작업계획  데이터 전송 시 지원하지 않는 야드정보동기화코드[" + szYD_INFO_SYNC_CD + "] 입니다.";
				ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.DEBUG);
				return 0;
			}
			/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
			...
			for(int nIdx=0; nIdx<intRtnVal; nIdx++) {
				...
				// 헤더부
				outRec = JDTORecordFactory.getInstance().create();									
				outRec.setField("MSG_ID" , "YDY1L003");
				outRec.setField("DATE"   , YdUtils.getCurDate("yyyy-MM-dd"));
				outRec.setField("TIME"   , YdUtils.getCurDate("HH-mm-ss"));
				outRec.setField("MSG_GP" , "I");
				outRec.setField("MSG_LEN", YdUtils.fillSpZr("" + nTcLen, 4, 0));
			...
			szMsg = "[크레인작업계획]Y1(C연주슬라브야드L2) 송신  크레인작업계획  데이터 반환 중 예외발생! 예외메세지: " + e.getMessage();
			ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.ERROR);
			return -1;
		}
	...
	/**
	 * makeY1LXX2 : 저장품 제원 일괄지시 
	 * @param JDTORecord inRec
	 * @return JDTORecord msgRec
	 */
	public static int makeY1LXX2(JDTORecord inRec, JDTORecordSet outRecSet){
		//		1.	전문 ID						MSG_ID					VARCHAR2(8)		YDY1L003
		//		2.	생성일						DATE					VARCHAR2(10)	YYYY-MM-DD
		//		3.	생성시간						TIME					VARCHAR2(8)		24HH-MM-SS
		//		4.	전문구분						MSG_GP					VARCHAR2(1)		I(신규), U(수정), D(취소,삭제), R(재 전송)
		...
		try{
			...
			for(int nIdx=0; nIdx<intRtnVal; nIdx++) {
				...
				// SCARFING깊이 [SCARFING깊이]
				outRec.setField("SCARFING_DEPTH", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal2, "SCARFING_DEPTH"), 2, 1));
	 ...
	 * @return JDTORecord msgRec
	 */
	public static int makeY1L004(JDTORecord inRec, JDTORecordSet outRecSet){
		//		1.	전문 ID					MSG_ID					VARCHAR2(8)		YDY1L004
		//		2.	생성일					DATE					VARCHAR2(10)	YYYY-MM-DD
		//		3.	생성시간					TIME					VARCHAR2(8)		24HH-MM-SS
		//		4.	전문구분					MSG_GP					VARCHAR2(1)		I(신규), U(수정), D(취소,삭제), R(재 전송)
		//		5.	전문길이					MSG_LEN					NUMBER  (4)		
		...
		//		42	야드차량사용구분			YD_CAR_USE_GP			VARCHAR2(1)	        권상 또는 권하위치가 차량인 경우("L" 구내운송차량, "G" 제품출하차량)
		//		43	차량번호					CAR_NO					VARCHAR2(15)	권상 또는 권하위치가 제품출하차량인 경우
		//		44	운송장비코드				TRN_EQP_CD				VARCHAR2(8)	        권상 또는 권하위치가 구내운송차량인 경우
		...
		//		GROUP[10]----------------------------------------------------------------------END
		//		52	야드스케쥴코드_Next		YD_SCH_CD_NEXT			VARCHAR2(8)		크레인스케줄에 등록된 다음 작업
		...
		// 변수선언
		String szMethodName       = "makeY1L004";
		String szMsg              = "";
		String szOperationName    = "C연주슬라브L2 크레인작업지시";
		String szYD_SCH_CD        = "";  
		...
		try{
			...
			// 헤더부
			outRec.setField("MSG_ID" , "YDY1L004");
			outRec.setField("DATE"   , YdUtils.getCurDate("yyyy-MM-dd"));
			outRec.setField("TIME"   , YdUtils.getCurDate("HH-mm-ss"));
			...
			// 야드설비ID [야드설비ID]
			outRec.setField("YD_EQP_ID", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_EQP_ID"), 6, 1));
			...
			if(!szYD_SCH_CD.trim().equals("")){
				...
				if(intRtnVal2 < 0) {
					...
					szYD_SCH_NAME = YdUtils.fillSpZr_KOR(ydDaoUtils.paraRecChkNull(recGetVal2, "CD_CONTENTS"), 30, 1);
					szYD_EQP_ID2 = YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal2, "YD_ALT_CRN"), 6, 1);
				}
			} else {
			...
			outRec.setField("YD_CRN_SCH_ID", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_CRN_SCH_ID"), 18, 1));
			
			// 야드크레인작업매수 [야드크레인작업매수]
			outRec.setField("YD_CRN_WRK_SH", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_EQP_WRK_SH"), 2, 1));
			
			// 야드크레인작업중량 [야드크레인작업중량]
			outRec.setField("YD_CRN_WRK_WT", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_EQP_WRK_WT"), 7, 1));
			
			// 야드크레인작업총두께 [야드크레인작업총두께]
			szConv = YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_EQP_WRK_T"), 8, 1);
			outRec.setField("YD_CRN_WRK_T", ydUtils.FloatLRPAD(szConv, 7, 3, '0'));	

			// 야드크레인작업최대폭 [야드크레인작업최대폭]
			szConv = YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_EQP_WRK_MAX_W"), 6, 1);
			outRec.setField("YD_CRN_WRK_MAX_W", ydUtils.FloatLRPAD(szConv, 5, 1, '0'));	
			
			// 야드크레인작업최대길이 [야드크레인작업최대길이]
			outRec.setField("YD_CRN_WRK_MAX_L", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_EQP_WRK_MAX_L"), 7, 1));
			...
			// 야드권상지시위치 [야드권상지시위치]
			outRec.setField("YD_UP_WO_LOC", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_UP_WO_LOC"), 8, 1));
			
			// 야드권상지시단 [야드권상지시단]
			outRec.setField("YD_UP_WO_LAYER", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_UP_WO_LAYER"), 3, 1));
			
			// 야드권상지시X축[야드권상지시X축]
			outRec.setField("YD_UP_WO_LOC_XAXIS", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_UP_WO_LOC_XAXIS"), 7, 1));
			
			// 야드권상지시X축오차최대 [야드권상지시X축오차최대]
			outRec.setField("YD_UP_WO_XAXIS_GAP_MAX", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_UP_WO_XAXIS_GAP_MAX"), 5, 1));
			
			// 야드권상지시X축오차최소 [야드권상지시X축오차최소]
			outRec.setField("YD_UP_WO_XAXIS_GAP_MIN", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_UP_WO_XAXIS_GAP_MIN"), 5, 1));
			
			// 야드권상지시Y축 [야드권상지시Y축]
			outRec.setField("YD_UP_WO_LOC_YAXIS", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_UP_WO_LOC_YAXIS"), 5, 1));
			
			// 야드권상지시Y축오차최대 [야드권상지시Y축오차최대]
			outRec.setField("YD_UP_WO_YAXIS_GAP_MAX", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_UP_WO_YAXIS_GAP_MAX"), 5, 1));
			
			// 야드권상지시Y축오차최소 [야드권상지시Y축오차최소]
			outRec.setField("YD_UP_WO_YAXIS_GAP_MIN", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_UP_WO_YAXIS_GAP_MIN"), 5, 1));
			
			// 야드권상지시Z축 [야드권상지시Z축]
			outRec.setField("YD_UP_WO_LOC_ZAXIS", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_UP_WO_LOC_ZAXIS"), 5, 1));
			
			// 야드권상지시Z축오차최대 [야드권상지시Z축오차최대]
			outRec.setField("YD_UP_WO_ZAXIS_GAP_MAX", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_UP_WO_ZAXIS_GAP_MAX"), 5, 1));
			
			// 야드권상지시Z축오차최소 [야드권상지시Z축오차최소]
			outRec.setField("YD_UP_WO_ZAXIS_GAP_MIN", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_UP_WO_ZAXIS_GAP_MIN"), 5, 1));
			
			// 야드권하지시위치 [야드권하지시위치]
			outRec.setField("YD_DN_WO_LOC", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_DN_WO_LOC"), 8, 1));
			
			// 야드권하지시단 [야드권하지시단]
			outRec.setField("YD_DN_WO_LAYER", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_DN_WO_LAYER"), 3, 1));
			
			// 야드권하지시X축 [야드권하지시X축]
			outRec.setField("YD_DN_WO_LOC_XAXIS", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_DN_WO_LOC_XAXIS"), 7, 1));
			
			// 야드권하지시X축오차최대 [야드권하지시X축오차최대]
			outRec.setField("YD_DN_WO_XAXIS_GAP_MAX", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_DN_WO_XAXIS_GAP_MAX"), 5, 1));
			
			// 야드권하지시X축오차최소 [야드권하지시X축오차최소]
			outRec.setField("YD_DN_WO_XAXIS_GAP_MIN", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_DN_WO_XAXIS_GAP_MIN"), 5, 1));
			
			// 야드권하지시Y축 [야드권하지시Y축]
			outRec.setField("YD_DN_WO_LOC_YAXIS", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_DN_WO_LOC_YAXIS"), 5, 1));
			
			// 야드권하지시Y축오차최대 [야드권하지시Y축오차최대]
			outRec.setField("YD_DN_WO_YAXIS_GAP_MAX", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_DN_WO_YAXIS_GAP_MAX"), 5, 1));
			
			// 야드권하지시Y축오차최소 [야드권하지시Y축오차최소]
			outRec.setField("YD_DN_WO_YAXIS_GAP_MIN", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_DN_WO_YAXIS_GAP_MIN"), 5, 1));
			
			// 야드권하지시Z축 [야드권하지시Z축]
			outRec.setField("YD_DN_WO_LOC_ZAXIS", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_DN_WO_LOC_ZAXIS"), 5, 1));
			
			// 야드권하지시Z축오차최대 [야드권하지시Z축오차최대]
			outRec.setField("YD_DN_WO_ZAXIS_GAP_MAX", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_DN_WO_ZAXIS_GAP_MAX"), 5, 1));
			
			// 야드권하지시Z축오차최소 [야드권하지시Z축오차최소]
			outRec.setField("YD_DN_WO_ZAXIS_GAP_MIN", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_DN_WO_ZAXIS_GAP_MIN"), 5, 1));			
			...
			if(intRtnVal2 < 0) {
				...
				// 야드대차목적동[야드대차목적동]
				outRec.setField("YD_TC_AIM_BAY_GP", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal2, "YD_AIM_BAY_GP"), 1, 1));

				// 야드차량사용구분 [야드차량사용구분] 
				outRec.setField("YD_CAR_USE_GP", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal2, "YD_CAR_USE_GP"), 1, 1));
					
				// 차량번호 [차량번호] 
				outRec.setField("CAR_NO", YdUtils.fillSpZr_KOR(ydDaoUtils.paraRecChkNull(recGetVal2, "CAR_NO"), 15, 1));
					
				// 운송장비코드 [운송장비코드] 
				outRec.setField("TRN_EQP_CD", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal2, "TRN_EQP_CD"), 8, 1));
			}											

			// 야드설비작업매수 [야드설비작업매수]                                                                                    
			outRec.setField("YD_EQP_WRK_SH", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_EQP_WRK_SH"), 2, 1)); 				
			...
			for(nIdx=0; nIdx<intRtnVal1; nIdx++) {
				...
				// 재료번호 [재료번호]
				outRec.setField("STL_NO"+(1+nIdx), YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "STL_NO"), 11, 1));
				
				// 야드재료중량 [야드재료중량]
				outRec.setField("YD_STL_WT"+(1+nIdx), YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_MTL_WT"), 5, 1));
				
				// 야드재료두께 [야드재료두께]
				szConv = YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_MTL_T"), 7, 1);
				outRec.setField("YD_STL_T"+(1+nIdx), ydUtils.FloatLRPAD(szConv, 6, 3, '0'));	

				// 야드재료폭 [야드재료폭]
				szConv = YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_MTL_W"), 6, 1);
				outRec.setField("YD_STL_W"+(1+nIdx), ydUtils.FloatLRPAD(szConv, 5, 1, '0'));	
				
				// 야드재료길이 [야드재료길이]
				outRec.setField("YD_STL_L"+(1+nIdx), YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_MTL_L"), 7, 1));				
			}
			...
			if(!szYD_SCH_CD_NEXT.trim().equals("")){
				...
				intRtnVal2 = ydSchRuleDao.getYdSchrule(recPara2, rsResult2, 0);
				if(intRtnVal2 < 0) {
					szMsg = "스케쥴 기준 테이블_NEXT 조회오류 (" + szYD_SCH_CD_NEXT + ")" + "[Ret : " + intRtnVal2 + "]";
					ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.ERROR);			
	
					szYD_SCH_NAME_NEXT = YdUtils.fillSpZr_KOR(" ", 30, 1);	
				} else if(intRtnVal2 == 0) {
					szMsg = "스케쥴 기준 테이블_NEXT 조회건수 없음 (" + szYD_SCH_CD_NEXT + ")" + "[Ret : " + intRtnVal2 + "]";
					ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.DEBUG);			
					...
					szYD_SCH_NAME_NEXT = YdUtils.fillSpZr_KOR(ydDaoUtils.paraRecChkNull(recGetVal2, "CD_CONTENTS"), 30, 1);
				}
			} else {
			...
			outRec.setField("YD_UP_WO_LOC_NEXT", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_UP_WO_LOC_NEXT"), 8, 1));
				
			// 야드권상지시단_Next [야드권상지시단_Next]
			outRec.setField("YD_UP_WO_LAYER_NEXT", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_UP_WO_LAYER_NEXT"), 3, 1));
			
			// 야드권하지시위치_Next [야드권하지시위치_Next]
			outRec.setField("YD_DN_WO_LOC_NEXT", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_DN_WO_LOC_NEXT"), 8, 1));
			
			// 야드권하지시단_Next [야드권하지시단_Next]
			outRec.setField("YD_DN_WO_LAYER_NEXT", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_DN_WO_LAYER_NEXT"), 3, 1));
			
			// 재료번호_Next [재료번호_Next]
			outRec.setField("STL_NO_NEXT", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "STL_NO_NEXT"), 11, 1));
			
			// 야드크레인작업매수_Next [야드설비작업매수]
			outRec.setField("YD_CRN_WRK_SH_NEXT", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_EQP_WRK_SH_NEXT"), 2, 1));
			
			// 야드크레인작업중량_Next [야드크레인작업중량_Next]
			outRec.setField("YD_CRN_WRK_WT_NEXT", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_EQP_WRK_WT_NEXT"), 7, 1));
			
			// 야드크레인작업총두께_Next [야드크레인작업총두께_Next]
			szConv = YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_EQP_WRK_T_NEXT"), 8, 1);
			outRec.setField("YD_CRN_WRK_T_NEXT", ydUtils.FloatLRPAD(szConv, 7, 3, '0'));					
			
			// 야드크레인작업최대폭_Next [야드크레인작업최대폭_Next]
			szConv = YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_EQP_WRK_MAX_W_NEXT"), 6, 1);
			outRec.setField("YD_CRN_WRK_MAX_W_NEXT", ydUtils.FloatLRPAD(szConv, 5, 1, '0'));					
			
			// 야드크레인작업최대길이_Next [야드크레인작업최대길이_Next]
			outRec.setField("YD_CRN_WRK_MAX_L_NEXT", YdUtils.fillSpZr(ydDaoUtils.paraRecChkNull(recGetVal1, "YD_EQP_WRK_MAX_L_NEXT"), 7, 1));				
			...
			szMsg = "Y1(C연주슬라브야드L2) 송신  크레인작업지시  데이터 반환 중 예외발생! 예외메세지: " + e.getMessage();
			ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.ERROR);		
			return -1;
		}
	 ...
	 * @return JDTORecord msgRec
	 */
	public static int makeY1L005(JDTORecord inRec, JDTORecordSet outRecSet){
		//		1.	전문 ID				MSG_ID				VARCHAR2(8)		YDY1L005
		//		2.	생성일				DATE				VARCHAR2(10)	YYYY-MM-DD
		//		3.	생성시간				TIME				VARCHAR2(8)		24HH-MM-SS
		//		4.	전문구분				MSG_GP				VARCHAR2(1)		I(신규), U(수정), D(취소,삭제), R(재 전송)
		//		5.	전문길이				MSG_LEN				NUMBER  (4)		
		...
		// 변수선언
		String szMethodName     = "makeY1L005";
		String szMsg            = "";
		String szOperationName  = "C연주슬라브L2 크레인작업실적응답";
		...
		try{
			...
			// 헤더부
			outRec.setField("MSG_ID" , "YDY1L005");
			outRec.setField("DATE"   , YdUtils.getCurDate("yyyy-MM-dd"));
			outRec.setField("TIME"   , YdUtils.getCurDate("HH-mm-ss"));
			outRec.setField("MSG_GP" , "I");
			outRec.setField("MSG_LEN", YdUtils.fillSpZr("" + nTcLen, 4, 0));
			...
			szMsg = "Y1(C연주슬라브야드L2) 송신  크레인작업실적응답  데이터 반환 중 예외발생! 예외메세지: " + e.getMessage();
			ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.ERROR);
			return -1;
		}
