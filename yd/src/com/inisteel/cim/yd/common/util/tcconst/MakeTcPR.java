...
public class MakeTcPR {
	 ...
	 */
	public static int makePRJ001(JDTORecord inRec, JDTORecordSet outRecSet){
		//		1	JMS_TC_CD			JMSTCCODE 			VARCHAR2(8)			YDPRJ001
		...
		// 변수선언
		String szMethodName              = "makePRJ001";
		String szMsg                     = "";
		String szOperationName           = "후판조업 후판제품반납하차실적";
		...
		try{
			...
			if(!szYD_CRN_SCH_ID.equals("")){
				...
				// 헤더부
				outRec.setField("JMS_TC_CD",          "YDPRJ001");
			...
			szMsg = "PR(후판조업) 송신 후판제품반납하차실적  데이터 반환 중 예외발생! 예외메세지: " + e.getMessage();
			ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.ERROR);							
			return -1;
		}	
	 ...
	 * @return JDTORecord msgRec
	 */
	public static int makePRJ002(JDTORecord inRec, JDTORecordSet outRecSet){
		//		1	JMS_TC_CD			JMSTCCODE 			VARCHAR2(8)			YDPRJ002
		//		2	JMS_TC_CREATE_DDTT	JMSTC생성일시			DATE	(14)		YYYYMMDDHHMMSS (24시간개념)
		...
		// 변수선언
		String szMethodName       = "makePRJ002";
		String szMsg              = "";
		String szOperationName    = "후판조업 후판제품입고상차실적";
		...
		try{
			...
			// 헤더부
			outRec.setField("JMS_TC_CD", 			"YDPRJ002");
			outRec.setField("JMS_TC_CREATE_DDTT", 	YdUtils.getCurDate("yyyyMMddHHmmss"));
			...
			// 발지개소코드 [발지개소코드]
			outRec.setField("SPOS_WLOC_CD", ydDaoUtils.paraRecChkNull(recGetVal, "SPOS_WLOC_CD"));

			// 발지야드포인트코드 [상차정지위치 = 적치열구분]
			szEqpWrkStat =  ydDaoUtils.paraRecChkNull(recGetVal, "YD_EQP_WRK_STAT").trim();
			if(szEqpWrkStat != "" && szEqpWrkStat.equals("U")){
				recParaCol.setField("YD_STK_COL_GP", ydDaoUtils.paraRecChkNull(recGetVal, "YD_CARLD_STOP_LOC").trim());
				intRtnValCol = ydStkColDao.getYdStkcol(recParaCol, rsResultCol, 0);
				if(intRtnValCol > 0){
					for(int nIdx2=0; nIdx2<intRtnValCol; nIdx2++) {
						recGetValCol = rsResultCol.getRecord(nIdx2);
					...
					szMsg = "적치열 테이블 조회오류  [Ret : " + intRtnVal + "]";
					ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.ERROR);					
				}				
			}
			outRec.setField("SPOS_YD_PNT_CD", szSposYdPndCd);
			...
			// 운송작업재료구분 [야드재료품목]
			outRec.setField("TRN_WRK_MTL_GP", ydDaoUtils.paraRecChkNull(recGetVal, "YD_MTL_ITEM"));

			// 재료긴급구분 [긴급이송지작업지시구분]
			outRec.setField("MTL_UGNT_GP", ydDaoUtils.paraRecChkNull(recGetVal, "URGENT_FRTOMOVE_WORD_GP"));

			// HCR구분 [HCR구분]
			outRec.setField("HCR_GROUP", ydDaoUtils.paraRecChkNull(recGetVal, "HCR_GP"));

			// 상차매수 [야드설비작업매수]
			outRec.setField("CARLD_SH", ydDaoUtils.paraRecChkNull(recGetVal, "YD_EQP_WRK_SH"));
			...
			szMsg = "PR(후판조업) 송신 후판제품입고상차실적  데이터 반환 중 예외발생! 예외메세지: " + e.getMessage();
			ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.ERROR);				
			return -1;
		}	
	 ...
	 * @return JDTORecord msgRec
	 */
	public static int makePRJ003(JDTORecord inRec, JDTORecordSet outRecSet){
		//		1	JMS_TC_CD			JMSTCCODE 			VARCHAR2(8)			YDPRJ003
		//		2	JMS_TC_CREATE_DDTT	JMSTC생성일시			DATE	(14)		YYYYMMDDHHMMSS (24시간개념)
		//		3	STL_NO              재료번호			    VARCHAR2(11)        SLAB번호	       (운송지시 Table 내에 저장된 재료번호)
		//		4	EXT_DT              불출일시                           VARCHAR2(14)        YYYYMMDDHHMMSS (야드적치일시)
		...
		// 변수선언
		String szMethodName       = "makePRJ003";
		String szMsg              = "";
		String szOperationName    = "후판조업 후판재열재슬라브적치실적";
		String szSTL_No           = "";
		...
		try{
			...
			// 전문편성
			outRec.setField("JMS_TC_CD"         , "YDPRJ003");
			outRec.setField("JMS_TC_CREATE_DDTT", YdUtils.getCurDate("yyyyMMddHHmmss"));
			outRec.setField("STL_NO"            , szSTL_No);										
			outRec.setField("EXT_DT"            , YdUtils.getCurDate("yyyyMMddHHmmss"));
			...
			szMsg = "PR(후판조업)송신 후판재열재슬라브적치실적  데이터 반환 중 예외발생! 예외메세지: " + e.getMessage();
			ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.ERROR);				
			return -1;
		}	
	...
	/**
	 * YDPRJ004 : 후판정정야드 가스절단실적 
	 * @param JDTORecord inRec
	 * @return JDTORecord msgRec
	 */
	public static int makePRJ004(JDTORecord inRec, JDTORecordSet outRecSet){
		...
		// 변수선언
		String szMethodName       = "makePRJ004";
		String szMsg              = "";
		String szOperationName    = "후판정정야드 가스절단실적";
		String szSTL_No           = "";
		...
		try{
			...
			// 수신전문에서 항목 추출
			szSTL_No = ydDaoUtils.paraRecChkNull(inRec, "PL_MPL_NO");
			String szPL_WR_PRSNT_PROC_CD = ydDaoUtils.paraRecChkNull(inRec, "PL_WR_PRSNT_PROC_CD");
			String szPL_WR_ELE_PROC_CD = ydDaoUtils.paraRecChkNull(inRec, "PL_WR_ELE_PROC_CD");
			String szMTL_STAT_CD = ydDaoUtils.paraRecChkNull(inRec, "MTL_STAT_CD");
			String szMODIFIER = ydDaoUtils.paraRecChkNull(inRec, "MODIFIER");
			...
			// 전문편성
			outRec.setField("JMS_TC_CD"         , "YDPRJ004");
			outRec.setField("JMS_TC_CREATE_DDTT", YdUtils.getCurDate("yyyyMMddHHmmss"));
			outRec.setField("PL_MPL_NO"            , szSTL_No);
			outRec.setField("PL_WR_PRSNT_PROC_CD"            , szPL_WR_PRSNT_PROC_CD);		
			outRec.setField("PL_WR_ELE_PROC_CD"            , szPL_WR_ELE_PROC_CD);
			outRec.setField("MTL_STAT_CD"            , szMTL_STAT_CD);
			outRec.setField("MODIFIER"            , szMODIFIER);
			...
			szMsg = "PR(후판조업)후판정정야드 가스절단실적   데이터 반환 중 예외발생! 예외메세지: " + e.getMessage();
			ydUtils.putLog(szClassName, szMethodName, szMsg, YdConstant.ERROR);				
			return -1;
		}	
