...
//*/
public class M10YdExLm21SenderFaEJBBean { //extends BaseSessionBean{
...
//	*/
	public HashMap SendMessage(final HashMap paramMap) throws IOException, TimeoutException, Exception{
		...
		try {
			...
			piYdUtils.printLog("MES_PI_RABBIT_MQ", "===== M10YdExLm21SenderFaEJB(후판야드)송신Message 전문: >>>>>>>>>> :" + paramMap, "S+");
			
		} 
		catch (IOException e) {
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10YdExLm21SenderFaEJB(후판야드송신) IOException Error " + e.getMessage());
		}
		catch (Exception e) {
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10YdExLm21SenderFaEJB(후판야드송신) Exception Error " + e.getMessage());
		}
	...
	public HashMap RabbitConnect(String keyName, String exchangeName, String ydSndYn)  throws IOException, TimeoutException, Exception{
		...
		try {
			...
			if ("Y".equals(ydSndYn)) { // 야드 재송신
				...
				this.EXCHANGENAME = propertyService.getProperty("common.properties","rabbitmq.YD.M10YD-EX-LM-21");
				this.ROUTINGKEY   = propertyService.getProperty("common.properties","rabbitmq.YD.m10yd.lm.21");
			}	
//LOCAL
//			this.EXCHANGENAME = "M10YD-Q-LM-21S";   
//			this.ROUTINGKEY   = "m10yd.lm.21S";  
			...
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10YdExLm21SenderFaEJB(후판야드송신) RabbitConnect IOException ======" + e.getMessage());
		}
		catch (TimeoutException e) {
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10YdExLm21SenderFaEJB(후판야드송신) RabbitConnect TimeoutException ======" + e.getMessage());
		}
		catch (Exception e) {
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10YdExLm21SenderFaEJB(후판야드송신) Exception ======" + e.getMessage());
		}

		piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10YdExLm21SenderFaEJB(후판야드송신) RabbitConnected ======OK");
...
//	*/
	public void RabbitDisConnect() throws IOException, TimeoutException{
		...
		piYdUtils.printLog("MES_PI_RABBIT_MQ","", "=====M10YdExLm21SenderFaEJB(후판야드송신) RabbitDisConnect called");
	}
