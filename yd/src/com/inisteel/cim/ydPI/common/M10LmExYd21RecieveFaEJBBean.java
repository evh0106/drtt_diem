 ...
 * @ejb.bean name="M10LmExYd21RecieveFaEJB" jndi-name="M10LmExYd21RecieveFaEJB" type="Stateless"
...
*/
public class M10LmExYd21RecieveFaEJBBean extends BaseSessionBean{
	...
	public void ejbCreate() throws javax.ejb.CreateException {
		...
		try {//HS(조업서버), TM(물류서버), PM(진행서버)중 선택
			...
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd21RecieveFaEJB(후판야드수신) ejbCreate IOException error :" + e.getMessage());
		}
		catch (TimeoutException e) {
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd21RecieveFaEJB(후판야드수신) ejbCreate TimeoutException error :" + e.getMessage());
		}		
		catch (Exception e) {
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd21RecieveFaEJB(후판야드수신) ejbCreate Exception error :" + e.getMessage());
		}
	}
	...
	public void ejbRemove() {
    	...
    	piYdUtils.printLog("MES_PI_RABBIT_MQ", "", "===== M10LmExYd21RecieveFaEJB(후판야드수신) ejbRemove called");
    	...
    	try {
			...
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd21RecieveFaEJB(후판야드수신) ejbRemove error :" + e1.getMessage());
		}
		catch (TimeoutException e2) {
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd21RecieveFaEJB(후판야드수신) ejbRemove error :" + e2.getMessage());
		}		
		catch (Exception e3) {
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd21RecieveFaEJB(후판야드수신) ejbRemove error :" + e3.getMessage());
		}
	...
	*/
	public void RabbitConnect() throws IOException, TimeoutException, Exception{
		try {
			...
			this.QNAME        = propertyService.getProperty("common.properties","rabbitmq.YD.M10LM-Q-YD-21");
			...
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "=====M10LmExYd21RecieveFaEJB(후판야드수신) ejbRemove error :" + e.getMessage());
		}
		catch (TimeoutException e) {
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "=====M10LmExYd21RecieveFaEJB(후판야드수신) ejbRemove error :" + e.getMessage());
		}		
		...
		piYdUtils.printLog("MES_PI_RABBIT_MQ", "", "===== M10LmExYd21RecieveFaEJB(후판야드수신)       RabbitConnected OK");
	...
	*/
	public void RabbitDisConnect() throws IOException, TimeoutException{
		...
		piYdUtils.printLog("MES_PI_RABBIT_MQ", "", "===== M10LmExYd21RecieveFaEJB(후판야드수신) RabbitDisConnect called");
	}
	...
	*/
		...
		Consumer consumer = new DefaultConsumer(this.channel)
		{
	    	...
	    	public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties basicproperties, byte[] message) throws IOException{
    			...
    			try {
					...
					piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd21RecieveFaEJB(후판야드)수신Message 전문: <<<<<<<<<< : " + recievedMessage);

					//받은 메세지를 처리하는 함수 호출
					MessageProcessYd21(recievedMessage);
					...
					piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd21RecieveFaEJB(후판야드) StartReceive handleErrorMessage error " + e.getMessage());
				}//end of catch
	 ...
	 * @throws Exception
	*/
	public HashMap MessageProcessYd21( String paramMap ) throws JSONException,Exception{
...
// 1201 추가			
			...
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd21RecieveFaEJB(후판야드수신) MessageProcessYd21 JSONException " + e.getMessage());
			AlimtalkSendUtil.SendAlimTalk(this.QNAME, "M10LmExYd21RecieveFaEJB", e.getMessage());//알림톡전송
			httpRaggitMqlog.postReceiveLog(trId, recvTime, Util.Now_YYYYMMddHHmmss(), "E", "MessageProcessExam Exception:" +e.getMessage());

		} catch (Exception e) {
			piYdUtils.printLog("MES_PI_RABBIT_MQ","", "===== M10LmExYd21RecieveFaEJB(후판야드수신) MessageProcessYd21 Exception " + e.getMessage());
			AlimtalkSendUtil.SendAlimTalk(this.QNAME, "M10LmExYd21RecieveFaEJB", e.getMessage());//알림톡전송
			httpRaggitMqlog.postReceiveLog(trId, recvTime, Util.Now_YYYYMMddHHmmss(), "E", "MessageProcessExam Exception:" +e.getMessage());
		}
