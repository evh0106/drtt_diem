	...
	public boolean chkPara_getYdMtlstatmodhist(JDTORecord inRec, int intGp) throws JDTOException  {
		...
		try {
			if (intGp == 0) {
				...
				blnErr = ymCommonUtil.chkField(inRec, szFieldName, 11, 1, 'S', 0, 0);
...
//			if (!blnErr) return blnErr;
			...
			szFieldName = "V_STL_NO";
			blnErr = ymCommonUtil.chkField(inRec, szFieldName, 11, 2, 'S', 0, 0);
			if (!blnErr) return blnErr;
			...
			blnErr = ymCommonUtil.chkField(inRec, szFieldName, 3, 2, 'S', 0, 0);
			if (!blnErr) return blnErr;
			...
			blnErr = ymCommonUtil.chkField(inRec, szFieldName, 30, 2, 'S', 0, 0);
			if (!blnErr) return blnErr;
			...
			blnErr = ymCommonUtil.chkField(inRec, szFieldName, 7, 2, 'S', 0, 0);
			if (!blnErr) return blnErr;
			...
			blnErr = ymCommonUtil.chkField(inRec, szFieldName, 22, 2, 'S', 0, 0);
			if (!blnErr) return blnErr;
...
//			blnErr = ymCommonUtil.chkField(inRec, szFieldName, 4, 2, 'L', 0, 0);
//			if (!blnErr) return blnErr;
